<?php defined('ByShopWWI') or exit('Access Invalid!');?>
    <ul class="bannerImg inner">
    	 <?php if (is_array($output['code_screen_list']['code_info']) && !empty($output['code_screen_list']['code_info'])) {$i=0; ?>
          ...
          <li ap_id="<?php echo $val['ap_id'];?>" color="<?php echo $val['color'];?>" style="<?php echo $i==1 ? 'display: block':'display: none'?>; background: <?php echo $val['color'];?>;" class="">
            <a href="<?php echo $val['pic_url'];?>" target="_blank"><img alt="" shopwwi-url="<?php echo UPLOAD_SITE_URL.'/'.$val['pic_img'];?>" style="display:block" rel='lazy' src="<?php echo MALL_SITE_URL;?>/img/loading.gif" title="<?php echo $val['pic_name'];?>"></a>
        </li>	
          <?php }else { ?>
    ...
    <span class="wwi-prev" style="display: none;"></span>
    <span class="wwi-next" style="display: none;"></span>
