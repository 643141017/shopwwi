...
function collect_store(fav_id,jstype,jsobj){
    $.get('index.php?app=index&wwi=login', function(result){
        if(result=='0'){
            login_dialog();
        }else{
            var url = 'index.php?app=member_favorite_store&wwi=favoritestore';
...
function collect_goods(fav_id,jstype,jsobj){
    $.get('index.php?app=index&wwi=login', function(result){
        if(result=='0'){
            ...
            var url = 'index.php?app=member_favorite_goods&wwi=favoritegoods';
            $.getJSON(url, {'fid':fav_id}, function(data){
                if (data.done)
            {
                showDialog(data.msg, 'succ','','','','','','','','',2);
...
function load_cart_information(){
	$.getJSON(SITEURL+'/index.php?app=cart&wwi=ajax_load&callback=?', function(result){
...
function drop_topcart_item(cart_id){
    $.getJSON(SITEURL+'/index.php?app=cart&wwi=del&cart_id='+cart_id+'&callback=?', function(result){
        if(result.state){
            var obj = $('.mod_minicart');
            //删除成功
            if(result.quantity == 0){
    	        ...
    	        if ($('#rtoolbar_cartlist > ul').children().size() != result.quantity) {
    	        	$("#rtoolbar_cartlist").load(MALL_SITE_URL+ '/index.php?app=cart&wwi=ajax_load&type=html');return ;
    	        }
            }
        }else{
            alert(result.msg);
...
function load_history_information(){
    $.getJSON(SITEURL+'/index.php?app=index&wwi=viewed_info', function(result){
...
(function($) {
    $.show_nc_login = function(options) {
        var settings = $.extend({}, {action:'/index.php?app=login&wwi=index', nchash:'', formhash:'' ,anchor:''}, options);
        ...
        if (connect_wx == '1'){
            add_html += '<a href="javascript:void(0);" onclick="ajax_form(\'weixin_form\', \'微信账号登录\', \''+LOGIN_SITE_URL+'/index.php?app=connect_wx&wwi=index\', 360);" title="微信账号登录" class="wx"><i></i></a>';
        }
        add_html += '</span>';
        login_dialog_html.append('<form class="bg" method="post" id="ajax_login" action="'+LOGIN_SITE_URL+settings.action+'"></form>').find('form')
        	...
        	.append('<dl><dt>验证码</dt><dd><input type="text" size="10" maxlength="4" class="text fl w60" name="captcha"><img border="0" onclick="this.src=\'index.php?app=seccode&wwi=makecode&amp;nchash='+settings.nchash+'&amp;t=\' + Math.random()" name="codeimage" title="看不清，换一张" src="index.php?app=seccode&wwi=makecode&amp;nchash='+settings.nchash+'" class="fl ml10"><span>不区分大小写</span></dd></dl>')
        	.append('<ul><li>›&nbsp;如果您没有登录账号，请先<a class="register" href="'+LOGIN_SITE_URL+'/index.php?app=login&amp;wwi=register">注册会员</a>然后登录</li><li>›&nbsp;如果您<a class="forget" href="'+LOGIN_SITE_URL+'/index.php?app=login&wwi=forget_password">忘记密码</a>？，申请找回密码</li></ul>')
...
(function($){
	$.fn.membershipCard = function(options){
		var defaults = {
				type:''			// params  mall/tyq/tzx/micormall
			};
	                ...
	                ajax: {
		                ...
		                success: function(data) {
		                	if(data){
		                		...
		                		// ajax info
		                		if(data.url != ''){
			                		$.getJSON(data.url+'/index.php?app=member_card&wwi=mcard_info&uid='+data.id, function(d){
			                			if(d){
		                		...
		                		if(data.follow != 2){
		                			...
		                			$('<div class="send-msg"> <a href="'+MEMBER_SITE_URL+'/index.php?app=member_message&wwi=sendmsg&member_id='+data.id+'" target="_blank"><i></i>站内信</a> </div>').appendTo(_bottom);
		                		}
		...
		function follow(o){
			...
			$.getJSON(MEMBER_SITE_URL+'/index.php?app=member_snsfriend&wwi=addfollow&callback=?&mid='+data_str.mid, function(data){
				if(data){
		...
		function onfollow(o){
			var data_str = o.parent().attr('data-param');
			eval( "data_str = "+data_str);
			$.getJSON(MEMBER_SITE_URL+'/index.php?app=member_snsfriend&wwi=delfollow&callback=?&mid='+data_str.mid, function(data){
				if(data){
		...
		function tyq_function(d){
			var rs = $('<dd class="ajax-info"></dd>');
			$.each(d,function(i, n){
				rs.append('<div class="rank-div" title="'+n.tyq_name+'圈等级'+n.cm_level+'，经验值'+n.cm_exp+'"><a href="'+TYQ_SITE_URL+'/index.php?app=group&c_id='+n.tyq_id+'" target="_blank">'+n.tyq_name+'</a><i></i><em class="rank-em rank-'+n.cm_level+'">'+n.cm_level+'</em></div>');
			});
			return rs;
		}
		function tfx_function(d){
			var rs = $('<dd class="ajax-info"></dd>');
            rs.append('<span class="ajax-info-tfx">随心看：' + d.goods_count + '</span>');
            rs.append('<span class="ajax-info-tfx">个人秀：' + d.personal_count + '</span>');
			return rs;
		}
	};
})(jQuery);
	...
	$.fn.nc_region = function(options) {
		...
		this.each(function() {
			...
			if ($inputArea.val() && /^\d+$/.test($inputArea.val())) {
				$.getJSON(SITEURL + "/index.php?app=index&wwi=json_area_show&area_id=" + $inputArea.val() + "&callback=?", function(data) {
		...
		function loadAreaArray(callback) {
			if (typeof nc_a === "undefined") {
				$.getJSON(SITEURL + "/index.php?app=index&wwi=json_area&src=" + settings.src + "&callback=?", function(data) {
					nc_a = data;
					callback()
				})
			} else {
...
function addcart(goods_id,quantity,callbackfunc) {
    ...
    var url = 'index.php?app=cart&wwi=add';
    ...
    $.getJSON(url, {'goods_id':goods_id, 'quantity':quantity}, function(data) {
        if (data != null) {
            if (data.state) {
                ...
                $("#rtoolbar_cartlist").load(MALL_SITE_URL + '/index.php?app=cart&wwi=ajax_load&type=html');
            } else {
                alert(data.msg);
            }
        }
