...
class tyq_expModel extends Model {
    ...
    private function relOperation($param){
        $exp = 0;
        $today = date('Y-m-d');
        $dexp = $param['type'] == 'release' ? intval(C('tyq_exprelease')) : intval(C('tyq_expreply'));
        if(intval(C('tyq_expreleasemax')) != 0){
            $exp_info = $this->table('tyq_expmember')->where(array('member_id'=>$param['member_id']))->find();
            if(!empty($exp_info) && $exp_info['em_time'] == $today){        // there are data
                $exp = ( $dexp > (intval(C('tyq_expreleasemax'))-intval($exp_info['em_exp'])) ) ? (intval(C('tyq_expreleasemax'))-intval($exp_info['em_exp'])) : $dexp;
            }else{                                                          // There is no data
                $exp = ( $dexp > intval(C('tyq_expreleasemax')) ) ? intval(C('tyq_expreleasemax')) : $dexp ;
            }
            $insert = array();
            $insert['member_id']    = $param['member_id'];
            $insert['tyq_id']    = $param['tyq_id'];
            $insert['em_exp']       = empty($exp_info) || $today != $exp_info['em_time'] ? $exp : intval($exp_info['em_exp'])+$exp ;
            $insert['em_time']      = $today;
            $this->table('tyq_expmember')->insert($insert, true);
        }else{
            $exp = $dexp;
        }
        if( $exp != 0 ){
    ...
    private function repdOperation($param){
        ...
        if(intval(C('tyq_exprepliedmax')) != 0){
            $exp_info = $this->table('tyq_exptheme')->where(array('theme_id'=>$param['theme_id']))->find();
            if(!empty($exp_info) && $exp_info['et_time'] == $today){            // there are data
                $exp = ( intval(C('tyq_expreplied')) > (intval(C('tyq_exprepliedmax'))-intval($exp_info['et_exp'])) ) ? (intval(C('tyq_exprepliedmax'))-intval($exp_info['et_exp'])) : intval(C('tyq_expreplied'));
            }else{                                                              // There is no data
                $exp = ( intval(C('tyq_expreplied')) > (intval(C('tyq_exprepliedmax'))) ) ? intval(C('tyq_exprepliedmax')) : intval(C('tyq_expreplied'));
            }
            $insert = array();
            ...
            $this->table('tyq_exptheme')->insert($insert, true);
        }else{
            $exp = intval(C('tyq_expelease'));
        }
        if($exp != 0){
            $param['exp'] = $exp;
            return $this->expOperation(1, $param);
    ...
    private function expOperation($type, $param){
        ...
        $cm_info = $this->table('tyq_member')->where($where)->find();
        ...
        if($type){
            ...
            if( intval($cm_info['cm_nextexp']) != 0 && (intval($cm_info['cm_exp'])+$param['exp']) >= intval($cm_info['cm_nextexp']) && intval($cm_info['cm_level']) != 16 ){
                ...
                $ml_info = $this->table('tyq_ml')->where(array('tyq_id'=>$param['tyq_id']))->find();
        ...
        $rs = $this->table('tyq_member')->where($where)->update($update);
        if($rs){
    ...
    private function recordsLog($type, $param){
        ...
        switch ($param['type']){
            case 'release':
                ...
                $this->table('tyq_theme')->where(array('theme_id'=>$param['itemid']))->update($update);

                $insert['el_type']  = 2;
                $insert['el_desc']  = L('tyq_exp_release_theme');
                break;
            case 'reply':
                $update = array();
                ...
                $this->table('tyq_threply')->where($where)->update($update);

                $insert['el_type']  = 3;
                $insert['el_desc']  = L('tyq_exp_reply');
                break;
            case 'replied':
                $update = array();
                $update['theme_exp']= array('exp', 'theme_exp+'.$param['exp']);
                $this->table('tyq_theme')->where(array('theme_id'=>$param['theme_id']))->update($update);

                $insert['el_type']  = 4;
                $insert['el_desc']  = L('tyq_exp_theme_replied');
                break;
            case 'delRelease':
                $insert['el_type']  = 5;
                $insert['el_desc']  = L('tyq_exp_theme_delete');
                break;
            case 'delReplied':
                $insert['el_type']  = 6;
                $insert['el_desc']  = L('tyq_exp_reply_delete');
                break;
            case 'master':
                $insert['el_type']  = 1;
        ...
        $this->table('tyq_explog')->insert($insert);
    }
}
