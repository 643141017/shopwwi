...
class tzx_specialModel extends Model{

    const SPECIAL_TYPE_TZX = 1;
    const SPECIAL_TYPE_MALL = 2;

    private $special_type_array = array(
        self::SPECIAL_TYPE_TZX => '资讯',
        self::SPECIAL_TYPE_MALL => '商城',
    );

    public function __construct(){
        parent::__construct('tzx_special');
    }
    ...
    public function getList($condition, $page=null, $order='', $field='*', $limit=''){
        ...
        foreach ($list as $key => $value) {
            ...
            if($value['special_type'] == self::SPECIAL_TYPE_MALL) {
                ...
                $list[$key]['special_link'] = getTZXSpecialUrl($value['special_id']);
            }
        }
        return $list;
    }

    public function getTZXList($condition, $page=null, $order='', $field='*', $limit=''){
        $condition['special_type'] = self::SPECIAL_TYPE_TZX;
	...
	**/
     ...
     *   根据id获取举报详细信息
     */
    public function getonlyOne($special_id) {   $param = array() ;  $param['table'] = 'tzx_special'; $param['field'] = 'special_id' ; $param['value'] = intval($special_id);return Db::getRow($param) ;}
