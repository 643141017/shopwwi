	...
	$(function(){
		if(user['seller_id'] != '') {
            ...
            web_info['html_store_msg'] = '<div id="store_msg_dialog" class="dialog_wrapper" style="z-index: 3100; float: right; margin: 0; position: fixed; width: 280px; right: 50px; bottom: 0;display: none;">'+
									...
									'<div class="dialog_content" style="margin: 0px; padding: 0px;"><div class="wwist-form-default"><dl><dd style="width:100%; padding:10px;"><p></p></dd></dl>'+
			...
			var chat_user_list = '<div class="chat-box"><div class="chat-list"><div class="chat-list-top"><h1><i></i>联系人</h1><span class="minimize-chat-list" onclick="chat_show_list();"></span></div>'+
									...
									'</div></div>';
			var ajaxurl = CHAT_SITE_URL+'/index.php?app=web_chat&wwi=get_seller_list&n=99&f_id='+user['u_id'];
			$.ajax({
				type: "GET",
				url: ajaxurl,
				dataType:"jsonp",
	...
	function store_msg(msg){
	    ...
	    if( user['seller_is_admin'] == 1 || msg_limits[code] == 1) {
	        ...
	        var text_append = '<a href="'+SW_SITE_URL+'/index.php?app=store_msg&wwi=index">'+sm_content+'</a>';
	        store_msg_obj.find(".wwist-form-default dl p").html(text_append);
	        store_msg_obj.show();
