...
class loginControl extends BaseLoginControl {
    ...
    public function indexWwi(){
        ...
        if ($_GET['inajax'] == 1 && C('captcha_status_login') == '1'){
            $script = "document.getElementById('codeimage').src='index.php?app=seccode&wwi=makecode&nchash=".getNchash()."&t=' + Math.random();";
        }
        ...
        if ($result !== false){
            ...
            if(empty($_GET['ref_url'])) {
                $ref_url = getReferer();
                if (!preg_match('/app=login&wwi=logout/', $ref_url)) {
     ...
     * @return
     */
    public function check_emailWwi() {
		if(C('ucenter_status')) {
			/**
		 	* 实例化Ucenter模型
		 	*/
     ...
     * 忘记密码页面
     */
    public function forget_passwordWwi(){
        /**
         * 读取语言包
         */
        Language::read('home_login_register');
    ...
    public function bind_emailWwi() {
       ...
       showMessage('邮箱设置成功','index.php?app=member_security&wwi=index');
