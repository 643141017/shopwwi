...
class member_bindControl extends BaseMemberControl {
     ...
     * 微信解绑
     */
    public function weixinunbindWwi(){
        //修改密码
        $model_member = Model('member');
        $update_arr = array();
        if ($_POST['is_editpw'] == 'yes'){
    ...
    private function profile_menu($menu_key='',$array=array()) {
        ...
        $menu_array     = array();
        $menu_array = array(
            1=>array('menu_key'=>'qq_bind', 'menu_name'=>'QQ绑定',   'menu_url'=>'index.php?app=member_bind&wwi=qqbind'),
            2=>array('menu_key'=>'sina_bind',   'menu_name'=>'新浪绑定', 'menu_url'=>'index.php?app=member_bind&wwi=sinabind'),
            3=>array('menu_key'=>'weixin_bind',   'menu_name'=>'微信绑定', 'menu_url'=>'index.php?app=member_bind&wwi=weixinbind'),
        );
        if(!empty($array)) {
            $menu_array[] = $array;
        }
