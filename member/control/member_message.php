...
class member_messageControl extends BaseMemberControl {
     ...
     * @return
     */
    public function personalmsgWwi() {
        $model_message  = Model('message');
        $page   = new Page();
        $page->setEachNum(10);
        $page->setStyle('admin');
    ...
    public function savemsgWwi() {
        ...
        if (chksubmit()) {
            ...
            if($_GET['type'] == 'sns_board'){
                ...
                showDialog(Language::get('home_message_send_success'),'index.php?app=member_message&wwi=privatemsg','succ');
            }
        }
    }
    /**
     ...
     * @return
     */
    public function showmsgcommonWwi() {
        $model_message  = Model('message');
     ...
     * @return
     */
    public function showmsgbatchWwi() {
        $model_message  = Model('message');
        $message_id =  intval($_GET['message_id']);
        $drop_type = trim($_GET['drop_type']);
        $referer_url = getReferer();
    ...
    public function savereplyWwi() {
        ...
        if (!$isallowsend){
            if($_GET['inajax'] == 1){
                ...
                showMessage(Language::get('home_message_noallowsend'),'index.php?app=member_message&wwi=message','html','error');
            }
        }
        if ($_POST['form_submit'] == 'ok') {
            $message_id = intval($_POST["message_id"]);
            if ($message_id <=0){
                showMessage(Language::get('wrong_argument'),'index.php?app=member_message&wwi=message','html','error');
            }
            $obj_validate = new Validate();
            $obj_validate->validateparam = array(
            ...
            if($_GET['inajax'] == 1){
                ...
                showMessage(Language::get('home_message_send_success'),'index.php?app=member_message&wwi=privatemsg');
            }
        }else {
            if($_GET['inajax'] == 1){
     ...
     * 删除普通信
     */
    public function dropcommonmsgWwi() {
     ...
     * 删除批量站内信
     */
    public function dropbatchmsgWwi() {
        $message_id = trim($_GET['message_id']);
        $drop_type = trim($_GET['drop_type']);
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            array('menu_key'=>'message', 'menu_name'=>Language::get('home_message_received_message'),'menu_url'=>'index.php?app=member_message&wwi=message'),
            array('menu_key'=>'private', 'menu_name'=>Language::get('home_message_private_message'), 'menu_url'=>'index.php?app=member_message&wwi=privatemsg'),
            array('menu_key'=>'system',  'menu_name'=>Language::get('home_message_system_message'),  'menu_url'=>'index.php?app=member_message&wwi=systemmsg'),
            array('menu_key'=>'ggmsg',  'menu_name'=>'系统公告',  'menu_url'=>'index.php?app=member_message&wwi=ggmsg'),
            array('menu_key'=>'close',   'menu_name'=>Language::get('home_message_close'),   'menu_url'=>'index.php?app=member_message&wwi=personalmsg'),
            array('menu_key'=>'setting', 'menu_name'=>'接收设置', 'menu_url'=>urlMember('member_message', 'setting'))
        );
        if($menu_key == 'sendmsg') {
            $menu_array[] = array('menu_key'=>'sendmsg','menu_name'=>Language::get('home_message_send_message'),'menu_url'=>'index.php?app=member_message&wwi=sendmsg');
