...
class member_securityControl extends BaseMemberControl {
     ...
     * 绑定邮箱 - 发送邮件
     */
    public function send_bind_emailWwi() {
        ...
        $verify_url = MEMBER_SITE_URL.'/index.php?app=login&wwi=bind_email&uid='.$uid.'&hash='.md5($seed);
	    ...
	    showDialog('验证邮件已经发送至您的邮箱，请于24小时内登录邮箱并完成验证！','index.php?app=member_security&wwi=index','succ','',5);
    ...
    public function authWwi() {
        ...
        if (chksubmit(false,true)) {
            ...
            $member_info['member_mobile_bind'] == '0') {
                showMessage('请先绑定邮箱或手机','index.php?app=member_security&wwi=index','html','error');
            }
     ...
     * 统一发送身份验证码
     */
    public function send_auth_codeWwi() {
     ...
     * 修改密码
     */
    public function modify_pwdWwi() {
        $model_member = Model('member');

        //身份验证后，需要在30分钟内完成修改密码操作
        if (TIMESTAMP - $_SESSION['auth_modify_pwd'] > 1800) {
            showDialog('操作超时，请重新获得验证码','index.php?app=member_security&wwi=auth&type=modify_pwd','html','error');
        }
        ...
        showDialog($message,'index.php?app=member_security&wwi=index',$update ? 'succ' : 'error');
     ...
     * 设置支付密码
     */
    public function modify_paypwdWwi() {
        $model_member = Model('member');

        //身份验证后，需要在30分钟内完成修改密码操作
        if (TIMESTAMP - $_SESSION['auth_modify_paypwd'] > 1800) {
            showMessage('操作超时，请重新获得验证码','index.php?app=member_security&wwi=auth&type=modify_paypwd','html','error');
        }
     ...
     * 绑定手机
     */
    public function modify_mobileWwi() {
        $model_member = Model('member');
        ...
        if (chksubmit()) {
            ...
            showDialog('手机号绑定成功','index.php?app=member_security&wwi=index','succ');
        }
    }
     ...
     * 修改手机号 - 发送验证码
     */
    public function send_modify_mobileWwi() {
        $obj_validate = new Validate();
        $obj_validate->validateparam = array(
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'index':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>'账户安全','menu_url'=>'index.php?app=member_security&wwi=index'),
                );
                break;
            case 'modify_pwd':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>'账户安全','menu_url'=>'index.php?app=member_security&wwi=index'),
                array('menu_key'=>'modify_pwd','menu_name'=>'修改登录密码','menu_url'=>'index.php?app=member_security&wwi=auth&type=modify_pwd'),
                );
                break;
            case 'modify_email':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>'账户安全','menu_url'=>'index.php?app=member_security&wwi=index'),
                array('menu_key'=>'modify_email', 'menu_name'=>'邮箱验证','menu_url'=>'index.php?app=member_security&wwi=auth&type=modify_email'),
                );
                break;
            case 'modify_mobile':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>'账户安全','menu_url'=>'index.php?app=member_security&wwi=index'),
                array('menu_key'=>'modify_mobile','menu_name'=>'手机验证','menu_url'=>'index.php?app=member_security&wwi=auth&type=modify_mobile'),
                );
                break;
            case 'modify_paypwd':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>'账户安全','menu_url'=>'index.php?app=member_security&wwi=index'),
                array('menu_key'=>'modify_paypwd','menu_name'=>'设置支付密码','menu_url'=>'index.php?app=member_security&wwi=auth&type=modify_paypwd'),
                );
                break;
            case 'pd_cash':
                $menu_array = array(
                array('menu_key'=>'loglist','menu_name'=>'账户余额',    'menu_url'=>'index.php?app=predeposit&wwi=pd_log_list'),
                array('menu_key'=>'recharge_list','menu_name'=>'充值明细', 'menu_url'=>'index.php?app=predeposit&wwi=index'),
                array('menu_key'=>'cashlist','menu_name'=>'余额提现', 'menu_url'=>'index.php?app=predeposit&wwi=pd_cash_list'),
                array('menu_key'=>'pd_cash','menu_name'=>'提现申请','menu_url'=>'index.php?app=member_security&wwi=auth&type=pd_cash'),
                );
                break;
        }
