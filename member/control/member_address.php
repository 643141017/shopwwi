...
class member_addressControl extends BaseMemberControl{
    ...
    public function addressWwi() {
        ...
        if (!empty($_GET['type'])){
            ...
            if (intval($_GET['id']) > 0){
                ...
                if ($address_info['member_id'] != $_SESSION['member_id']){
                    showMessage($lang['member_address_wrong_argument'],'index.php?app=member_address&wwi=address','html','error');
                }
                /**
                 * 输出地址信息
                 */
        ...
        if ($del_id > 0){
            ...
            if ($rs){
                showDialog(Language::get('member_address_del_succ'),'index.php?app=member_address&wwi=address','js');
            }else {
     ...
     * 添加自提点型收货地址
     */
    public function delivery_addWwi() {
        if (chksubmit()) {
     ...
     * 展示自提点列表
     */
    public function delivery_listWwi() {
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'address':
                $menu_array = array(
                1=>array('menu_key'=>'address','menu_name'=>'地址列表',   'menu_url'=>'index.php?app=member_adderss&wwi=address'));
                break;
        }
