	...
	$.addFlex = function(t,p)
	{
		...
		var g = {
			...
			addData: function (data) { //parse data
				...
				if (!data)
					{
                    $(t).append('<table><tbody><tr><td colspan="100" class="no-data"><i class="fa fa-exclamation-tyq"></i>' + p.nomsg + '</td></tr></tbody></table>');
					return false;
					}
				...
				if (p.dataType=='json')
				{
				    if (typeof(data.rows) == 'undefined') {
	                    $(tbody).append('<tr><td colspan="100" class="no-data"><i class="fa fa-exclamation-tyq"></i>' + p.nomsg + '</td></tr>');
				    } else {
    					$.each
    					(
    					 data.rows,
			...
			populate: function () { //get latest data
				...
				$('.pPageStat',this.pDiv).html(p.protzxg);
