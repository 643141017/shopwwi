...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?app=brand&wwi=brand" title="返回品牌列表"><i class="fa fa-arrow-tyq-o-left"></i></a>
      <div class="subject">
...
// 点击查看图片
  ...
  function ajaxFileUpload()
  {
    ...
    (
      {
        url : '<?php echo ADMIN_SITE_URL?>/index.php?app=common&wwi=pic_upload&form_submit=ok&uploadpath=<?php echo ATTACH_BRAND;?>',
        secureuri:false,
        fileElementId:'_pic',
        dataType: 'json',
        ...
        {
          if (data.status == 1){
            ajax_form('cutpic','<?php echo $lang['nc_cut'];?>','<?php echo ADMIN_SITE_URL?>/index.php?app=common&wwi=pic_cut&type=brand&x=150&y=50&resize=1&ratio=3&url='+data.url,690);
          }else{
            alert(data.msg);
          }
      ...
      var error_td = element.parent('dd').children('span.err');
        ...
        rules : {
            brand_name : {
                required : true,
                remote   : {
                    url :'index.php?app=brand&wwi=ajax&branch=check_brand_name',
                    type:'get',
                    data:{
                        brand_name : function(){
        ...
        messages : {
            brand_name : {
                required : '<i class="fa fa-exclamation-tyq"></i><?php echo $lang['brand_add_name_null'];?>',
                remote   : '<i class="fa fa-exclamation-tyq"></i><?php echo $lang['brand_add_name_exists'];?>'
            },
            brand_initial : {
                initial : '<i class="fa fa-exclamation-tyq"></i>请填写正确首字母'
            },
            brand_sort  : {
                number   : '<i class="fa fa-exclamation-tyq"></i><?php echo $lang['brand_add_sort_int'];?>'
            }
        }
  });
});
