...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="javascript:history.back(-1)" title="返回列表"><i class="fa fa-arrow-tyq-o-left"></i></a>
      <div class="subject">
  ...
  <form id="post_form" method="post" action="index.php?app=vr_refund&wwi=edit&refund_id=<?php echo $output['refund']['refund_id']; ?>">
      ...
      <dl class="row">
        ...
        <dd class="opt"><?php echo ncPriceFormat($output['order']['pay_amount']);?></dd>
          ...
          <dl class="row">
            <dt class="tit"></dt>
            <dd class="opt">
                <a href="<?php echo ADMIN_SITE_URL;?>/index.php?app=vr_refund&wwi=alipay&refund_id=<?php echo $output['refund']['refund_id']; ?>" target="_blank">支付宝退款</a>  
...
$(function(){
	...
	$("#wxpayBtn").click(function(){
	    var ajaxurl = '<?php echo ADMIN_SITE_URL;?>/index.php?app=vr_refund&wwi=wxpay&refund_id=<?php echo $output['refund']['refund_id']; ?>';
		show_msg(ajaxurl);
	});
	$("#alipayBtn").click(function(){
	    var ajaxurl = '<?php echo ADMIN_SITE_URL;?>/index.php?app=vr_refund&wwi=get_detail&refund_id=<?php echo $output['refund']['refund_id']; ?>';
		show_msg(ajaxurl);
	});
    $('#post_form').validate({
        ...
        messages : {
            admin_state : {
                required : '<i class="fa fa-exclamation-tyq"></i>请选择是否同意退款'
            },
            admin_message  : {
                required : '<i class="fa fa-exclamation-tyq"></i><?php echo $lang['refund_message_null'];?>'
            }
        }
    });
});
