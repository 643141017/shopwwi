...
<div class="page">
  ...
  <form method="post" name="form1" id="form1" action="index.php?app=<?php echo $_GET['app'];?>&wwi=change_state&state_type=receive_pay&order_id=<?php echo intval($_GET['order_id']);?>">
...
$(function(){
	...
	$("#form1").validate({
        ...
        messages : {
        	payment_time : {
                required : '<i class="fa fa-exclamation-tyq"></i>请填写付款准确时间'
            },
            payment_code : {
                required : '<i class="fa fa-exclamation-tyq"></i>请选择付款方式'
            },
            trade_no : {
                required : '<i class="fa fa-exclamation-tyq"></i>请填写第三方支付平台交易号'
            }
        }
	});
});
