...
<script>
$(function(){
    var flexUrl = 'index.php?app=redpacket&wwi=rptlist_xml';
...
function fg_operate(name, bDiv) {
	if (name == 'add') {
		window.location.href = 'index.php?app=redpacket&wwi=rptadd';
    }
}
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除吗？')){
        $.getJSON('index.php?app=redpacket&wwi=rptdel', {tid:id}, function(data){
            if (data.state) {
