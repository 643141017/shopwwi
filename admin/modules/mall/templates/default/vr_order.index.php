...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?app=vr_order&wwi=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?app=vr_order&wwi=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: 'index.php?app=vr_order&wwi=get_xml',
        colModel : [
...
function fg_csv(ids) {
    id = ids.join(',');
    window.location.href = $("#flexigrid").flexSimpleSearchQueryString()+'&wwi=export_step1&order_id=' + id;
}
function fg_cancel(id) {
	...
	$.ajax({
        type: "GET",
        dataType: "json",
        url: "index.php?app=vr_order&wwi=change_state&state_type=cancel",
        data: "order_id="+id,
