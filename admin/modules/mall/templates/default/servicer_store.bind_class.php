...
<div class="ncap-form-default" >
  ...
  <dl class="row">
    ...
    <dd class="opt">
      <form id="add_form" action="<?php echo urlAdminMall('servicer_store', 'store_bind_class_add');?>" method="post">
...
$(document).ready(function(){
    ...
    $('[nctype="btn_del_store_bind_class"]').on('click', function() {
        if(confirm('确认删除？删除后店铺对应分类商品将全部下架')) {
            ...
            $this = $(this);
            $.post('<?php echo urlAdminMall('servicer_store', 'store_bind_class_del');?>', {bid: bid}, function(data) {
                 if(data.result) {
    ...
    // 修改分佣比例
    $('span[nc_type="commis_rate"]').inline_edit({app: 'servicer_store',wwi: 'store_bind_class_update'});
});
