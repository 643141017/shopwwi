...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?app=goods_recommend&wwi=index" title="返回商品分类推荐列表"><i class="fa fa-arrow-tyq-o-left"></i></a>
      <div class="subject">
  ...
  <form id="goods_form" method="post" action='index.php?app=goods_recommend&wwi=save'>
...
$(function(){
	$("#submitBtn").click(function(){
		if ($('input[name="goods_id_list[]"]').size() == 0) {
			$('#valid_recommend').rules('add',{
				 required: true,
				 messages: {required : '<i class="fa fa-exclamation-tyq"></i>请选择推荐商品'}
			});
		}else{
    ...
    $('#goods_search').on('click',function(){
    	...
    	if($("#goods_form").valid()){
    		...
    		<?php } ?>
    	    $('#show_recommend_goods_list').load('index.php?app=goods_recommend&wwi=get_goods_list&gc_id='+gc_id+'&goods_name='+$('#goods_name').val());
    	}
    });
	...
	$('#goods_form').validate({
        ...
        messages : {
        	gc_id : {
                required : '<i class="fa fa-exclamation-tyq"></i>请将分类选择完整'
            }
        }
    });
