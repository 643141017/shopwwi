...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?app=store&wwi=get_xml',
        colModel : [
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?app=store&wwi=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });

    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?app=store&wwi=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
});
...
function fg_operations(name, bDiv) {
    if (name == 'shopwwi_add') {
        window.location.href = 'index.php?app=store&wwi=shopwwi_add';
    }
}
