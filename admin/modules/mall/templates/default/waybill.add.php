...
<div class="page"> 
  <!-- 页面导航 -->
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?app=waybill&wwi=waybill_list" title="返回运单模板列表"><i class="fa fa-arrow-tyq-o-left"></i></a>
      <div class="subject">
...
$(document).ready(function(){
    ...
    $('#add_form').validate({
        ...
        messages : {
            waybill_name: {
                required : "<i class='fa fa-exclamation-tyq'></i>模板名称不能为空",
                maxlength : "<i class='fa fa-exclamation-tyq'></i>模板名称最多10个字" 
            },
            waybill_width: {
                required : "<i class='fa fa-exclamation-tyq'></i>宽度不能为空",
                digits: "<i class='fa fa-exclamation-tyq'></i>宽度必须为数字"
            },
            waybill_height: {
                required : "<i class='fa fa-exclamation-tyq'></i>高度不能为空",
                digits: "<i class='fa fa-exclamation-tyq'></i>高度必须为数字"
            },
            waybill_top: {
                required : "<i class='fa fa-exclamation-tyq'></i>上偏移量不能为空",
                number: "<i class='fa fa-exclamation-tyq'></i>上偏移量必须为数字"
            },
            waybill_left: {
                required : "<i class='fa fa-exclamation-tyq'></i>左偏移量不能为空",
                number: "<i class='fa fa-exclamation-tyq'></i>左偏移量必须为数字"
            },
            waybill_image: {
                <?php if(!$output['waybill_info']) { ?>
                required : '<i class="fa fa-exclamation-tyq"></i>图片不能为空',
                <?php } ?>
                accept: '<i class="fa fa-exclamation-tyq"></i>图片类型不正确' 
            }
        }
    });
});
