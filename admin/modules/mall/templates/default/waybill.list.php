...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?app=waybill&wwi=get_xml',
        colModel : [
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?app=waybill&wwi=waybill_add';
    }
}
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?app=waybill&wwi=waybill_del', {id:id}, function(data){
            if (data.state) {
