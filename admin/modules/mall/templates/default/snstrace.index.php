...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?app=snstrace&wwi=get_xml',
        colModel : [
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?app=snstrace&wwi=commentlist';
    }
}
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?app=snstrace&wwi=tracedel', {id:id}, function(data){
            if (data.state) {
