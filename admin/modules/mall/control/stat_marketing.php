...
class stat_marketingControl extends SystemControl{
    private $links = array(
        array('url'=>'app=stat_marketing&wwi=promotion','lang'=>'stat_promotion'),
        array('url'=>'app=stat_marketing&wwi=group','lang'=>'stat_group'),
    );
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (!isset($this->search_arr['wwi']) || in_array($this->search_arr['wwi'],array('promotion','group','get_groupgoods_xml','groupgoods','get_grouplist_xml'))){
     ...
     * 促销销售趋势分析
     */
    public function promotiontrendWwi(){
        //优惠类型数组
        $goodstype_arr = array(2=>'团购',3=>'限时折扣',4=>'优惠套装');
     ...
     * 输出团购统计XML数据
     */
    public function get_grouplist_xmlWwi(){
     ...
     * 输出团购商品统计XML数据
     */
    public function get_groupgoods_xmlWwi(){
        $model = Model('stat');
        $where = array();
        //获得搜索的开始时间和结束时间
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
