...
class help_storeControl extends SystemControl{
    ...
    public function get_xmlWwi() {
        ...
        foreach ($help_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn red' href=\"javascript:void(0);\" onclick=\"fg_del('".$value['help_id']."')\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='index.php?app=help_store&wwi=edit_help&help_id=".$value['help_id']."' class='url'><i class='fa fa-pencil-square-o'></i>编辑</a>";
     ...
     * 输出XML数据
     */
    public function get_type_xmlWwi() {
        $model_help = Model('help');
        $condition = array();
        if ($_POST['query'] != '') {
            $condition[$_POST['qtype']] = array('like', '%' . $_POST['query'] . '%');
        ...
        foreach ($type_list as $value) {
            ...
            $operation .= "<a class='btn blue' href='index.php?app=help_store&wwi=edit_type&type_id=".$value['type_id']."' class='url'><i class='fa fa-pencil-square-o'></i>编辑</a>";
    ...
    public function add_helpWwi() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('nc_common_save_succ'),'index.php?app=help_store&wwi=help_store');
            } else {
    ...
    public function add_typeWwi() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('nc_common_save_succ'),'index.php?app=help_store&wwi=help_type');
            } else {
                showMessage(Language::get('nc_common_save_fail'));
            }
        }
