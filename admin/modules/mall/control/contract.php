...
class contractControl extends SystemControl{
     ...
     * 保障服务项目列表XML
     */
    public function citemlist_xmlWwi()
    {
        $model_contract = Model('contract');
     ...
     * 保障服务项目编辑
     */
    public function citemeditWwi(){
        $itemid_id = intval($_GET['itemid']);
        if ($itemid_id <= 0){
            $itemid_id = intval($_POST['itemid_id']);
        }
        if ($itemid_id <= 0){
            showDialog(L('param_error'),'index.php?app=contract&wwi=citemlist');
        }
        $model_contract = Model('contract');
        //查询模板信息
        $where = array();
        ...
        if(chksubmit()){
            ...
            if($rs){
                ...
                showDialog(L('nc_common_save_succ'),'index.php?app=contract&wwi=citemlist','succ');
            } else {
     ...
     * 店铺保障服务列表
     */
    public function contractlistWwi(){
        //查询保障项目
     ...
     * 店铺保障服务列表XML
     */
    public function contractlist_xmlWwi()
    {
        $where = array();
     ...
     * 店铺保障服务编辑
     */
    public function contracteditWwi(){
        $ct_id = intval($_GET['ct_id']);
        if ($ct_id <= 0){
            $ct_id = intval($_POST['ct_id']);
        }
        if ($ct_id <= 0){
            showDialog(L('param_error'),'index.php?app=contract&wwi=contractlist');
        }
        $model_contract = Model('contract');
        //查询店铺保障服务信息
        $where = array();
        ...
        if(chksubmit()){
            ...
            if ($c_info['ct_closestate_key'] == $_POST['c_state']) {
                showDialog(L('nc_common_save_succ'),'index.php?app=contract&wwi=contractlist','succ');
            }
            try {
     ...
     * 服务加入申请列表XML
     */
    public function applylist_xmlWwi()
    {
        $where = array();
        $sort = '';
        if ($_REQUEST['advanced']) {
     ...
     * 保障服务申请编辑
     */
    public function applyeditWwi(){
        $aid = intval($_GET['aid']);
        if ($aid <= 0){
            $aid = intval($_POST['aid']);
        }
        if ($aid <= 0){
            showDialog(L('param_error'),'index.php?app=contract&wwi=applylist');
        }
        $model_contract = Model('contract');
        //查询申请信息
        $where = array();
        ...
        if(chksubmit()){
            if ($_POST['apply_state'] == $apply_info['cta_auditstate_key']) {
                showDialog('不能重复更改申请状态','index.php?app=contract&wwi=applylist');
            }
            if (!in_array($_POST['apply_state'],array_keys($curr_applystatearr))) {
                showDialog('编辑失败','index.php?app=contract&wwi=applyedit&aid='.$apply_info['cta_id']);
            }
            try {
                $model_contract->beginTransaction();
                //编辑申请状态
                ...
                showDialog(L('nc_common_save_succ'),'index.php?app=contract&wwi=applylist','succ');
            }catch (Exception $e){
                $model_contract->rollback();
                showDialog($e->getMessage(), '', 'error');
            }
     ...
     * 服务退出申请列表XML
     */
    public function quitlist_xmlWwi()
    {
        $where = array();
        $sort = '';
        if ($_REQUEST['advanced']) {
     ...
     * 保障服务申请编辑
     */
    public function quiteditWwi(){
        $qid = intval($_GET['qid']);
        if ($qid <= 0){
            $qid = intval($_POST['qid']);
        }
        if ($qid <= 0){
            showDialog(L('param_error'),'index.php?app=contract&wwi=quitlist');
        }
        $model_contract = Model('contract');
        //查询申请信息
        $where = array();
        ...
        if(chksubmit()){
            if ($_POST['quit_state'] == $quit_info['ctq_auditstate_key']) {
                showDialog('不能重复编辑申请状态','index.php?app=contract&wwi=quitlist');
            }
            if (!in_array($_POST['quit_state'],array_keys($this->quit_auditstate_arr))) {
                showDialog('编辑失败','index.php?app=contract&wwi=quitedit&qid='.$quit_info['ctq_id']);
            }
            try {
                $model_contract->beginTransaction();
                //编辑申请状态
                ...
                showDialog(L('nc_common_save_succ'),'index.php?app=contract&wwi=quitlist','succ');
            }catch (Exception $e){
                $model_contract->rollback();
                showDialog($e->getMessage(), '', 'error');
            }
     ...
     * 店铺保障服务详情
     */
    public function contractinfoWwi(){
        $ct_id = intval($_GET['ct_id']);
        if ($ct_id <= 0){
            showDialog(L('param_error'),'index.php?app=contract&wwi=contractlist');
        }
     ...
     * 保障服务日志列表XML
     */
    public function contractlog_xmlWwi()
    {
     ...
     * 保证金列表XML
     */
    public function costlist_xmlWwi()
    {
        $item_id = intval($_GET['item_id']);
        $store_id = intval($_GET['store_id']);
        if ($item_id <= 0 || $store_id <= 0) {
     ...
     * 更新店铺商品保障服务状态
     */
    public function contractgoodsWwi(){
    ...
    private function show_menu($menu_key=''){
        $menu_array = array(
            1=>array('menu_key'=>'applylist','menu_name'=>'服务加入申请', 'menu_url'=>'index.php?app=contract&wwi=applylist'),
            2=>array('menu_key'=>'quitlist','menu_name'=>'服务退出申请', 'menu_url'=>'index.php?app=contract&wwi=quitlist'),
            3=>array('menu_key'=>'contractlist','menu_name'=>'店铺保障服务', 'menu_url'=>'index.php?app=contract&wwi=contractlist'),
            4=>array('menu_key'=>'citemlist','menu_name'=>'保障服务管理', 'menu_url'=>'index.php?app=contract&wwi=citemlist')
        );
