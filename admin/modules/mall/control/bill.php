...
class billControl extends SystemControl{
    ...
    private $links = array(
        array('url'=>'app=bill&wwi=index','lang'=>'nc_manage'),
    );
    ...
    public function get_bill_info_xmlWwi(){
        $ob_id = intval($_GET['ob_id']);
        if ($ob_id <= 0 ) {
            exit();
        }
        ...
        if ($_GET['query_type'] == 'refund') {
            ...
            foreach ($refund_list as $refund_info) {
                ...
                if ($refund_info['refund_type'] == 1) {
                    $list['operation'] = "<a class=\"btn green\" href=\"index.php?app=refund&wwi=view&refund_id={$refund_info['refund_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
                } else {
                    $list['operation'] = "<a class=\"btn green\" href=\"index.php?app=return&wwi=view&return_id={$refund_info['refund_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
                }
            ...
            foreach ($order_list as $order_info) {
                $list = array();
                $list['operation'] = "<a class=\"btn green\" href=\"index.php?app=order&wwi=show_order&order_id={$order_info['order_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
     ...
     *
     */
    public function export_billWwi(){
        $model_bill = Model('bill');
        $condition = array();
     ...
     *
     */
    public function export_orderWwi(){
        $ob_id = intval($_GET['ob_id']);
        if ($ob_id <= 0) {
            exit();
        }
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){
                ...
                Tpl::output('list',$array);
                Tpl::output('murl','index.php?app=bill&wwi=show_bill&ob_id='.$ob_id);
								//网 店 运 维mall wwi.com
		Tpl::setDirquna('mall');
                Tpl::showpage('export.excel');
                exit();
     ...
     *
     */
    public function export_bookWwi(){
        $ob_id = intval($_GET['ob_id']);
        if ($ob_id <= 0) {
            exit();
        }
     ...
     *
     */
    public function export_refund_orderWwi(){
        $ob_id = intval($_GET['ob_id']);
        if ($ob_id <= 0) {
            exit();
        }
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('list',$array);
                Tpl::output('murl','index.php?app=bill&wwi=show_bill&query_type=refund&ob_id='.$ob_id);
								//网 店 运 维mall wwi.com
		Tpl::setDirquna('mall');
                Tpl::showpage('export.excel');
                exit();
    ...
    public function get_statis_xmlWwi(){
        $condition = array();
        ...
        foreach ($bill_list as $bill_info) {
            $list = array();
            $list['operation'] = "<a class=\"btn green\" href=\"index.php?app=bill&wwi=show_statis&os_month={$bill_info['os_month']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
    ...
    public function get_bill_xmlWwi(){
        $model_bill = Model('bill');
        $condition = array();
        list($condition,$order) = $this->_get_bill_condition($condition);
        ...
        foreach ($bill_list as $bill_info) {
            ...
            if (in_array($bill_info['ob_state'],array(2,3))) {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?app=bill&wwi=show_bill&ob_id={$bill_info['ob_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            } else {
                $list['operation'] = "<a class=\"btn green\" href=\"index.php?app=bill&wwi=show_bill&ob_id={$bill_info['ob_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
