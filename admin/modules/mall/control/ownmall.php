...
Tpl::setDirquna('mall');/*网 店 运 维mall wwi.com*/
    ...
    public function bind_class_updateWwi()
    {
        $bid = intval($_GET['id']);
        if($bid <= 0) {
    ...
    public function get_xmlWwi() {
        ...
        foreach ($store_list as $value) {
            $param = array();
            $operation = "<a class='btn blue' href='index.php?app=ownmall&wwi=edit&id=".$value['store_id']."'><i class='fa fa-pencil-square-o'></i>编辑</a>";
            if ($value['bind_all_gc'] != 1 || empty($stores_with_goods[$value['store_id']])) {
                ...
                if ($value['bind_all_gc'] != 1) {
                    $operation .= "<li><a href='index.php?app=ownmall&wwi=bind_class&id=" . $value['store_id'] . "'>编辑经营类目</a></li>";
                }
