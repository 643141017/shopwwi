...
class promotion_fcodeControl extends SystemControl{
    private $links = array(
        array('url'=>'app=promotion_fcode&wwi=index','text'=>'商品列表'),
        array('url'=>'app=promotion_fcode&wwi=fcode_quota_list','text'=>'套餐列表'),
        array('url'=>'app=promotion_fcode&wwi=fcode_setting','text'=>'设置')
    );
     ...
     * 下载F码
     */
    public function download_f_code_excelWwi() {
        $goods_id = $_GET['gid'];
        if ($goods_id <= 0) {
     ...
     * 套餐列表
     */
    public function fcode_quota_listWwi() {
     ...
     * 设置
     */
    public function fcode_settingWwi() {
        // 实例化模型
        $model_setting = Model('setting');
