...
class store_gradeControl extends SystemControl{
                ...
                if (is_array($_POST['check_sg_id'])){
                    ...
                    foreach ($_POST['check_sg_id'] as $k => $v){
                        ...
                        if ($v == 1){
                            ...
                            showMessage($lang['default_store_grade_no_del'],'index.php?app=store_grade&wwi=store_grade');
                        }
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    showMessage($lang['nc_common_save_succ'],'index.php?app=store_grade&wwi=store_grade');
                }else {
    ...
    public function store_grade_editWwi(){
        ...
        if (chksubmit()){
            if (!$_POST['sg_id']){
                showMessage($lang['grade_parameter_error'],'index.php?app=store_grade&wwi=store_grade');
            }
            /**
             * 验证
             */
            ...
            if ($error != ''){
                ...
                if (!$this->checkGradeName(array('sg_name'=>trim($_POST['sg_name']),'sg_id'=>intval($_POST['sg_id'])))){
                    showMessage($lang['now_store_grade_name_is_there'],'index.php?app=store_grade&wwi=store_grade_edit&sg_id='.intval($_POST['sg_id']));
                }
                //验证级别是否存在
                if (!$this->checkGradeSort(array('sg_sort'=>trim($_POST['sg_sort']),'sg_id'=>intval($_POST['sg_id'])))){
                    showMessage($lang['add_gradesortexist'],'index.php?app=store_grade&wwi=store_grade_edit&sg_id='.intval($_POST['sg_id']));
                }
                $update_array = array();
    ...
    public function store_grade_delWwi(){
        ...
        if (intval($_GET['sg_id']) > 0){
            ...
            if (!$this->isable_delGrade(intval($_GET['sg_id']))){
                showMessage($lang['del_gradehavestore'],'index.php?app=store_grade&wwi=store_grade');
            }
            /**
             * 删除分类
             */
            ...
            showMessage($lang['nc_common_del_succ'],'index.php?app=store_grade&wwi=store_grade');
        }else {
            showMessage($lang['nc_common_del_fail'],'index.php?app=store_grade&wwi=store_grade');
        }
    }
     ...
     * 等级：设置可选模板
     */
    public function store_grade_templatesWwi(){
        /**
         * 读取语言包
         */
        $lang   = Language::getLangContent();
