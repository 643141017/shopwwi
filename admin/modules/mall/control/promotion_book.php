...
class promotion_bookControl extends SystemControl{
    private $links = array(
        array('url'=>'app=promotion_book&wwi=index','text'=>'定金预售'),
        array('url'=>'app=promotion_book&wwi=presell_goods_list', 'text'=>'全款预售'),
        array('url'=>'app=promotion_book&wwi=book_quota_list','text'=>'套餐列表'),
        array('url'=>'app=promotion_book&wwi=book_setting','text'=>'设置')
    );
    ...
    public function indexWwi() {
        ...
        $this->book_goods_listWwi();
    }

    /**
     * 活动商品列表
     */
    public function book_goods_listWwi() {
     ...
     * 活动商品管理XML
     */
    public function get_book_goods_xmlWwi() {
     ...
     * 删除订金预售商品活动
     */
    public function del_book_goodsWwi() {
        $id = intval($_GET['id']);
        if ($id > 0) {
     ...
     * 活动商品列表
     */
    public function presell_goods_listWwi() {
     ...
     * 活动商品管理XML
     */
    public function get_presell_goods_xmlWwi() {
        $model_book = Model('p_book');
        $condition = array();
        if ($_POST['query'] != '') {
            $condition[$_POST['qtype']] = array('like', '%' . $_POST['query'] . '%');
     ...
     * 删除订金预售商品活动
     */
    public function del_presell_goodsWwi() {
        $id = intval($_GET['id']);
        if ($id > 0) {
     ...
     * 套餐列表
     */
    public function book_quota_listWwi() {
     ...
     * 套餐列表XML
     */
    public function get_quota_xmlWwi() {
        $model_book = Model('p_book');
        $condition = array();
        if ($_POST['query'] != '') {
            $condition[$_POST['qtype']] = array('like', '%' . $_POST['query'] . '%');
     ...
     * 设置
     */
    public function book_settingWwi() {
        // 实例化模型
        $model_setting = Model('setting');
