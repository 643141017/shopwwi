...
class voucherControl extends SystemControl{
    ...
    public function __construct(){
        ...
        if (C('voucher_allow') != 1 || C('points_isuse')!=1){
            showMessage(Language::get('admin_voucher_unavailable'),'index.php?app=operation&wwi=point','html','succ',1,4000);
        }
     ...
     * 代金券面额列表XML
     */
    public function pricelist_xmlWwi()
    {
        $model = Model();
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($rs){
                    ...
                    showMessage(Language::get('nc_common_save_succ'),'index.php?app=voucher&wwi=pricelist');
                }else {
                    showMessage(Language::get('nc_common_save_fail'),'index.php?app=voucher&wwi=priceadd');
                }
            }
        }else {
     ...
     * 编辑代金券面额
     */
    public function priceeditWwi(){
        $id = intval($_GET['priceid']);
        if ($id <= 0){
            $id = intval($_POST['priceid']);
        }
        if ($id <= 0){
            showMessage(Language::get('param_error'),'index.php?app=voucher&wwi=pricelist');
        }
        $model = Model();
        if (chksubmit()){
            $obj_validate = new Validate();
            ...
            if ($error != ''){
                ...
                if ($rs){
                    ...
                    showMessage(Language::get('nc_common_save_fail'),'index.php?app=voucher&wwi=pricelist');
                }
            }
        }else {
    ...
    public function pricedropWwi(){
        ...
        if ($rs){
            ...
            showMessage(Language::get('nc_common_del_succ'),'index.php?app=voucher&wwi=pricelist');
        }else{
            showMessage(Language::get('nc_common_del_fail'),'index.php?app=voucher&wwi=pricelist');
        }
    }
     ...
     * 套餐管理XML
     */
    public function quotalist_xmlWwi()
    {
        $condition = array();
     ...
     * 代金券列表
     */
    public function templatelistWwi()
    {
     ...
     * 代金券列表XML
     */
    public function templatelist_xmlWwi()
    {
        $condition = array();
     ...
     * 代金券模版编辑
     */
    public function templateeditWwi(){
        $t_id = intval($_GET['tid']);
        if ($t_id <= 0){
            $t_id = intval($_POST['tid']);
        }
        if ($t_id <= 0){
            showMessage(Language::get('param_error'),'index.php?app=voucher&wwi=templatelist','','error');
        }
        $model = Model('voucher');
        //查询模板信息
        $param = array();
        ...
        if (empty($t_info)){
            showMessage(Language::get('param_error'),'index.php?app=voucher&wwi=templatelist','html','error');
        }
        if(chksubmit()){
            ...
            if($rs){
                ...
                showMessage(Language::get('nc_common_save_succ'),'index.php?app=voucher&wwi=templatelist','succ');
            }else{
                showMessage(Language::get('nc_common_save_fail'),'index.php?app=voucher&wwi=templatelist','error');
            }
        }else{            
    ...
    private function show_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'voucher':
                $menu_array = array(
                3=>array('menu_key'=>'templatelist','menu_name'=>Language::get('admin_voucher_template_manage'), 'menu_url'=>'index.php?app=voucher&wwi=templatelist'),
                2=>array('menu_key'=>'quotalist','menu_name'=>Language::get('admin_voucher_quota_manage'), 'menu_url'=>'index.php?app=voucher&wwi=quotalist'),
                5=>array('menu_key'=>'pricelist','menu_name'=>Language::get('admin_voucher_pricemanage'), 'menu_url'=>'index.php?app=voucher&wwi=pricelist'),
                4=>array('menu_key'=>'setting','menu_name'=>Language::get('admin_voucher_setting'), 'menu_url'=>'index.php?app=voucher&wwi=setting'),
                );
                break;
            case 'priceedit':
                $menu_array = array(
                1=>array('menu_key'=>'setting','menu_name'=>Language::get('admin_voucher_setting'), 'menu_url'=>'index.php?app=voucher&wwi=setting'),
                2=>array('menu_key'=>'pricelist','menu_name'=>Language::get('admin_voucher_pricemanage'), 'menu_url'=>'index.php?app=voucher&wwi=pricelist'),
                3=>array('menu_key'=>'priceedit','menu_name'=>Language::get('admin_voucher_priceedit'), 'menu_url'=>'')
                );
                break;
            case 'templateedit':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('admin_voucher_template_manage'), 'menu_url'=>'index.php?app=voucher&wwi=templatelist'),
