...
class stat_memberControl extends SystemControl{
    private $links = array(
        array('url'=>'app=stat_member&wwi=newmember','lang'=>'stat_newmember'),
        array('url'=>'app=stat_member&wwi=analyze','lang'=>'stat_memberanalyze'),
        array('url'=>'app=stat_member&wwi=scale','lang'=>'stat_scaleanalyze'),
        array('url'=>'app=stat_member&wwi=area','lang'=>'stat_areaanalyze'),
        array('url'=>'app=stat_member&wwi=buying','lang'=>'stat_buying'),
    );
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (!isset($this->search_arr['wwi']) || in_array($_REQUEST['wwi'],array('newmember','analyze','scale','area','buying','get_scale_xml'))){
    ...
    public function newmemberWwi(){
        ...
        if($this->search_arr['search_type'] == 'day'){
            ...
            Tpl::output('actionurl','index.php?app=stat_member&wwi=newmember&search_type=day&search_time='.date('Y-m-d',$this->search_arr['day']['search_time']));
        }
        ...
        if($this->search_arr['search_type'] == 'week'){
            ...
            Tpl::output('actionurl','index.php?app=stat_member&wwi=newmember&search_type=week&searchweek_year='.$this->search_arr['week']['current_year'].'&searchweek_month='.$this->search_arr['week']['current_month'].'&searchweek_week='.$this->search_arr['week']['current_week']);
        }
        ...
        if($this->search_arr['search_type'] == 'month'){
            ...
            Tpl::output('actionurl','index.php?app=stat_member&wwi=newmember&search_type=month&searchmonth_year='.$this->search_arr['month']['current_year'].'&searchmonth_month='.$this->search_arr['month']['current_month']);
        }
     ...
     * 会员分析异步详细列表
     */
    public function analyzeinfoWwi(){
        $model = Model('stat');
        $where = array();
     ...
     * 输出会员分析详细列表XML数据
     */
    public function get_analyzeinfo_xmlWwi(){
        $model = Model('stat');
        $where = array();
        $searchtime_arr_tmp = explode('|',$this->search_arr['t']);
        foreach ((array)$searchtime_arr_tmp as $k => $v){
    ...
    public function showmemberWwi(){
        ...
        $this->links[] = array('url'=>'app=stat_member&wwi=showmember','lang'=>'stat_memberlist');
     ...
     * 输出订单统计XML数据
     */
    public function get_member_xmlWwi(){
        $model = Model('stat');
        $where = array();
        if (!empty($_GET['t'])){
            $searchtime_arr_tmp = explode('|',$_GET['t']);
     ...
     * 输出会员规模XML数据
     */
    public function get_scale_xmlWwi(){
        if(!$this->search_arr['search_type']){
            $this->search_arr['search_type'] = 'day';
        }
        $model = Model('stat');
     ...
     * 输出区域分析详细XML数据
     */
    public function get_arealist_xmlWwi(){
        $model = Model('stat');
        $where = array();
        $where['order_isvalid'] = 1;//计入统计的有效订单
        $searchtime_arr_tmp = explode('|',$this->search_arr['t']);
