...
class stat_storeControl extends SystemControl{
    private $links = array(
        array('url'=>'app=stat_store&wwi=newstore','lang'=>'stat_newstore'),
        array('url'=>'app=stat_store&wwi=hotrank','lang'=>'stat_storehotrank'),
        array('url'=>'app=stat_store&wwi=storesales','lang'=>'stat_storesales'),
        array('url'=>'app=stat_store&wwi=degree','lang'=>'stat_storedegree'),
        array('url'=>'app=stat_store&wwi=storearea','lang'=>'stat_storearea'),
    );
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (in_array($_REQUEST['wwi'],array('hotrank','storesales','get_storesales_xml','storesales_list','storesales_trends'))){
     ...
     * 新增店铺
     */
    public function newstoreWwi(){
        //导出excel连接地址
        $actionurl = 'index.php?app=stat_store&wwi=newstore';
        $where = array();
        //所属店铺分类
     ...
     * 热卖排行列表
     */
    public function hotrank_listWwi(){
        $datanum = 30;
        $model = Model('stat');
        switch ($_GET['type']){
           case 'ordernum':
    ...
    public function showstoreWwi(){
        ...
        $this->links[] = array('url'=>'app=stat_store&wwi=showstore','lang'=>'stat_storelist');
     ...
     * 输出店铺列表XML数据
     */
    public function get_store_xmlWwi(){
        $model = Model('stat');
     ...
     * 店铺销售统计列表
     */
    public function storesales_listWwi(){
        $model = Model('stat');
        //获得搜索的开始时间和结束时间
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        $where = array();
     ...
     * 输出店铺销售统计XML数据
     */
    public function get_storesales_xmlWwi(){
        $model = Model('stat');
        //获得搜索的开始时间和结束时间
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        $where = array();
     ...
     * 销售走势
     */
    public function storesales_trendsWwi(){
    ...
    public function storeareaWwi(){
        ...
        Tpl::output('stat_json',$stat_json);
        Tpl::output('statlist',$statlist);
        $actionurl = 'index.php?app=stat_store&wwi=showstore&type=storearea';
        if (trim($_GET['search_time'])){
