...
class stat_goodsControl extends SystemControl{
    private $links = array(
        array('url'=>'app=stat_goods&wwi=pricerange','lang'=>'stat_goods_pricerange'),
        array('url'=>'app=stat_goods&wwi=hotgoods','lang'=>'stat_hotgoods'),
        array('url'=>'app=stat_goods&wwi=goods_sale','lang'=>'stat_goods_sale'),
    );
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (!isset($this->search_arr['wwi']) || in_array($this->search_arr['wwi'],array('pricerange','hotgoods','goods_sale','get_goods_xml'))){
     ...
     * 热卖商品列表
     */
    public function hotgoods_listWwi(){
        $model = Model('stat');
        switch ($_GET['type']){
           case 'goodsnum':
               $sort_text = '下单量';
     ...
     * 输出商品销售明细XML数据
     */
    public function get_goods_xmlWwi(){
        if(!$this->search_arr['search_type']){
            $this->search_arr['search_type'] = 'day';
        }
        $model = Model('stat');
