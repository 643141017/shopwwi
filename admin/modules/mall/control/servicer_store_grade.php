...
class servicer_store_gradeControl extends SystemControl{
    ...
    public function store_gradeWwi(){
        ...
        $model_grade = Model('servicer_store_grade');
        /**
         * 删除
         */
        if (chksubmit()){
            if (!empty($_POST['check_ssg_id'])){
                if (is_array($_POST['check_ssg_id'])){
                    ...
                    foreach ($_POST['check_ssg_id'] as $k => $v){
                        ...
                        //判断是否默认等级，默认等级不能删除
                        if ($v == 1){
                            showMessage($lang['default_servicer_store_grade_no_del'],'index.php?app=servicer_store_grade&wwi=store_grade');
                        }
                ...
                dkcache('servicer_store_grade');
                $this->log(L('nc_del,servicer_store_grade').'[ID:'.implode(',',$_POST['check_ssg_id']).']',1);
    ...
    public function store_grade_addWwi(){
        ...
        if (chksubmit()){
            ...
            $obj_validate->validateparam = array(
                array("input"=>$_POST["ssg_name"], "require"=>"true", "message"=>$lang['servicer_store_grade_name_no_null']),
            );
            ...
            if ($error != ''){
                ...
                if (!$this->checkGradeName(array('ssg_name'=>trim($_POST['ssg_name'])))){
                    showMessage($lang['now_servicer_store_grade_name_is_there']);
                }
                ...
                if ($result){
                    dkcache('servicer_store_grade');
                    $this->log(L('nc_add,servicer_store_grade').'['.$_POST['ssg_name'].']',1);
                    showMessage($lang['nc_common_save_succ'],'index.php?app=servicer_store_grade&wwi=store_grade');
                }else {
		...
		Tpl::setDirquna('mall');/*网 店 运 维mall wwi.com*/
        Tpl::showpage('servicer_store_grade.add');
    }
    ...
    public function store_grade_editWwi(){
        ...
        if (chksubmit()){
            if (!$_POST['ssg_id']){
                showMessage($lang['grade_parameter_error'],'index.php?app=servicer_store_grade&wwi=store_grade');
            }
            /**
             * 验证
             */
            ...
            if ($error != ''){
                ...
                if (!$this->checkGradeName(array('ssg_name'=>trim($_POST['ssg_name']),'ssg_id'=>intval($_POST['ssg_id'])))){
                    showMessage($lang['now_servicer_store_grade_name_is_there'],'index.php?app=servicer_store_grade&wwi=store_grade_edit&ssg_id='.intval($_POST['ssg_id']));
                }
                ...
                if ($result){
                    dkcache('servicer_store_grade');
                    $this->log(L('nc_edit,servicer_store_grade').'['.$_POST['ssg_name'].']',1);
		...
		Tpl::setDirquna('mall');/*网 店 运 维mall wwi.com*/
        Tpl::showpage('servicer_store_grade.edit');
    }
    ...
    public function store_grade_delWwi(){
        ...
        if (intval($_GET['ssg_id']) > 0){
            ...
            if (!$this->isable_delGrade(intval($_GET['ssg_id']))){
                showMessage($lang['del_gradehavestore'],'index.php?app=servicer_store_grade&wwi=store_grade');
            }
            /**
             * 删除分类
             */
            ...
            dkcache('servicer_store_grade');
            $this->log(L('nc_del,servicer_store_grade').'[ID:'.intval($_GET['ssg_id']).']',1);
            showMessage($lang['nc_common_del_succ'],'index.php?app=servicer_store_grade&wwi=store_grade');
        }else {
            showMessage($lang['nc_common_del_fail'],'index.php?app=servicer_store_grade&wwi=store_grade');
        }
    }
