...
class messageControl extends SystemControl{
    private $links = array(
        array('url'=>'app=message&wwi=seller_tpl', 'lang'=>'seller_tpl'),
        array('url'=>'app=message&wwi=member_tpl', 'lang'=>'member_tpl'),
    );
     ...
     * 商家消息模板编辑
     */
    public function seller_tpl_editWwi() {
        if (chksubmit()) {
        ...
        $this->links[] = array('url'=>'app=message&wwi=seller_tpl_edit','lang'=>'seller_tpl_edit');
    ...
    private function seller_tpl_update_showmessage($result) {
        if ($result) {
            showMessage(L('nc_common_op_succ'), urlAdminMall('message', 'seller_tpl'));
        } else {
     ...
     * 用户消息模板编辑
     */
    public function member_tpl_editWwi() {
        if (chksubmit()) {
            $code = trim($_POST['code']);
            $type = trim($_POST['type']);
            if (empty($code) || empty($type)) {
        ...
        $this->links[] = array('url'=>'app=message&wwi=member_tpl_edit','lang'=>'member_tpl_edit');
    ...
    private function member_tpl_update_showmessage($result) {
        if ($result) {
            showMessage(L('nc_common_op_succ'), urlAdminMall('message', 'member_tpl'));
        } else {
            showMessage(L('nc_common_op_fail'));
        }
    }
