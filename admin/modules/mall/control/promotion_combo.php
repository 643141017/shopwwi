...
class promotion_comboControl extends SystemControl{
    private $links = array(
        array('url'=>'app=promotion_combo&wwi=index','text'=>'商品列表'),
        array('url'=>'app=promotion_combo&wwi=combo_quota_list','text'=>'套餐列表'),
        array('url'=>'app=promotion_combo&wwi=combo_setting','text'=>'设置')
    );
    ...
    public function indexWwi() {
        ...
        $this->combo_goods_listWwi();
    }

    /**
     * 活动商品列表
     */
    public function combo_goods_listWwi() {
     ...
     * 活动商品管理XML
     */
    public function get_combo_goods_xmlWwi() {
        $condition = array();
        if ($_POST['query'] != '') {
     ...
     * 删除订金预售商品活动
     */
    public function del_combo_goodsWwi() {
        $id = intval($_GET['id']);
        if ($id > 0) {
     ...
     * 套餐列表
     */
    public function combo_quota_listWwi() {
     ...
     * 设置
     */
    public function combo_settingWwi() {
        // 实例化模型
        $model_setting = Model('setting');
