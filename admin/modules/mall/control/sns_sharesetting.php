...
class sns_sharesettingControl extends SystemControl{
    ...
    public function indexWwi() {
        $this->sharesettingWwi();
    }

    /**
     * 绑定接口列表
     */
    public function sharesettingWwi(){
    ...
    public function editWwi(){
        ...
        if(chksubmit()){
            ...
            if ($result){
                $this->log(L('nc_edit,nc_binding_manage'),null);
                showMessage(Language::get('nc_common_save_succ'),'index.php?app=sns_sharesetting&wwi=sharesetting');
            }else {
