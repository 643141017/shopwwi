...
class brandControl extends SystemControl{
    ...
    public function get_xmlWwi() {
        ...
        foreach ($brand_list as $value) {
            ...
            if ($_GET['type'] == 'apply') {
                ...
                $operation .= "<a class='btn blue' href='index.php?app=brand&wwi=brand_edit&brand_id=" . $value['brand_id'] . "'><i class='fa fa-pencil-square-o'></i>编辑</a>";
            }
            ...
            $param['brand_recommend'] = $value['brand_recommend'] ==  '1' ? '<span class="yes"><i class="fa fa-check-tyq"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    $url = array(
                        array(
                            'url'=>'index.php?app=brand&wwi=brand_add',
                            'msg'=>$lang['brand_add_again'],
                        ),
                        array(
                            'url'=>'index.php?app=brand&wwi=brand',
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    $url = array(
                        array(
                            'url'=>'index.php?app=brand&wwi=brand_edit&brand_id='.intval($_POST['brand_id']),
     ...
     * 品牌申请
     */
    public function brand_applyWwi(){
                        //网 店 运 维mall wwi.com
        Tpl::setDirquna('mall');
     ...
     * 审核 申请品牌操作
     */
    public function brand_apply_setWwi(){
        $brand_id = intval($_GET['id']);
        if ($brand_id <= 0) {
    ...
    public function export_step1Wwi(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('list',$array);
                Tpl::output('murl','index.php?app=brand&wwi=brand');
                                //网 店 运 维mall wwi.com
        Tpl::setDirquna('mall');
                Tpl::showpage('export.excel');
            }else{  //如果数量小，直接下载
