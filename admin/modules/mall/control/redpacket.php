...
class redpacketControl extends SystemControl{
        ...
        if (chksubmit()){
            ...
            if ($error){
                ...
                if($rs){
                    ...
                    showDialog(L('nc_common_save_succ'),'index.php?app=redpacket&wwi=rptlist','succ');
                }else{
     ...
     * 红包模板列表XML
     */
    public function rptlist_xmlWwi()
    {
        $where = array();
     ...
     * 红包模版编辑
     */
    public function rpteditWwi(){
        $t_id = intval($_GET['tid']);
        if ($t_id <= 0){
            $t_id = intval($_POST['tid']);
        }
        if ($t_id <= 0){
            showDialog(L('param_error'),'index.php?app=redpacket&wwi=rptlist');
        }
        $model_redpacket = Model('redpacket');
        //查询模板信息
        $where = array();
    ...
    public function export_step1Wwi(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){//显示下载链接
                ...
                Tpl::output('murl','index.php?app=redpacket&wwi=rptinfo&tid='.$t_id);
				Tpl::setDirquna('mall');/*网 店 运 维mall wwi.com*/
