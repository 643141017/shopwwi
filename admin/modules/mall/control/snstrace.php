...
class snstraceControl extends SystemControl{
    ...
    public function get_xmlWwi() {
        ...
        foreach ((array)$help_list as $value) {
            ...
            $operation .= "<li><a href='index.php?app=snstrace&wwi=commentlist&tid=". $value['trace_id'] ."'>查看评论</a></li>";
            ...
            $param['trace_state'] = $value['trace_state'] ==  '0' ? '<span class="yes"><i class="fa fa-check-tyq"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
     ...
     * 评论列表
     */
    public function commentlistWwi(){
        $id = intval($_GET['tid']);
        if ($id <= 0) {
            $title = '全部评论列表';
        } else {
     ...
     * 输出XML数据
     */
    public function get_comment_xmlWwi() {
        ...
        foreach ((array)$help_list as $value) {
            ...
            $param['comment_state'] = $value['comment_state'] ==  '1' ? '<span class="yes"><i class="fa fa-check-tyq"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
     ...
     * 编辑评论
     */
    public function commenteditWwi(){
        $cid = intval($_GET['id']);
        if($cid <= 0){
            showDialog(L('admin_snstrace_pleasechoose_edit'));
        }
