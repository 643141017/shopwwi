...
class vr_refundControl extends SystemControl{
    ...
    private $links = array(
            array('url'=>'app=vr_refund&wwi=refund_manage','text'=>'待处理'),
            array('url'=>'app=vr_refund&wwi=refund_all','text'=>'所有记录'),
    );
    ...
    public function indexWwi() {
        $this->refund_manageWwi();
    }

    /**
     * 待处理列表
     */
    public function refund_manageWwi() {
        $_GET['waiting'] = 1;
        $this->refund_allWwi();
    }
    ...
    public function get_all_xmlWwi() {
        ...
        foreach ($refund_list as $k => $refund_info) {
            ...
            if ($_GET['waiting'] == 1 && $refund_info['admin_state'] == 1) {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?app=vr_refund&wwi=edit&refund_id={$refund_info['refund_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            } else {
                $list['operation'] = "<a class=\"btn green\" href=\"index.php?app=vr_refund&wwi=view&refund_id={$refund_info['refund_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
    ...
    public function editWwi() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('nc_common_save_succ'),'index.php?app=vr_refund&wwi=refund_manage');
            } else {
                showMessage(Language::get('nc_common_save_fail'));
            }
        }
