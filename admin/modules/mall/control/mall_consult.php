...
class mall_consultControl extends SystemControl{
    ...
    public function get_xmlWwi(){
        ...
        foreach ($consult_list as $consult_info) {
            ...
            if ($consult_info['is_reply']) {
                $list['operation'] .= "<a class=\"btn green\" href=\"index.php?app=mall_consult&wwi=consult_reply&id={$consult_info['mc_id']}\"><i class=\"fa fa-list-alt\"></i>编辑</a>";
            } else {
                $list['operation'] .= "<a class=\"btn green\" href=\"index.php?app=mall_consult&wwi=consult_reply&id={$consult_info['mc_id']}\"><i class=\"fa fa-list-alt\"></i>回复</a>";
            }
     ...
     * 回复咨询
     */
    public function consult_replyWwi() {
        $model_mallconsult = Model('mall_consult');
        if (chksubmit()) {
     ...
     * 删除平台客服咨询
     */
    public function del_consultWwi(){
     ...
     * 批量删除平台客服咨询
     */
    public function del_consult_batchWwi(){
        $ids = $_POST['id'];
        if(empty($ids)){
            showMessage(Language::get('nc_common_del_fail'));
        }
    ...
    public function type_addWwi() {
        if (chksubmit()) {
            ...
            if ($result){
                $this->log('新增咨询类型',1);
                showMessage(L('nc_common_save_succ'), urlAdminMall('mall_consult', 'type_list'));
            }else {
                $this->log('新增咨询类型',0);
    ...
    public function type_editWwi() {
        ...
        if (chksubmit()) {
            ...
            if ($result){
                ...
                showMessage(L('nc_common_op_succ'), urlAdminMall('mall_consult', 'type_list'));
            }else {
