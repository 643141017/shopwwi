...
class returnControl extends SystemControl{
    ...
    private $links = array(
            array('url'=>'app=return&wwi=return_manage','text'=>'待处理'),
            array('url'=>'app=return&wwi=return_all','text'=>'所有记录')
    );
    public function __construct(){
        parent::__construct();
        $model_refund = Model('refund_return');
        $model_refund->getRefundStateArray();
        if ($_GET['wwi'] == 'index') $_GET['wwi'] = 'return_manage';
        Tpl::output('top_link',$this->sublink($this->links,$_GET['wwi']));
    }

    public function indexWwi() {
        $this->return_manageWwi();
    }

    /**
     * 待处理列表
     */
    public function return_manageWwi() {
    ...
    public function get_manage_xmlWwi() {
        ...
        foreach ($return_list as $k => $return_info) {
            ...
            $list['operation'] = "<a class=\"btn orange\" href=\"index.php?app=return&wwi=edit&return_id={$return_info['refund_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
    ...
    public function get_all_xmlWwi() {
        ...
        foreach ($return_list as $k => $return_info) {
            ...
            $list['operation'] .= "<a class=\"btn green\" href=\"index.php?app=return&wwi=view&return_id={$return_info['refund_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            $list['refund_sn'] = $return_info['refund_sn'];
            $list['refund_amount'] = ncPriceFormat($return_info['refund_amount']);
            if(!empty($return_info['pic_info'])) {
                $info = unserialize($return_info['pic_info']);
    ...
    public function editWwi() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('nc_common_save_succ'),'index.php?app=return&wwi=return_manage');
            } else {
                showMessage(Language::get('nc_common_save_fail'));
            }
        }
