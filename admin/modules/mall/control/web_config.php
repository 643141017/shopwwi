...
class web_configControl extends SystemControl{
    ...
    public function web_editWwi(){
        ...
        if (chksubmit()){
            ...
            showMessage(Language::get('nc_common_save_succ'),'index.php?app=web_config&wwi=web_config');
        }
    ...
    public function web_htmlWwi(){
        ...
        if(!empty($web_array) && is_array($web_array)) {
            ...
            showMessage(Language::get('nc_common_op_succ'),'index.php?app=web_config&wwi=web_config');
        } else {
     ...
     * 商品分类
     */
    public function category_listWwi() {
        $model_class = Model('goods_class');
     ...
     * 中部推荐图片
     */
    public function recommend_picWwi() {
        $code_id = intval($_POST['code_id']);
        $web_id = intval($_POST['web_id']);
        $model_web_config = Model('web_config');
        $code = $model_web_config->getCodeRow($code_id,$web_id);
