...
class consultingControl extends SystemControl{
    private $links = array(
            array('url'=>'app=consulting&wwi=consulting','text'=>'咨询管理'),
            array('url'=>'app=consulting&wwi=type_list','text'=>'类型设置'),
            array('url'=>'app=consulting&wwi=setting','text'=>'头部文字设置')
    );
    public function __construct(){
        parent::__construct();
        Language::read('consulting');
        if ($_GET['wwi'] == 'index') $_GET['wwi'] = 'consulting';
    ...
    public function get_consulting_xmlWwi(){
    ...
    public function type_addWwi() {
        if (chksubmit()) {
            ...
            if ($result){
                $this->log('新增咨询类型',1);
                showMessage(L('nc_common_save_succ'), urlAdminMall('consulting', 'type_list'));
            }else {
                $this->log('新增咨询类型',0);
                showMessage(L('nc_common_save_fail'));
            }
    ...
    public function type_editWwi() {
        ...
        if (chksubmit()) {
            ...
            if ($result){
                ...
                showMessage(L('nc_common_op_succ'), urlAdminMall('consulting', 'type_list'));
            }else {
