...
class sns_straceControl extends SystemControl{
    ...
    public function get_xmlWwi() {
        ...
        foreach ($strace_list as $value) {
            ...
            $operation .= "<li><a href='index.php?app=sns_strace&wwi=scomm_list&st_id=". $value['strace_id'] ."'>查看评论</a></li>";
            ...
            $param['strace_state'] = $value['strace_state'] ==  '1' ? '<span class="yes"><i class="fa fa-check-tyq"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
     ...
     * 动态详细信息
     */
    public function strace_infoWwi() {
        $model_stracelog = Model('store_sns_tracelog');
        $id = $_GET['id'];
     ...
     * 编辑动态
     */
    public function strace_editWwi(){
        // 验证参数
        $st_id = intval($_GET['id']);
        if($st_id <= 0){
     ...
     * 输出XML数据
     */
    public function get_scomm_xmlWwi() {
        ...
        foreach ($scomm_list as $value) {
            ...
            $param['scomm_state'] = $value['scomm_state'] ==  '1' ? '<span class="yes"><i class="fa fa-check-tyq"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
