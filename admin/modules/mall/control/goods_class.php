...
class goods_classControl extends SystemControl{
    private $links = array(
        array('url'=>'app=goods_class&wwi=goods_class','lang'=>'nc_manage'),
        array('url'=>'app=goods_class&wwi=goods_class_import','lang'=>'goods_class_index_import'),
        array('url'=>'app=goods_class&wwi=tag','lang'=>'goods_class_index_tag')
    );
     ...
     * 分类管理
     */
    public function goods_classWwi(){
     ...
     * 商品分类添加
     */
    public function goods_class_addWwi(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    $url = array(
                        array(
                            'url'=>'index.php?app=goods_class&wwi=goods_class_add&gc_parent_id='.$_POST['gc_parent_id'],
                            'msg'=>$lang['goods_class_add_again'],
                        ),
                        array(
                            'url'=>'index.php?app=goods_class&wwi=goods_class',
     ...
     * 编辑
     */
    public function goods_class_editWwi(){
        Tpl::output('show_type', $this->show_type);
        $lang   = Language::getLangContent();
        $model_class = Model('goods_class');
        ...
        if (chksubmit()){
            ...
            $url = array(
                array(
                    'url'=>'index.php?app=goods_class&wwi=goods_class_edit&gc_id='.intval($_POST['gc_id']),
        ...
        $this->links[] = array('url'=>'app=goods_class&wwi=goods_class_edit','lang'=>'nc_edit');
     ...
     * 分类导入
     */
    public function goods_class_importWwi(){
        $lang   = Language::getLangContent();
        $model_class = Model('goods_class');
        //导入
        if (chksubmit()){
            ...
            if (!empty($_FILES['csv']) && !empty($_FILES['csv']['name']) && $file_type == 'csv'){
                ...
                showMessage($lang['nc_common_op_succ'],'index.php?app=goods_class&wwi=goods_class');
            }else {
     ...
     * 分类导出
     */
    public function goods_class_exportWwi(){
            $model_class = Model('goods_class');
     ...
     * 删除分类
     */
    public function goods_class_delWwi(){
        if ($_GET['id'] != ''){
            //删除分类
    ...
    public function tag_resetWwi(){
        ...
        if(!$return){
            showMessage($lang['goods_class_reset_tag_fail'], 'index.php?app=goods_class&wwi=tag');
        }
        ...
        if(is_array($goods_class) and !empty($goods_class)) {
            ...
            if($return){
                ...
                showMessage($lang['nc_common_op_succ'], 'index.php?app=goods_class&wwi=tag');
            }else{
                $this->log(L('nc_reset').'tag',0);
                showMessage($lang['nc_common_op_fail'], 'index.php?app=goods_class&wwi=tag');
            }
        }else{
            $this->log(L('nc_reset').'tag',0);
            showMessage($lang['goods_class_reset_tag_fail_no_class'], 'index.php?app=goods_class&wwi=tag');
        }
    }
    ...
    public function tag_updateWwi(){
        ...
        if(is_array($tag_list) && !empty($tag_list)){
            ...
            $this->log(L('nc_update').'tag',0);
            showMessage($lang['goods_class_update_tag_fail_no_class'], 'index.php?app=goods_class&wwi=tag');
        }

    }

    public function tag_editWwi() {
        $model_class_tag = Model('goods_class_tag');
