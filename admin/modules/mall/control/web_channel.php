...
class web_channelControl extends SystemControl{
    private $links = array(
        array('url'=>'app=web_channel&wwi=web_channel','text'=>'频道列表'),
        array('url'=>'app=web_channel&wwi=floor_list','text'=>'模块列表')
    );
     ...
     * 频道列表
     */
    public function web_channelWwi() {
     ...
     * 输出频道列表XML数据
     */
    public function get_channel_xmlWwi() {
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                ...
                if ($v['channel_show'] == 1) {
                    $out_array['channel_show'] = '<span class="yes"><i class="fa fa-check-tyq"></i>是</span>';
                } else {
                ...
                $operation .= '<li><a href="index.php?app=web_channel&wwi=edit_channel&channel_id='.$v['channel_id'].'">基本设置</a></li>';
                $operation .= '<li><a href="index.php?app=web_channel&wwi=set_channel&channel_id='.$v['channel_id'].'">编辑模块</a></li>';
                $operation .= '</ul></span>';
     ...
     * 输出模块列表XML数据
     */
    public function get_floor_xmlWwi() {
        $model_channel = Model('web_channel');
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                ...
                if ($v['web_show'] == 1) {
                    $out_array['web_show'] = '<span class="yes"><i class="fa fa-check-tyq"></i>是</span>';
                } else {
                    $out_array['web_show'] = '<span class="no"><i class="fa fa-ban"></i>否</span>';
                }
                $operation = '';
                $operation .= '<a class="btn purple" href="index.php?app=web_channel&wwi=edit_floor&web_id='.$v['web_id'].'"><i class="fa fa-cog"></i>设置</a>';
                $operation .= '<a class="btn orange" href="index.php?app=web_channel&wwi=edit_'.$v['web_page'].'&web_id='.$v['web_id'].'"><i class="fa fa-steam"></i>模块设计</a>';
                $out_array['operation'] = $operation;
     ...
     * 中部模块数据
     */
    public function get_channel_flWwi() {
        $model_channel = Model('web_channel');
        $condition = array();
        if (!empty($_GET['name'])) {
     ...
     * 新增频道
     */
    public function add_channelWwi() {
        $model_channel = Model('web_channel');
        if (chksubmit()) {
            $channel_array = array();
            ...
            if ($state) {
                ...
                showMessage(Language::get('nc_common_save_succ'),'index.php?app=web_channel&wwi=web_channel');
            } else {
     ...
     *
     */
    public function del_channelWwi() {
    ...
    public function add_floorWwi() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('nc_common_save_succ'),'index.php?app=web_channel&wwi=floor_list');
            } else {
                showMessage(Language::get('nc_common_save_fail'));
            }
        }
     ...
     * 编辑频道
     */
    public function edit_channelWwi() {
        $model_channel = Model('web_channel');
        $condition = array();
     ...
     * 编辑频道模块
     */
    public function set_channelWwi() {
     ...
     * 编辑顶部模块
     */
    public function edit_channel_tpWwi() {
        $model_web_config = Model('web_config');
     ...
     * 编辑中部模块
     */
    public function edit_channel_flWwi() {
        $model_web_config = Model('web_config');
        $web_id = intval($_GET["web_id"]);
        $code_list = $model_web_config->getCodeList(array('web_id'=>"$web_id"));
        if(is_array($code_list) && !empty($code_list)) {
     ...
     * 商品分类
     */
    public function get_category_listWwi() {
        $model_class = Model('goods_class');
     ...
     * 检查分类是否已经绑定到频道
     */
    public function check_goods_classWwi() {
        $model_channel = Model('web_channel');
     ...
     * 保存设置
     */
    public function code_updateWwi() {
     ...
     * 更新html内容
     */
    public function html_updateWwi() {
        $model_web_config = Model('web_config');
        $web_id = intval($_GET["web_id"]);
     ...
     * 商品分类图片
     */
    public function upload_gc_picWwi() {
     ...
     * 保存焦点区切换大图
     */
    public function channel_slideWwi() {
        $code_id = intval($_POST['code_id']);
        $web_id = intval($_POST['web_id']);
        $model_web_config = Model('web_config');
        $code = $model_web_config->getCodeRow($code_id,$web_id);
     ...
     * 保存焦点区切换小图
     */
    public function channel_advWwi() {
        $code_id = intval($_POST['code_id']);
        $web_id = intval($_POST['web_id']);
        $model_web_config = Model('web_config');
        $code = $model_web_config->getCodeRow($code_id,$web_id);
