...
class deliveryControl extends SystemControl{
    ...
    public function index_xmlWwi()
    {
        ...
        foreach ($list as $val) {
            ...
            $i['operation'] = <<<EOB
<a class="btn green" href="index.php?app=delivery&wwi=order_list&dlyp_id={$val['dlyp_id']}"><i class="fa fa-list-alt"></i>查看订单</a>
<a class="btn blue" href="index.php?app=delivery&wwi=edit_delivery&d_id={$val['dlyp_id']}"><i class="fa fa-pencil-square-o"></i>编辑</a>
EOB;
     ...
     * 提说站设置保存
     */
    public function save_settingWwi() {
        if (!chksubmit()){
     ...
     * 编辑物流自提服务站信息
     */
    public function edit_deliveryWwi() {
    ...
    public function save_editWwi() {
        ...
        if ($result) {
            ...
            showMessage(L('nc_common_op_succ'), urlAdminMall('delivery', 'index'));
        } else {
     ...
     * 订单列表
     */
    public function order_list_xmlWwi()
    {
        $condition = array();
        if (strlen($q = trim($_REQUEST['query']))) {
            switch ($_REQUEST['qtype']) {
