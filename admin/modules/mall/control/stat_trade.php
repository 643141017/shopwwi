...
class stat_tradeControl extends SystemControl{
    private $links = array(
        array('url'=>'app=stat_trade&wwi=income','lang'=>'stat_sale_income'),
        array('url'=>'app=stat_trade&wwi=predeposit','lang'=>'stat_predeposit'),
        array('url'=>'app=stat_trade&wwi=sale','lang'=>'stat_sale')
    );
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (in_array($_REQUEST['wwi'],array('sale_trend','get_sale_xml','get_plat_sale'))){
     ...
     * 输出平台总数据
     */
    public function get_plat_incomeWwi(){
        $model = Model('stat');
        $where = array();
        if ($_GET['store_name']) {
            $where['ob_store_name'] = array('like',"%{$_GET['store_name']}%");
     ...
     * 输出销售收入统计XML数据
     */
    public function get_income_xmlWwi(){
        $model = Model('stat');
        $where = array();
        if ($_GET['store_name']) {
            $where['ob_store_name'] = array('like',"%{$_GET['store_name']}%");
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                $out_array = array();
                $out_array['operation'] = '';
                //$out_array['operation'] = '<a class="btn green" href="index.php?app=stat_trade&wwi=sale&search_type=month&searchmonth_month='.$_GET['search_month'].
     ...
     * 预存款统计
     */
    public function get_predeposit_highchartsWwi(){
        $where = array();
        if(trim($_GET['pd_type'])=='cash_pay'){
            $field = 'sum(lg_freeze_amount) as allnum';
        }else{
     ...
     * 获取平台总预存款统计
     */
    public function get_plat_predepositWwi(){
        $model = Model('stat');
        $where = array();
        if(trim($_GET['pd_type'])=='cash_pay'){
            $field = 'sum(lg_freeze_amount) as allnum';
     ...
     * 输出销售收入统计XML数据
     */
    public function get_predeposit_xmlWwi(){
        $model = Model('stat');
        $where = array();
        if(trim($_GET['pd_type'])=='cash_pay'){
            $field = 'sum(lg_freeze_amount) as allnum';
     ...
     * 输出平台订单总数据
     */
    public function get_plat_saleWwi(){
        $model = Model('stat');
     ...
     * 输出订单统计XML数据
     */
    public function get_sale_xmlWwi(){
        $model = Model('stat');
