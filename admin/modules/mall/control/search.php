...
class searchControl extends SystemControl{
    ...
    private $_links = array(
        array('url'=>'app=search&wwi=index','text'=>'默认搜索'),
        array('url'=>'app=search&wwi=hot','text'=>'热门搜索')
    );
    ...
    public function hot_addWwi() {
        ...
        if (chksubmit()) {
            if (count($search_list) >= 10) {
                showMessage('最多可设置10个热搜词','index.php?app=search&wwi=hot');
            }
            ...
            if ($result){
                showMessage('保存成功','index.php?app=search&wwi=hot');
            }else {
                showMessage('保存失败');
            }
        }
    ...
    public function hot_editWwi() {
        ...
        if (!chksubmit()) {
            ...
            $result = $model_setting->updateSetting(array('rec_search'=>serialize($search_list)));
            if ($result){
                showMessage('编辑成功','index.php?app=search&wwi=hot');
            }
            showMessage('编辑失败');
        }
