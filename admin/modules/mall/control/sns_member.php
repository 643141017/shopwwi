        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                if ($result){
                    $url = array(
                            array(
                                    'url'=>'index.php?app=sns_member&wwi=tag_add',
                                    'msg'=>Language::get('sns_member_add_once_more'),
                            ),
                            array(
                                    'url'=>'index.php?app=sns_member&wwi=member_tag',
...
Tpl::setDirquna('mall');/*网 店 运 维mall wwi.com*/
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                if ($result){
                    ...
                    showMessage(Language::get('nc_common_op_succ'),'index.php?app=sns_member&wwi=member_tag');
                }else {
                    showMessage(Language::get('nc_common_op_fail'));
                }
            }
     ...
     * 推荐标签
     */
    public function tag_recommendWwi() {
        // 验证
        $mtag_id = intval($_GET['id']);
        if($mtag_id <= 0){
    ...
    public function get_xmlWwi() {
        ...
        foreach ($tag_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn red' href='index.php?app=sns_member&wwi=tag_del&id=" . $value['mtag_id'] . "'><i class='fa fa-trash-o'></i>删除</a><span class='btn'><em><i class='fa fa-cog'></i>" . L('nc_set') . " <i class='arrow'></i></em><ul><li><a href='index.php?app=sns_member&wwi=tag_edit&id=" . $value['mtag_id'] . "'>编辑标签</a></li><li><a href='index.php?app=sns_member&wwi=tag_member&id=".$value['mtag_id']."'>标签会员</a></li><li><a href='javascript:void(0);' onclick='ajaxget(\"" . urlAdminMall('sns_member','tag_recommend',array('id'=> $value['mtag_id'], 'recommend' => ($value['mtag_recommend'] == 1 ? 0 : 1))) . "\")'>" . ($value['mtag_recommend'] == 1 ? '取消推荐' : '推荐标签') . "</a></li></ul></span>";
            ...
            $param['mtag_recommend'] = $value['mtag_recommend'] ==  '1' ? '<span class="yes"><i class="fa fa-check-tyq"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
     ...
     * 推荐标签会员
     */
    public function tag_mrecommendWwi() {
        // 验证
        $mtag_id = intval($_GET['id']);
    ...
    public function get_tm_xmlWwi() {
        ...
        foreach ($tag_list as $value) {
            ...
            $param['recommend'] = $value['recommend'] ==  '1' ? '<span class="yes"><i class="fa fa-check-tyq"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
            $data['list'][$value['mtag_id']] = $param;
        }
        echo Tpl::flexigridXML($data);exit();
    }
