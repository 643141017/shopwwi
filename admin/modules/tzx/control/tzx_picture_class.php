...
class tzx_picture_classControl extends SystemControl{
    ...
    public function indexWwi() {
        $this->tzx_picture_class_listWwi();
    }

    /**
     * tzx画报分类列表
     **/
    public function tzx_picture_class_listWwi() {
        $model = Model('tzx_picture_class');
        $list = $model->getList(TRUE);
        $this->show_menu('list');
        Tpl::output('list',$list);
        Tpl::setDirquna('tzx');
Tpl::showpage("tzx_picture_class.list");
    }

    /**
     * tzx画报分类添加
     **/
    public function tzx_picture_class_addWwi() {
        $this->show_menu('add');
        Tpl::setDirquna('tzx');
Tpl::showpage('tzx_picture_class.add');
    }

    /**
     * tzx画报分类保存
     **/
    public function tzx_picture_class_saveWwi() {
        ...
        $model_class = Model('tzx_picture_class');
        $result = $model_class->save($param);
        if($result) {
            $this->log(Language::get('tzx_log_picture_class_save').$result, 1);
            showMessage(Language::get('class_add_success'),'index.php?app=tzx_picture_class&wwi=tzx_picture_class_list');
        } else {
            $this->log(Language::get('tzx_log_picture_class_save').$result, 0);
            showMessage(Language::get('class_add_fail'),'index.php?app=tzx_picture_class&wwi=tzx_picture_class_list','','error');
        }
     ...
     * tzx画报分类删除
     **/
     public function tzx_picture_class_dropWwi() {
        ...
        if($result) {
            $this->log(Language::get('tzx_log_picture_class_drop').$result, 1);
            showMessage(Language::get('class_drop_success'),'');
        } else {
            $this->log(Language::get('tzx_log_picture_class_drop').$result, 0);
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'list'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_list'),'menu_url'=>'index.php?app=tzx_picture_class&wwi=tzx_picture_class_list'),
            'add'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_new'),'menu_url'=>'index.php?app=tzx_picture_class&wwi=tzx_picture_class_add'),
        );
