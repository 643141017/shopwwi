...
class tzx_specialControl extends SystemControl{

    const LINK_TZX_SPECIAL = 'index.php?app=tzx_special&wwi=tzx_special_list';
    //专题状态草稿箱
    ...
    public function indexWwi() {
        $this->tzx_special_listWwi();
    }

    /**
     * tzx专题列表
     **/
    public function tzx_special_listWwi() {
        ...
        Tpl::setDirquna('tzx');
Tpl::showpage("tzx_special.list");
    }

    /**
     * tzx专题添加
     **/
    public function tzx_special_addWwi() {
        $model_special = Model('tzx_special');
        $this->show_menu('special_add');
        Tpl::output('special_type_array', $model_special->getSpecialTypeArray());
        Tpl::setDirquna('tzx');
Tpl::showpage('tzx_special.add');
    }

    /**
     * tzx专题编辑
     */
    public function tzx_special_editWwi() {
...
Tpl::showpage('tzx_special.add');
    ...
    public function tzx_special_saveWwi() {
        ...
        if($result) {
            ...
            $this->log(Language::get('tzx_log_special_save').$result, 1);
            showMessage(Language::get('nc_common_save_succ'), self::LINK_TZX_SPECIAL);
        } else {
            $this->log(Language::get('tzx_log_special_save').$result, 0);
            showMessage(Language::get('nc_common_save_fail'), self::LINK_TZX_SPECIAL);
        }
    }
     ...
     * 专题详细页
     */
    public function tzx_special_detailWwi() {
        $this->get_tzx_special_detail($_GET['special_id']);
    }

    private function get_tzx_special_detail($special_id) {
        $model_special = Model('tzx_special');
        $special_detail = $model_special->getOne(array('special_id'=>$special_id));
        Tpl::output('special_detail', $special_detail);
        Tpl::setDirquna('tzx');
Tpl::showpage('tzx_special.detail', 'null_layout');
    }
    ...
    private function generate_html($special_id) {
        $html_path = BASE_UPLOAD_PATH.DS.ATTACH_TZX.DS.'special_html'.DS;
        if(!is_dir($html_path)){
            if (!@mkdir($html_path, 0755)){
                showMessage(Language::get('tzx_special_build_fail'),'','','error');
            }
        }

        ob_start();
        $this->get_tzx_special_detail($special_id);
     ...
     * tzx专题删除
     */
    public function tzx_special_dropWwi() {
        $condition = array();
        ...
        if(!empty($special_list)) {
            ...
            foreach ($special_list as $value) {
                //删除图片
                $this->tzx_special_image_drop($value['special_background']);
                $this->tzx_special_image_drop($value['special_image']);
                ...
                if(!empty($special_image_list)) {
                    foreach ($special_image_list as $value_image) {
                        $this->tzx_special_image_drop($value_image['image_name']);
                    }
                }
                //删除静态文件
        ...
        if($result) {
            $this->log(Language::get('tzx_log_special_drop').$_REQUEST['special_id'], 1);
            showMessage(Language::get('nc_common_del_succ'),'');
        } else {
            $this->log(Language::get('tzx_log_special_drop').$_REQUEST['special_id'], 0);
     ...
     * 上传图片
     */
    private function tzx_special_image_upload($image) {
        if(!empty($_FILES[$image]['name'])) {
            $upload = new UploadFile();
            $upload->set('default_dir',ATTACH_TZX.DS.'special');
     ...
     * 图片删除
     */
    private function tzx_special_image_drop($image) {
        $file = getTZXSpecialImagePath($image);
        if(is_file($file)) {
            unlink($file);
        }
    }
     ...
     * 专题图片上传
     */
    public function special_image_uploadWwi() {
        $data = array();
        $data['status'] = 'success';
        if(!empty($_FILES['special_image_upload']['name'])) {
            ...
            $data['file_url'] = getTZXSpecialImageUrl($upload->file_name);
        }
     ...
     * 专题图片删除
     */
    public function special_image_dropWwi() {
        $data = array();
        $data['status'] = 'success';
        $this->tzx_special_image_drop($_GET['image_name']);
        echo json_encode($data);
    }
    ...
    private function get_special_state_list() {
        $array = array();
        $array[self::SPECIAL_STATE_DRAFT] = Language::get('tzx_text_draft');
        $array[self::SPECIAL_STATE_PUBLISHED] = Language::get('tzx_text_published');
        return $array;
    }
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'special_list'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_manage'),'menu_url'=>'index.php?app=tzx_special&wwi=tzx_special_list'),
            'special_add'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_new'),'menu_url'=>'index.php?app=tzx_special&wwi=tzx_special_add'),
