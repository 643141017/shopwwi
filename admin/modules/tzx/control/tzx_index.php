...
class tzx_indexControl extends SystemControl{
...
Tpl::showpage('tzx_index');
     ...
     * 模块添加
     */
    public function add_page_moduleWwi() {
        $data = array();
        $data['result'] = TRUE;
     ...
     * 模块删除
     */
    public function drop_page_moduleWwi() {
        $data = array();
        $data['result'] = TRUE;
     ...
     * 启用页面模块
     */
    public function update_page_module_showWwi() {
     ...
     * 关闭页面模块
     */
    public function update_page_module_hideWwi() {
     ...
     * 更新模块标题
     */
    public function update_page_module_titleWwi() {
     ...
     * 更新模块主题
     */
    public function update_page_module_styleWwi() {
     ...
     * 更新模块显示样式
     */
    public function update_page_module_viewWwi() {
     ...
     * 更新页面模块排序
     */
    public function update_page_module_indexWwi() {
        $data = array();
        $data['result'] = TRUE;
    ...
    public function tzx_index_previewWwi() {
        $model_index_module = Model('tzx_index_module');
        ...
        Tpl::output('module_list', $module_list);
        Tpl::setDirquna('tzx');
Tpl::showpage('tzx_index.template', 'null_layout');
    }

    /**
     * 首页静态文件生成
     */
    public function tzx_index_buildWwi() {
        $data = array();
        $data['result'] = TRUE;
        $html_path = BASE_UPLOAD_PATH.DS.ATTACH_TZX.DS.'index_html'.DS;
        if(!is_dir($html_path)){
            if (!@mkdir($html_path, 0755)){
                $data = array();
                $data['result'] = FALSE;
                $data['message'] = Language::get('tzx_index_build_fail');
...
Tpl::showpage('tzx_index.template', 'null_layout');
        ...
        if($result) {
            $this->log(Language::get('tzx_log_index_build'), 1);
            $data['message'] = Language::get('tzx_index_build_success');
        } else {
            $this->log(Language::get('tzx_log_index_build'), 0);
            $data['message'] = Language::get('tzx_index_build_fail');
        }
        echo json_encode($data);die;
    }
     ...
     * 保存自定义模块
     */
    public function save_moduleWwi() {
        $data = array();
        $data['result'] = TRUE;
        ...
        $html_path = BASE_UPLOAD_PATH.DS.ATTACH_TZX.DS.'module_template'.DS;
        if(!is_dir($html_path)){
            if (!@mkdir($html_path, 0755)){
                $data = array();
                $data['result'] = FALSE;
        ...
        ob_start();
        Tpl::setDirquna('tzx');
Tpl::showpage('tzx_module.template', 'null_layout');
        $result = file_put_contents($html_path.'tzx_module.'.$new_module['module_name'].'.php', ob_get_clean());
     ...
     * 删除自定义模块
     */
    public function drop_moduleWwi() {
        $data = array();
        $data['result'] = TRUE;
        $module_id = intval($_POST['module_id']);
        if($module_id <= 0) {
        ...
        $model_tzx_module = Model('tzx_module');
        $module_detail = $model_tzx_module->getOne(array('module_id'=>$module_id));
        ...
        //删除模块数据库记录
        $model_tzx_module->drop(array('module_id'=>$module_id));
        $model_tzx_index_module = Model('tzx_index_module');
        $model_tzx_index_module->drop(array('module_name'=>$module_detail['module_name']));

        //删除模板文件
        $module_template = BASE_UPLOAD_PATH.DS.ATTACH_TZX.DS.'module_template'.DS.'tzx_module.'.$module_detail['module_name'].'.php';
     ...
     * tzx首页模块编辑
     */
    public function module_editWwi() {
        ...
        Tpl::setDirquna('tzx');
Tpl::showpage('tzx_index_module.edit');
    }

    /**
     * tzx首页模块保存
     */
    public function save_page_moduleWwi() {
        ...
        $model_tzx_index_module = Model('tzx_index_module');
        $result = $model_tzx_index_module->modify(array('module_content'=>base64_encode(serialize($module_content))), array('module_id'=>$_POST['module_id']));

        if($result) {
            $this->log(Language::get('tzx_log_index_edit').$_POST['module_id'], 1);
            showMessage(Language::get('nc_common_save_succ'), 'index.php?app=tzx_index&wwi=tzx_index');
        } else {
            $this->log(Language::get('tzx_log_index_edit').$_POST['module_id'], 0);
    ...
    public function image_uploadWwi() {
        ...
        if(!empty($_FILES['image_upload']['name'])) {
            ...
            $upload->set('default_dir',ATTACH_TZX.DS.'index');
            ...
            $data['file_url'] = getTZXIndexImageUrl($upload->file_name);
        }
    ...
    private function image_drop($image_name) {
        $file = getTZXIndexImagePath($image_name);
        if(is_file($file)) {
            unlink($file);
        }
    }
    ...
    private function get_module_template_path($module_detail) {
        if($module_detail['module_type'] === $module_detail['module_name']) {
            return 'tzx_index_widget.'.$module_detail['module_type'].'.php';
        } else {
            return BASE_UPLOAD_PATH.DS.ATTACH_TZX.DS.'module_template'.DS.'tzx_module.'.$module_detail['module_name'].'.php';
        }
    }
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'tzx_index'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_manage'),'menu_url'=>'index.php?app=tzx_index&wwi=tzx_index'),
        );
     ...
     * 获取文章列表
     */
    public function get_article_listWwi() {
        //获取文章列表
        $condition = array();
        ...
        Tpl::setDirquna('tzx');
Tpl::showpage('tzx_widget_article_list','null_layout');
    }

    /**
     * 获取店铺列表
     */
    public function get_store_listWwi() {
        //获取店铺列表
        $condition = array();
        ...
        Tpl::setDirquna('tzx');
Tpl::showpage('tzx_widget_store_list', 'null_layout');
    }

    /**
     * 获取会员列表
     */
    public function get_member_listWwi() {
        //获取店铺列表
        $condition = array();
        ...
        Tpl::setDirquna('tzx');
Tpl::showpage('tzx_widget_member_list', 'null_layout');
    }

    /**
     * 获取品牌列表
     */
    public function get_brand_listWwi() {
        $model_brand = Model('brand');
        $brand_list = $model_brand->getBrandPassedList(array(), '*', 6);
        Tpl::output('show_page',$model_brand->showpage());
        Tpl::output('brand_list',$brand_list);
        Tpl::setDirquna('tzx');
Tpl::showpage('tzx_widget_brand_list','null_layout');
    }

    /**
     * 商品分类列表
     */
    public function get_goods_class_list_jsonWwi() {
     ...
     * 商品分类详细列表
     */
    public function get_goods_class_detailWwi(){
        $model_class = Model('goods_class');
        ...
        Tpl::setDirquna('tzx');
Tpl::showpage('tzx_widget_goods_class_list','null_layout');
    }

    /**
     * 图片商品添加
     */
    public function goods_info_by_urlWwi() {
