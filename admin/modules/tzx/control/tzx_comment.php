...
class tzx_commentControl extends SystemControl{
     ...
     * 评论管理
     */
    public function comment_manageWwi()
    {
        $this->get_type_array();
        Tpl::setDirquna('tzx');
Tpl::showpage('tzx_comment.manage');
    }

    /**
     * 评论管理
     */
    public function comment_manage_xmlWwi()
    {
        $condition = array();
        ...
        $model_comment = Model("tzx_comment");
        ...
        foreach ($list as $val) {
            ...
            $o = '<a class="btn red confirm-del-on-click" href="index.php?app=tzx_comment&wwi=comment_drop&comment_id=' .
                $val['comment_id'] .
                '"><i class="fa fa-trash"></i>删除</a>';
            $o .= '<a class="btn green" target="_blank" href="' .
                TZX_SITE_URL.DS.'index.php?app=' .
                $channel['key'] .
                '&wwi=' .
                $channel['key'] .
                '_detail&' .
    ...
    private function get_type_array() {
        $type_array = array();
        $type_array[1] = array('name'=>Language::get('tzx_text_artcile'),'key'=>'article');
        $type_array[2] = array('name'=>Language::get('tzx_text_picture'),'key'=>'picture');
     ...
     * 评论删除
     */
    public function comment_dropWwi() {
        $model = Model('tzx_comment');
        $condition = array();
        $condition['comment_id'] = array('in',trim($_REQUEST['comment_id']));
        $result = $model->drop($condition);
        if($result) {
            $this->log(Language::get('tzx_log_comment_drop').$_REQUEST['comment_id'], 1);
            showMessage(Language::get('nc_common_del_succ'),'');
        } else {
            $this->log(Language::get('tzx_log_comment_drop').$_REQUEST['comment_id'], 0);
