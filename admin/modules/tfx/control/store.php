...
class storeControl extends SystemControl{
     ...
     * 店铺管理
     */
    public function store_manageWwi()
    {
        $this->show_menu_store('store_manage');
        Tpl::setDirquna('tfx');
Tpl::showpage('tfx_store.manage');
    }

    /**
     * 店铺管理
     */
    public function store_manage_xmlWwi()
    {
        $condition = array();
        ...
        foreach ($list as $val) {
            $o = '<a class="btn red confirm-del-on-click" href="index.php?app=store&wwi=store_drop_save&store_id=' .
                $val['store_id'] .
                '"><i class="fa fa-trash"></i>删除</a>';

            if ($val['tfx_commend'] == 1) {
                $o .= '<a class="btn green" href="javascript:;" data-ie-column="tfx_commend" data-ie-value="0"><i class="fa fa-thumbs-o-down"></i>取消推荐</a>';
            } else {
                $o .= '<a class="btn green" href="javascript:;" data-ie-column="tfx_commend" data-ie-value="1"><i class="fa fa-thumbs-o-up"></i>推荐店铺</a>';
            }

            $i = array();
            $i['operation'] = $o;

            $i['tfx_sort'] = '<span class="editable" title="可编辑" style="width:50px;" data-live-inline-edit="tfx_sort">' .
                $val['tfx_sort'] . '</span>';

            $i['store_name'] = '<a target="_blank" href="' .
                TFX_SITE_URL.DS.
                'index.php?app=store&wwi=detail&store_id=' .
                $val['store_id'] .
                '">' .
                $val['store_name'] .
                '</a>';
            ...
            $i['added_state'] = $val['tfx_commend'] == 1
     ...
     * 店铺街添加列表
     */
    public function store_addWwi()
    {
        $this->show_menu_store('store_add');
        Tpl::setDirquna('tfx');
Tpl::showpage('tfx_store.add');
    }

    /**
     * 店铺街添加列表
     */
    public function store_add_xmlWwi()
    {
        $model_store = Model('store');
        $model_tfx_store = Model('micro_store');

        $tfx_store_list = $model_tfx_store->getList(TRUE);
        $tfx_store_array = array();
        if (!empty($tfx_store_list)) {
            foreach ($tfx_store_list as $value) {
                $tfx_store_array[] = $value['mall_store_id'];
            }
        }
        ...
        foreach ($list as $val) {
            $addedState = in_array($val['store_id'], $tfx_store_array);

            if ($addedState) {
                $o = '--';
            } else {
                $o = '<a class="btn green" href="index.php?app=store&wwi=store_add_save&store_id=' .
                    $val['store_id'] .
     ...
     * 店铺街添加保存
     */
    public function store_add_saveWwi()
    {
        ...
        if(!empty($store_id_array)) {
            foreach ($store_id_array as $value) {
                if(intval($value) > 0) {
                    $tfx_store['mall_store_id'] = $value;
                    $tfx_store['tfx_sort'] = 255;
                    $tfx_store['tfx_commend'] = 0;
                    $param[] = $tfx_store;
                }
            }
        }
     ...
     * 店铺街删除保存
     */
    public function store_drop_saveWwi() {
        $model_store = Model('micro_store');
        $condition = array();
     ...
     * 更新微商城店铺排序
     */
    public function store_sort_updateWwi() {
        if(intval($_GET['id']) <= 0) {
        ...
        if ($new_sort > 255){
            echo json_encode(array('result'=>FALSE,'message'=>Language::get('tfx_sort_error')));
            die;
        } else {
            $model_class = Model('micro_store');
            $result = $model_class->modify(array('tfx_sort'=>$new_sort),array('mall_store_id'=>$_GET['id']));
            if($result) {
    ...
    private function show_menu_store($menu_key) {
        $menu_array = array(
                'store_manage'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_manage'),'menu_url'=>'index.php?app=store&wwi=store_manage'),
                'store_add'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_new'),'menu_url'=>'index.php?app=store&wwi=store_add'),
        );
