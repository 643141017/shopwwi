...
class goods_classControl extends SystemControl{
    ...
    public function indexWwi() {
       $this->goodsclass_listWwi();
    }

    /**
     * 微商城商品(随心看)分类管理
     **/
    public function goodsclass_listWwi() {
        $model_class = Model("micro_goods_class");
        $list = $model_class->getList(TRUE);
        Tpl::output('list',$list);
        $this->show_menu_goods_class("goods_class_list");
        Tpl::setDirquna('tfx');
Tpl::showpage("tfx_goods_class.list");
    }

    /**
     * 微商城商品(随心看)分类添加
     **/
    public function goodsclass_addWwi() {
        //取得一级分类列表
        $model_tfx_goods_class = Model('micro_goods_class');
        $condition = array();
        $condition['class_parent_id'] = 0;
        $goods_class_list = $model_tfx_goods_class->getList($condition);
        ...
        Tpl::setDirquna('tfx');
Tpl::showpage('tfx_goods_class.add');
    }
     ...
     * 微商城商品(随心看)分类编辑
     **/
    public function goodsclass_editWwi() {
...
Tpl::showpage("tfx_goods_class.add");
     ...
     * 微商城商品(随心看)分类保存
     **/
    public function goodsclass_saveWwi() {
        ...
        if(!empty($_FILES['class_image']['name'])) {
            ...
            if(!empty($_POST['old_class_image'])) {
                $old_image = BASE_UPLOAD_PATH.DS.ATTACH_TFX.DS.$_POST['old_class_image'];
        ...
        if($result) {
            showMessage(Language::get('class_add_success'),"index.php?app=goods_class&wwi=goodsclass_list");
        } else {
            showMessage(Language::get('class_add_fail'),"index.php?app=goods_class&wwi=goodsclass_list",'','error');
        }
     ...
     * ajax修改分类排序
     */
    public function goodsclass_sort_updateWwi() {
        if(intval($_GET['id']) <= 0) {
     ...
     * ajax修改分类名称
     */
    public function goodsclass_name_updateWwi() {
     ...
     * 随心看分类删除
     **/
     public function goodsclass_dropWwi() {

        $class_id = trim($_REQUEST['class_id']);
        $model_tfx_goods_class = Model('micro_goods_class');
        $condition = array();
        $condition['class_parent_id'] = array('in',$class_id);
        $goods_class_list = $model_tfx_goods_class->getList($condition,'','','class_id');
        ...
        //删除分类图片
        $list = $model_tfx_goods_class->getList($condition);
        if(!empty($list)) {
            foreach ($list as $value) {
                if(!empty($value['class_image'])) {
                    //删除老图片
                    $old_image = BASE_UPLOAD_PATH.DS.ATTACH_TFX.DS.$value['class_image'];
                    if(is_file($old_image)) {
                        unlink($old_image);
                    }
                }
        ...
        //删除绑定关系
        $model_tfx_goods_relation = Model('micro_goods_relation');
        $model_tfx_goods_relation->drop($condition);

        //删除分类
        $result = $model_tfx_goods_class->drop($condition);
        if($result) {
     ...
     * 分类关键字和商品分类的绑定
     **/
    public function goodsclass_bindingWwi() {
        ...
        Tpl::setDirquna('tfx');
Tpl::showpage('tfx_goods_class.binding');
     ...
     * 分类关键字和商品分类的绑定保存
     **/
    public function goodsclass_binding_saveWwi() {
        ...
        if($result) {
            showMessage(Language::get('goods_relation_save_success'),self::TFX_CLASS_LIST);
        } else {
            showMessage(Language::get('goods_relation_save_fail'),self::TFX_CLASS_LIST,'','error');
        }
    }
     ...
     * 设为默认分类
     **/
    public function goodsclass_defaultWwi() {
        $class_id = intval($_GET['class_id']);
        if($class_id <= 0) {
            showMessage(Language::get('param_error'),'','','error');
        }
    ...
    public function goodsclass_getWwi() {
    ...
    private function show_menu_goods_class($menu_key) {
        $menu_array = array(
                'goods_class_list'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_manage'),'menu_url'=>'index.php?app=goods_class&wwi=goodsclass_list'),
                'goods_class_add'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_new'),'menu_url'=>'index.php?app=goods_class&wwi=goodsclass_add'),
        );
        ...
        if($menu_key == 'goods_class_binding') {
            $menu_array['goods_class_binding'] = array('menu_type'=>'link','menu_name'=>Language::get('tfx_goods_class_binding'),'menu_url'=>'###');
        }
