...
class personal_classControl extends SystemControl{
    ...
    public function indexWwi() {
       $this->personalclass_listWwi();
    }

    /**
     * 微商城商品(随心看)分类管理
     **/
    public function personalclass_listWwi() {
        $model_class = Model("micro_personal_class");
        $list = $model_class->getList(TRUE);
        Tpl::output('list',$list);
        $this->show_menu_personal_class("personal_class_list");
        Tpl::setDirquna('tfx');
Tpl::showpage("tfx_personal_class.list");
    }

    /**
     * 微商城个人秀分类添加
     **/
    public function personalclass_addWwi() {
        $this->show_menu_personal_class('personal_class_add');
        Tpl::setDirquna('tfx');
Tpl::showpage('tfx_personal_class.add');
    }

    /**
     * 微商城商品(随心看)分类编辑
     **/
    public function personalclass_editWwi() {
...
Tpl::showpage("tfx_personal_class.add");
     ...
     * 微商城个人秀分类保存
     **/
    public function personalclass_saveWwi() {
        ...
        if($result) {
            showMessage(Language::get('class_add_success'),"index.php?app=personal_class&wwi=personalclass_list");
        } else {
            showMessage(Language::get('class_add_fail'),"index.php?app=personal_class&wwi=personalclass_list",'','error');
        }
     ...
     * ajax修改分类排序
     */
    public function personalclass_sort_updateWwi() {
        if(intval($_GET['id']) <= 0) {
     ...
     * ajax修改分类名称
     */
    public function personalclass_name_updateWwi() {
     ...
     * 个人秀分类删除
     **/
     public function personalclass_dropWwi() {
     ...
     private function show_menu_personal_class($menu_key) {
         $menu_array = array(
                 'personal_class_list'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_manage'),'menu_url'=>'index.php?app=personal_class&wwi=personalclass_list'),
                 'personal_class_add'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_new'),'menu_url'=>'index.php?app=personal_class&wwi=personalclass_add'),
         );
