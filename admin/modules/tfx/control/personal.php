...
class personalControl extends SystemControl{
    ...
    public function indexWwi() {
       $this->personal_manageWwi();
    }

    /**
     * 个人秀管理
     */
    public function personal_manageWwi()
    {
        Tpl::setDirquna('tfx');
Tpl::showpage('tfx_personal.manage');
    }

    /**
     * 个人秀管理XML
     */
    public function personal_manage_xmlWwi()
    {
        $condition = array();
        ...
        if ($_REQUEST['advanced']) {
            ...
            if (strlen($q = trim((string) $_REQUEST['tfx_commend']))) {
                $condition['tfx_commend'] = (int) $q;
            }
        ...
        $list = (array) $model_personal->getListWithUserInfo($condition, $_REQUEST['rp'],
            'tfx_sort, commend_time desc', $field);
        ...
        foreach ($list as $val) {
            $o = '<a class="btn red confirm-del-on-click" href="index.php?app=personal&wwi=personal_drop&personal_id=' .
                $val['personal_id'] .
            ...
            if ($val['tfx_commend'] == '1') {
                $o .= '<li><a href="javascript:;" data-ie-column="tfx_commend" data-ie-value="0">取消推荐</a></li>';
            } else {
                $o .= '<li><a href="javascript:;" data-ie-column="tfx_commend" data-ie-value="1">推荐内容</a></li>';
            }

            $u = TFX_SITE_URL.DS.'index.php?app=personal&wwi=detail&personal_id=' . $val['personal_id'];
            ...
            $personal_image_array_240 = getTfxPersonalImageUrl($val, 'list');
...
EOB;
            ...
            $i['member_name'] = '<a href="' .
                TFX_SITE_URL.DS.'index.php?app=home&member_id=' . $val['commend_member_id'] .
                '" target="_blank">' .
                $val['member_name'] .
                '</a>';
            ...
            $i['tfx_commend'] = $val['tfx_commend'] == '1'
     ...
     * 个人秀删除
     */
    public function personal_dropWwi() {
        ...
        if(!empty($list)) {
            foreach ($list as $personal_info) {
                ...
                foreach ($image_array as $value) {
                    //删除原始图片
                    $image_name = BASE_UPLOAD_PATH.DS.ATTACH_TFX.DS.$personal_info['commend_member_id'].DS.$value;
                    ...
                    $ext = explode('.', $value);
                    $ext = $ext[count($ext) - 1];
                    $image_name = BASE_UPLOAD_PATH.DS.ATTACH_TFX.DS.$personal_info['commend_member_id'].DS.$value.'_list.'.$ext;
                    if(is_file($image_name)) {
                        unlink($image_name);
                    }
                    $image_name = BASE_UPLOAD_PATH.DS.ATTACH_TFX.DS.$personal_info['commend_member_id'].DS.$value.'_tiny.'.$ext;
                    if(is_file($image_name)) {
                        unlink($image_name);
                    }
                }
     ...
     * 更新微商城个人秀排序
     */
    public function personal_sort_updateWwi() {
        if(intval($_GET['id']) <= 0) {
        ...
        if ($new_sort > 255){
            ...
            $result = $model_class->modify(array('tfx_sort'=>$new_sort),array('personal_id'=>$_GET['id']));
            if($result) {
