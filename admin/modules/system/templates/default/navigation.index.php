    ...
    <table class="flex-table">
      ...
      <tbody>
        ...
        <tr data-id="<?php echo $v['nav_id'];?>">
          ...
          <a class="btn blue" href="index.php?app=navigation&wwi=navigation_edit&nav_id=<?php echo $v['nav_id'];?>"><i class="fa fa-pencil-square-o"></i><?php echo $lang['nc_edit'];?></a>
          </td>
...
$(document).ready(function(){
    ...
    //行内ajax编辑
    $('span[nc_type="tag_sort"]').inline_edit({app: 'navigation',wwi: 'ajax'});
    $('span[nc_type="tag_name"]').inline_edit({app: 'navigation',wwi: 'ajax'});
});
...
function submit_delete(id){
	...
	if(confirm('删除后将不能恢复，确认删除这 ' + id.length + ' 项吗？')){
		id = id.join(',');
        window.location.href = 'index.php?app=navigation&wwi=navigation_del&nav_id='+id;
    }
}
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?app=navigation&wwi=navigation_add';
    }else if (name == 'delete') { 
