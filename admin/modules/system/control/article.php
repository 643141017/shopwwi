...
class articleControl extends SystemControl{
    ...
    public function get_xmlWwi(){
        ...
        if (is_array($article_list)){
            ...
            foreach ($article_list as $k => $v){
                $list = array();
                $list['operation'] = "<a class='btn red' onclick=\"fg_delete({$v['article_id']})\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='index.php?app=article&wwi=article_edit&article_id={$v['article_id']}'><i class='fa fa-pencil-square-o'></i>编辑</a>";
                ...
                $list['article_show'] = $v['article_show'] == 0 ? '<span class="no"><i class="fa fa-ban"></i>否</span>' : '<span class="yes"><i class="fa fa-check-tyq"></i>是</span>';
     ...
     * 文章添加
     */
    public function article_addWwi(){
        $lang   = Language::getLangContent();
        $model_article = Model('article');
        /**
         * 保存
...
//                 array("input"=>$_POST["article_content"], "require"=>"true", "message"=>$lang['article_add_content_null']),
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    $url = array(
                        array(
                            'url'=>'index.php?app=article&wwi=article',
                            'msg'=>"{$lang['article_add_tolist']}",
                        ),
                        array(
                            'url'=>'index.php?app=article&wwi=article_add&ac_id='.intval($_POST['ac_id']),
     ...
     * 文章编辑
     */
    public function article_editWwi(){
        $lang    = Language::getLangContent();
        $model_article = Model('article');
...
//                 array("input"=>$_POST["article_content"], "require"=>"true", "message"=>$lang['article_add_content_null']),
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    $url = array(
                        ...
                        array(
                            'url'=>'index.php?app=article&wwi=article_edit&article_id='.intval($_POST['article_id']),
     ...
     * 文章图片上传
     */
    public function article_pic_uploadWwi(){
        /**
         * 上传图片
         */
