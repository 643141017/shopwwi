 ...
 *
 *
 * 本系统由网店运维 XS版 www. mall wwi .com提供
 */
...
class messageControl extends SystemControl{
    private $links = array(
        array('url'=>'app=message&wwi=email','lang'=>'email_set'),
        array('url'=>'app=message&wwi=email_tpl','lang'=>'email_tpl')
    );
     ...
     * 编辑邮件模板
     */
    public function email_tpl_editWwi(){
        $model_templates = Model('mail_templates');
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result === true){
                    ...
                    showMessage(L('mailtemplates_edit_succ'),'index.php?app=message&wwi=email_tpl');
                }else {
   ...
   /**
	 ...
	 * @return
	 */
	public function email_testingWwi(){
		/**
		 * 读取语言包
		 */
