...
class article_classControl extends SystemControl{
    ...
    public function indexWwi() {
        $this->article_classWwi();
    }

    /**
     * 文章管理
     */
    public function article_classWwi(){
     ...
     * 文章分类 新增
     */
    public function article_class_addWwi(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    $url = array(
                        array(
                            'url'=>'index.php?app=article_class&wwi=article_class_add&ac_parent_id='.intval($_POST['ac_parent_id']),
                            'msg'=>$lang['article_class_add_class'],
                        ),
                        array(
                            'url'=>'index.php?app=article_class&wwi=article_class',
     ...
     * 文章分类编辑
     */
    public function article_class_editWwi(){
        $lang   = Language::getLangContent();
        $model_class = Model('article_class');
...
//              $update_array['ac_parent_id'] = intval($_POST['ac_parent_id']);
                ...
                if ($result){
                    $url = array(
                        ...
                        array(
                            'url'=>'index.php?app=article_class&wwi=article_class_edit&ac_id='.intval($_POST['ac_id']),
                            'msg'=>$lang['article_class_edit_again'],
                        ),
                    );
                    $this->log(l('nc_edit,article_class_index_class').'['.$_POST['ac_name'].']',1);
                    showMessage($lang['article_class_edit_succ'],'index.php?app=article_class&wwi=article_class');
                }else {
     ...
     * 删除分类
     */
    public function article_class_delWwi(){
        $lang   = Language::getLangContent();
        $model_class = Model('article_class');
        if (intval($_GET['ac_id']) > 0){
            ...
            showMessage($lang['article_class_index_del_succ'],'index.php?app=article_class&wwi=article_class');
        }else {
            showMessage($lang['article_class_index_choose'],'index.php?app=article_class&wwi=article_class');
        }
    }
    /**
     * ajax操作
