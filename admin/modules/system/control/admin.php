...
class adminControl extends SystemControl{
    private $links = array(
        array('url'=>'app=admin&wwi=admin','lang'=>'limit_admin'),
        array('url'=>'app=admin&wwi=gadmin','lang'=>'limit_gadmin'),
    );
    ...
    public function admin_addWwi(){
        if (chksubmit()){
            ...
            if ($rs){
                ...
                showMessage(L('nc_common_save_succ'),'index.php?app=admin&wwi=admin');
            }else {
                showMessage(L('nc_common_save_fail'));
            }
        }
    ...
    public function admin_editWwi(){
        if (chksubmit()){
            ...
            if ($result){
                ...
                showMessage(Language::get('admin_edit_success'),'index.php?app=admin&wwi=admin');
            }else{
                showMessage(Language::get('admin_edit_fail'),'index.php?app=admin&wwi=admin');
            }
        }else{
            //查询用户信息
            $admin_model = Model('admin');
            ...
            if (!is_array($admininfo) || count($admininfo)<=0){
                showMessage(Language::get('admin_edit_admin_error'),'index.php?app=admin&wwi=admin');
            }
    ...
    public function gadmin_addWwi(){
        if (chksubmit()){
            ...
            if ($model->insert($data)){
                ...
                showMessage(L('nc_common_save_succ'),'index.php?app=admin&wwi=gadmin');
            }else {
                showMessage(L('nc_common_save_fail'));
            }
        }
     ...
     * 设置权限组权限
     */
    public function gadmin_editWwi(){
        $model = Model('gadmin');
        $gid = intval($_GET['gid']);
