...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?app=yydb&wwi=yydblist" title="返回列表"><i class="fa fa-arrow-tyq-o-left"></i></a>
      <div class="subject">
        <h3>平台夺宝 - 编辑夺宝模板</h3>
        <h5>平台夺宝新增与管理</h5>
      </div>
  ...
  <form id="ydb_form" method="post" name="ydb_form" enctype="multipart/form-data">
    ...
    <div class="ncap-form-default">
      ...
      <dl class="row">
        ...
        <dd class="opt">
          <div class="onoff">
            <label title="<?php echo $lang['nc_yes'];?>" class="cb-enable <?php if($output['t_info']['yydb_t_recommend'] == '1'){ ?>selected<?php } ?>" for="recommend1"><?php echo $lang['nc_yes'];?></label>
            <label title="<?php echo $lang['nc_no'];?>" class="cb-disable <?php if($output['t_info']['yydb_t_recommend'] == '0'){ ?>selected<?php } ?>" for="recommend0"><?php echo $lang['nc_no'];?></label>
...
//
    ...
    $('#fileupload').each(function(){
        $(this).fileupload({
            dataType: 'json',
            url: 'index.php?app=yydb&wwi=yydb_pic_upload&item_id=<?php echo $output['t_info']['yydb_t_id'];?>',
...
function del_file_upload(file_id)
{
    ...
    $.getJSON('index.php?app=yydb&wwi=ajaxdelupload&file_id=' + file_id, function(result){
        if(result){
            $('#' + file_id).remove();
        }else{
            alert('删除失败');
