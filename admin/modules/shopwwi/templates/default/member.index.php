...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?app=member&wwi=get_xml',
        colModel : [
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?app=member&wwi=member_add';
    }
    if (name == 'csv') {
...
function submit_delete(id){
	...
	if(confirm('删除后将不能恢复，确认删除这 ' + id.length + ' 项吗？')){
		id = id.join(',');
        window.location.href = 'index.php?app=member&wwi=member_del&member_id='+id;
    }
}
