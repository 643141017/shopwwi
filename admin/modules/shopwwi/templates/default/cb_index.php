...
$(document).ready(function(){
    ...
    $('#template_module_list').on('click', '[nctype="btn_add_page_module"]', function() {
        ...
        $.post("index.php?app=tzx_index&wwi=add_page_module", { module_title: page_module.module_title, module_name: page_module.module_name, module_type: page_module.module_type }, function(data) {
            if(data.result) {
                ...
                $('.page-module').last().find('span[nc_type="module_title"]').inline_edit_confirm({app: 'tzx_index', wwi:'update_page_module_title'});
    ...
    $('#template_module_list').on('click', '[nctype="btn_drop_module"]', function() {
        ...
        if(confirm('确认删除?')) {
            ...
            $.post("index.php?app=tzx_index&wwi=drop_module", { module_id: module_id }, function(data) {
                if(data.result) {
    ...
    $('#page_view').on('click', "[nctype='btn_show_page_module']", function() {
        ...
        $.post("index.php?app=tzx_index&wwi=update_page_module_show", { module_id: module_id }, function(data) {
            if(data.result) {
    ...
    $('#page_view').on('click', "[nctype='btn_hide_page_module']", function() {
        var $btn = $(this);
        var module_id = $btn.attr('data-module-id');
        $.post("index.php?app=tzx_index&wwi=update_page_module_hide", { module_id: module_id }, function(data) {
            if(data.result) {
                var $page_module = $btn.parents('.page-module');
    ...
    $('#page_view').on('click', "[nctype='btn_drop_page_module']", function() {
        if(confirm('确认删除')) {
            ...
            $.post("index.php?app=tzx_index&wwi=drop_page_module", { module_id: module_id }, function(data) {
                if(data.result) {
    ...
    $("#page_view").sortable({
        update: function(event, ui) {
            ...
            $.post("index.php?app=tzx_index&wwi=update_page_module_index", {page_module_id_string: page_module_id_string}, function(data) {
                if(!data.result) {
                    showError(data.message);
                }
            }, 'json');
    ...
    $('span[nc_type="module_title"]').inline_edit_confirm({app: 'tzx_index', wwi:'update_page_module_title'});

    //生成首页
    $("#btn_index_build").click(function() {
        $.getJSON('index.php?app=tzx_index&wwi=tzx_index_build', function(data) {
            if(data.result) {
    ...
    $('#btn_add_module').click(function() {
        ...
        if($module_frame_list.html() === '') {
            $.getJSON('index.php?app=tzx_index&wwi=get_module_frame_list', function(data) {
                if(data.frame_list) {
                    var template_data = {};
    ...
    $('#module_frame').on('click', '#btn_toggle_module_display_title', function() {
        ...
        var $div_title = $(this).parent('.tzx-module-frame-title');
        if(current === 'enable') {
    ...
    $('#btn_save_module').click(function() {
        ...
        $.post("index.php?app=tzx_index&wwi=save_module", {module_title: module_title, frame_name: frame_name, frame_block: frame_block, module_display_title_state: module_display_title_state}, function(data) {
            if(data.result) {
    ...
    $('#page_view').on('click', "[nctype='btn_module_style_select']", function() {
        ...
        $.post("index.php?app=tzx_index&wwi=update_page_module_style", {module_id: module_id, module_style: module_style}, function(data) {
            if(data.result) {
                var $page_module = $btn.parents('.page-module');
    ...
    $('#page_view').on('click', "[nctype='btn_module_view_change']", function() {
        ...
        $.post("index.php?app=tzx_index&wwi=update_page_module_view", {module_id: module_id, module_view: module_view}, function(data) {
            if(!data.result) {
                showError(data.message);
            }
        }, 'json');
...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      <div class="subject">
        <h3><?php echo $lang['nc_tzx_index_manage'];?></h3>
        <h5><?php echo $lang['nc_tzx_index_manage_subhead'];?></h5>
      </div>
    </div>
  </div>
  <div class="fix-bot"><a class="ncap-btn-big ncap-btn-orange" href="index.php?app=tzx_index&wwi=tzx_index_preview" target="_blank">页面预览</a> <a class="ncap-btn-big ncap-btn-blue ml10" id="btn_index_build" href="Javascript: void(0)"><?php echo $lang['tzx_index_build'];?></a> <a class="ncap-btn-big ncap-btn-green ml10" href="<?php echo TZX_SITE_URL;?>" target="_blank"><?php echo $lang['tzx_index_go'];?></a></div>
  <div class="tzx-edit-bg-line">
...
<div id="page_module_<%=module_id%>" data-module-id="<%=module_id%>" data-module-name="<%=module_name%>" class="page-module <%=module_css%> <%=module_state_css%> module-css-<%=module_type%> module-style-<%=module_style%>">
    ...
    <a href="index.php?app=tzx_index&wwi=module_edit&module_id=<%=module_id%>" class="edit" title="<?php echo $lang['nc_edit'];?>"></a>
...
<div class="tzx-module-frame">
    <div class="tzx-module-name"><input id="input_module_title" type="text" value="自定义模块" /></div>
    <div class="tzx-module-frame-title module-title-enable">
    <span>隐藏标题</span>
    <a id="btn_toggle_module_display_title" data-module-title-state="enable" href="Javascript:;"></a>
    </div>
    <%for(var block in list) {%>
    <%if(!list[block].child) {%>
    <div nctype="btn_select_module_frame_block" data-block-name="<%=block%>" class="tzx-module-frame-<%=list[block].name%> module-frame-normal"></div>
    <%} else {%>
    <div class="tzx-module-frame-<%=list[block].name%>">
        <%for(var block_child in list[block].child) {%>
        <div nctype="btn_select_module_frame_block" data-block-name="<%=block_child%>" class="tzx-module-frame-<%=list[block].child[block_child].name%> module-frame-normal"></div>
        <%}%>
    </div>
    <%}%>
    <%}%>
