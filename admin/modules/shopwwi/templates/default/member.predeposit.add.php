...
function checkmember(){
	...
	$.getJSON("index.php?app=member&wwi=checkmember", {'name':membername}, function(data){
	        if (data)
	        {
		        $("#tr_memberinfo").show();
...
$(function(){
    ...
    $('#points_form').validate({
        ...
        messages : {
			member_name: {
				required : '<i class="fa fa-exclamation-tyq"></i>请输入会员名'
			},
			member_id : {
				required : '<i class="fa fa-exclamation-tyq"></i>会员信息错误，请重新填写会员名'
            },
            pointsnum  : {
                required : '<i class="fa fa-exclamation-tyq"></i>请添加预存款',
                min : '<i class="fa fa-exclamation-tyq"></i>预存款必须大于0'
            }
        }
    });
});
