    ...
    <table class="flex-table">
      ...
      <tbody>
        ...
        <tr class="hover">
          ...
          <td class="handle">
          <a class="btn red" href="index.php?app=shopwwi&wwi=rc_del&id=<?php echo $k;?>" onclick="if(confirm('删除后将不能恢复，确认删除这  1 项吗？')){return true;} else {return false;}"><i class="fa fa-trash-o"></i>删除</a>
          <a class="btn blue" mff="sqde" href="index.php?app=shopwwi&wwi=rc_edit&id=<?php echo $k; ?>"><i class="fa fa-pencil-square-o"></i>编辑</a>
          </td>
          <td><?php echo $v['value']; ?></td>
          <td><?php echo $v['name'];?></td>
          <td><?php echo $v['is_blod']==2? '<span class="no"><i class="fa fa-ban"></i>否</span>' : '<span class="yes"><i class="fa fa-check-tyq"></i>是</span>';?></td>
          <td></td>
        </tr>
        <?php } ?>
        <?php }else { ?>
...
function fg_operation(name, grid) {
    if (name == 'add') {
        window.location.href = 'index.php?app=shopwwi&wwi=rc_add';
    }
}
</script>