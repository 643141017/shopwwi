...
class tzx_articleControl extends SystemControl{
    //文章状态草稿箱
    ...
    public function indexWwi() {
        $this->tzx_article_listWwi();
    }

    /**
     * tzx文章列表
     **/
    public function tzx_article_listWwi()
    {
     ...
     * 待审核文章列表
     */
    public function tzx_article_list_verifyWwi()
    {
     ...
     * 已发布文章列表
     */
    public function tzx_article_list_publishedWwi()
    {
    ...
    protected function doaction($state, $menuKey)
    {
        ...
        Tpl::setDirquna('tzx');
Tpl::showpage("tzx_article.list");
    }

    public function tzx_article_list_xmlWwi()
    {
        $condition = array();
        ...
        foreach ($list as $val) {
            ...
            $o .= '<li><a target="_blank" href="' .
                    TZX_SITE_URL .
                    '/index.php?app=article&wwi=article_detail&article_id=' .
                    $val['article_id'] .
                    '">查看文章</a></li>';
            ...
            $img = getTZXArticleImageUrl($val['article_attachment_path'], $val['article_image']);
            $i['img'] = <<<EOB
...
EOB;
     ...
     * tzx文章审核
     */
    public function tzx_article_verifyWwi() {
        if(intval($_REQUEST['verify_state']) === 1) {
            $this->tzx_article_state_modify(self::ARTICLE_STATE_PUBLISHED);
        } else {
            $this->tzx_article_state_modify(self::ARTICLE_STATE_DRAFT, array('article_verify_reason' => $_POST['verify_reason']));
        }
    }
     ...
     * tzx文章收回
     */
    public function tzx_article_callbackWwi() {
        $this->tzx_article_state_modify(self::ARTICLE_STATE_DRAFT);
    }

    /**
     * 修改文章状态
     */
    private function tzx_article_state_modify($new_state, $param = array()) {
     ...
     * tzx文章分类排序修改
     */
    public function update_article_sortWwi() {
        if(intval($_GET['id']) <= 0) {
     ...
     * tzx文章分类排序修改
     */
    public function update_article_clickWwi() {
     ...
     * tzx文章删除
     **/
     public function tzx_article_dropWwi() {
        ...
        if($result) {
            $this->log(Language::get('tzx_log_article_drop').$article_id, 1);
            showMessage(Language::get('nc_common_del_succ'),'');
        } else {
            $this->log(Language::get('tzx_log_article_drop').$article_id, 0);
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'list'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_list'),'menu_url'=>'index.php?app=tzx_article&wwi=tzx_article_list'),
            'list_verify'=>array('menu_type'=>'link','menu_name'=>Language::get('tzx_article_list_verify'),'menu_url'=>'index.php?app=tzx_article&wwi=tzx_article_list_verify'),
            'list_published'=>array('menu_type'=>'link','menu_name'=>Language::get('tzx_article_list_published'),'menu_url'=>'index.php?app=tzx_article&wwi=tzx_article_list_published'),
        );
