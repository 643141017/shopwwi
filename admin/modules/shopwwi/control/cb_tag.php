...
class tzx_tagControl extends SystemControl{
    ...
    public function indexWwi() {
        $this->tzx_tag_listWwi();
    }

    /**
     * tzx文章分类列表
     **/
    public function tzx_tag_listWwi() {
        $model = Model('tzx_tag');
        $list = $model->getList(TRUE, null, 'tag_id desc');
        $this->show_menu('list');
        Tpl::output('list',$list);
        Tpl::setDirquna('tzx');
Tpl::showpage("tzx_tag.list");
    }

    /**
     * tzx文章分类添加
     **/
    public function tzx_tag_addWwi() {
        $this->show_menu('add');
        Tpl::setDirquna('tzx');
Tpl::showpage('tzx_tag.add');
    }

    /**
     * tzx文章分类保存
     **/
    public function tzx_tag_saveWwi() {
        ...
        $model_class = Model('tzx_tag');
        $result = $model_class->save($param);
        if($result) {
            $this->log(Language::get('tzx_log_tag_save').$result, 1);
            showMessage(Language::get('tag_add_success'),'index.php?app=tzx_tag&wwi=tzx_tag_list');
        } else {
            $this->log(Language::get('tzx_log_tag_save').$result, 0);
            showMessage(Language::get('tag_add_fail'),'index.php?app=tzx_tag&wwi=tzx_tag_list','','error');
        }
     ...
     * tzx标签排序修改
     */
    public function update_tag_sortWwi() {
     ...
     * tzx标签标题修改
     */
    public function update_tag_nameWwi() {
     ...
     * tzx标签删除
     **/
     public function tzx_tag_dropWwi() {
        ...
        if($result) {
            $this->log(Language::get('tzx_log_tag_drop').$_POST['tag_id'], 1);
            showMessage(Language::get('tag_drop_success'),'');
        } else {
            $this->log(Language::get('tzx_log_tag_drop').$_POST['tag_id'], 0);
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'list'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_list'),'menu_url'=>'index.php?app=tzx_tag&wwi=tzx_tag_list'),
            'add'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_new'),'menu_url'=>'index.php?app=tzx_tag&wwi=tzx_tag_add'),
        );
