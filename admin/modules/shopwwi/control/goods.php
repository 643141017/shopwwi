...
class goodsControl extends SystemControl{
     ...
     * 违规下架商品管理
     */
    public function lockup_listWwi() {
     ...
     * 等待审核商品管理
     */
    public function waitverify_listWwi() {
    ...
    public function get_xmlWwi() {
        ...
        foreach ($goods_list as $value) {
            ...
            $param['is_virtual'] = $value['is_virtual'] ==  '1' ? '<span class="yes"><i class="fa fa-check-tyq"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
            $param['virtual_indate'] = $value['is_virtual'] == '1' && $value['virtual_indate'] > 0 ? date('Y-m-d', $value['virtual_indate']) : '--';
            $param['virtual_invalid_refund'] = $value['is_virtual'] ==  '1' ? ($value['virtual_invalid_refund'] == 1 ? '<span class="yes"><i class="fa fa-check-tyq"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>') : '--';
        ...
        if (chksubmit()){
            ...
            $result = Model('goods')->editGoodsById($data, $_POST['goodsid']);
                 ...
                 if ($result){
                    $url = array(
                        array(
                            'url'=>'index.php?app=goods&wwi=add&gid='.$_POST['goodsid'].'',
                            'msg'=>'继续',
                        ),
                        array(
     ...
     * 违规下架
     */
    public function goods_lockupWwi() {
        if (chksubmit()) {
     ...
     * ajax获取商品列表
     */
    public function get_goods_sku_listWwi() {
    ...
    public function export_csvWwi() {
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('murl','index.php?app=goods&wwi=index');
								//网 店 运 维mall wwi.com
		Tpl::setDirquna('mall');
