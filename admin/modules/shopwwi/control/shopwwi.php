<?php
/**
 * 运维控件管理 网店运维 mall wwi .com
 *by wansyb QQ499063702
 */
defined('ByShopWWI') or exit('Access Invalid!');
class shopwwiControl extends SystemControl{
	 private $links = array(
	    array('url'=>'app=shopwwi&wwi=base','lang'=>'shopwwi_set'),
        array('url'=>'app=shopwwi&wwi=banner','lang'=>'top_set'),
        array('url'=>'app=shopwwi&wwi=lc','lang'=>'lc_set'),
		array('url'=>'app=shopwwi&wwi=sms','lang'=>'sms_set'),
		array('url'=>'app=shopwwi&wwi=rc','lang'=>'rc_set'),
		array('url'=>'app=shopwwi&wwi=city','lang'=>'city_set'),
		array('url'=>'app=shopwwi&wwi=webchat','lang'=>'webchat_set'),
    ...
    public function baseWwi(){
        ...
        if (chksubmit()){
			...
			$update_array['shopwwi_index_brand'] = $_POST['shopwwi_index_brand'];
			$update_array['shopwwi_index_store'] = $_POST['shopwwi_index_store'];
			$update_array['shopwwi_index_goods'] = $_POST['shopwwi_index_goods'];
			$update_array['shopwwi_default_city'] = $_POST['shopwwi_default_city'];
    ...
    public function lcWwi() {
		...
		Tpl::setDirquna('shopwwi');/*网 店 运 维mall wwi.com*/
    ...
    public function lc_addWwi() {
        ...
        if (chksubmit()) {
            if (count($lc_list) >= 8) {
                showMessage('最多可设置8个楼层','index.php?app=shopwwi&wwi=lc');
            }
            ...
            if ($result){
                showMessage('保存成功','index.php?app=shopwwi&wwi=lc');
            }else {
                showMessage('保存失败');
            }
        }
    ...
    public function lc_editWwi() {
        ...
        if (!chksubmit()) {
            ...
            $result = $model_setting->updateSetting(array('shopwwi_lc'=>serialize($lc_list)));
            if ($result){
                showMessage('编辑成功','index.php?app=shopwwi&wwi=lc');
            }
            showMessage('编辑失败');
        }
    ...
    public function rc_addWwi() {
        ...
        if (chksubmit()) {
            if (count($rc_list) >= 8) {
                showMessage('最多可设置8个楼层','index.php?app=shopwwi&wwi=rc');
            }
            ...
            if ($result){
                showMessage('保存成功','index.php?app=shopwwi&wwi=rc');
            }else {
                showMessage('保存失败');
            }
        }
    ...
    public function rc_editWwi() {
        ...
        if (!chksubmit()) {
            ...
            $result = $model_setting->updateSetting(array('shopwwi_rc'=>serialize($rc_list)));
            if ($result){
                showMessage('编辑成功','index.php?app=shopwwi&wwi=rc');
            }
            showMessage('编辑失败');
        }
     ...
     * 城市开通列表
     */
    public function cityWwi() {
        $model_setting = Model('setting');
        $city_info = $model_setting->getRowSetting('shopwwi_city');
        if ($city_info !== false) {
		...
		Tpl::setDirquna('shopwwi');/*网 店 运 维mall wwi.com*/
        Tpl::showpage('shopwwi.city');
    }

    /**
     * 楼层快速直达添加
     */
    public function city_addWwi() {
        $model_setting = Model('setting');
        $city_info = $model_setting->getRowSetting('shopwwi_city');
        if ($city_info !== false) {
            $city_list = @unserialize($city_info['value']);
        ...
        if (chksubmit()) {
            if (count($city_list) >= 20) {
                showMessage('最多可设置20个城市','index.php?app=shopwwi&wwi=city');
            }
            ...
            $result = $model_setting->updateSetting(array('shopwwi_city'=>serialize($city_list)));
            if ($result){
                showMessage('保存成功','index.php?app=shopwwi&wwi=city');
            }else {
                showMessage('保存失败');
            }
        }
        ...
        Tpl::showpage('shopwwi.city_add');
    }

    /**
     * 删除
     */
    public function city_delWwi() {
        $model_setting = Model('setting');
        $city_info = $model_setting->getRowSetting('shopwwi_city');
        if ($city_info !== false) {
            $city_list = @unserialize($city_info['value']);
        ...
        $result = $model_setting->updateSetting(array('shopwwi_city'=>serialize(array_values($city_list))));
        if ($result){
            showMessage('删除成功');
        }
        showMessage('删除失败');
    ...
    public function city_editWwi() {
        ...
        if (!chksubmit()) {
            ...
            $result = $model_setting->updateSetting(array('shopwwi_city'=>serialize($city_list)));
            if ($result){
                showMessage('编辑成功','index.php?app=shopwwi&wwi=city');
            }
            showMessage('编辑失败');
        }
