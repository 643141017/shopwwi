...
class tzx_pictureControl extends SystemControl{
    //文章状态草稿箱
    ...
    public function indexWwi() {
        $this->tzx_picture_listWwi();
    }

    /**
     * tzx文章列表
     **/
    public function tzx_picture_listWwi()
    {
     ...
     * 待审核文章列表
     */
    public function tzx_picture_list_verifyWwi()
    {
     ...
     * 已发布文章列表
     */
    public function tzx_picture_list_publishedWwi()
    {
    ...
    protected function doaction($state, $menuKey)
    {
        ...
        Tpl::setDirquna('tzx');
Tpl::showpage("tzx_picture.list");
    }

    public function tzx_picture_list_xmlWwi()
    {
        $condition = array();
        ...
        $model_picture = Model('tzx_picture');
        ...
        foreach ($list as $val) {
            ...
            $o .= '<li><a target="_blank" href="' .
                    TZX_SITE_URL .
                    '/index.php?app=picture&wwi=picture_detail&picture_id=' .
                    $val['picture_id'] .
                    '">查看画报</a></li>';
            ...
            $img = getTZXArticleImageUrl($val['picture_attachment_path'], $val['picture_image']);
            $i['img'] = <<<EOB
...
EOB;
            ...
            $i['picture_comment_flag'] = $val['picture_comment_flag'] == 1
                ? '<span class="yes"><i class="fa fa-check-tyq"></i>开启</span>'
     ...
     * tzx文章审核
     */
    public function tzx_picture_verifyWwi() {
        if(intval($_REQUEST['verify_state']) === 1) {
            $this->tzx_picture_state_modify(self::ARTICLE_STATE_PUBLISHED);
        } else {
            $this->tzx_picture_state_modify(self::ARTICLE_STATE_DRAFT, array('picture_verify_reason' => $_POST['verify_reason']));
        }
    }
     ...
     * tzx文章回收
     */
    public function tzx_picture_callbackWwi() {
        $this->tzx_picture_state_modify(self::ARTICLE_STATE_DRAFT);
    }

    /**
     * 修改文章状态
     */
    private function tzx_picture_state_modify($new_state, $param = array()) {
     ...
     * tzx文章分类排序修改
     */
    public function update_picture_sortWwi() {
        if(intval($_GET['id']) <= 0) {
     ...
     * tzx文章分类排序修改
     */
    public function update_picture_clickWwi() {
     ...
     * tzx文章删除
     **/
     public function tzx_picture_dropWwi() {
        ...
        if($result) {
            $this->log(Language::get('tzx_log_picture_drop').$picture_id, 1);
            showMessage(Language::get('nc_common_del_succ'),'');
        } else {
            $this->log(Language::get('tzx_log_picture_drop').$picture_id, 0);
    ...
    private function get_picture_state_list() {
        $array = array();
        $array[self::ARTICLE_STATE_DRAFT] = array('text'=>Language::get('tzx_text_draft'));
        $array[self::ARTICLE_STATE_VERIFY] = array('text'=>Language::get('tzx_text_verify'));
        $array[self::ARTICLE_STATE_PUBLISHED] = array('text'=>Language::get('tzx_text_published'));
        $array[self::ARTICLE_STATE_RECYCLE] = array('text'=>Language::get('tzx_text_recycle'));
        return $array;
    }

    private function show_menu($menu_key) {
        $menu_array = array(
            'list'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_list'),'menu_url'=>'index.php?app=tzx_picture&wwi=tzx_picture_list'),
            'list_verify'=>array('menu_type'=>'link','menu_name'=>Language::get('tzx_article_list_verify'),'menu_url'=>'index.php?app=tzx_picture&wwi=tzx_picture_list_verify'),
            'list_published'=>array('menu_type'=>'link','menu_name'=>Language::get('tzx_article_list_published'),'menu_url'=>'index.php?app=tzx_picture&wwi=tzx_picture_list_published'),
        );
