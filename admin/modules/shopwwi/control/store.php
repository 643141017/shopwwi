...
class storeControl extends SystemControl{
    ...
    private $_links = array(
        array('url'=>'app=store&wwi=store','text'=>'店铺管理'),
        array('url'=>'app=store&wwi=distribution_list','text'=>'分销申请')
    );
    ...
    public function get_xmlWwi() {
        ...
        foreach ($store_list as $value) {
            ...
            $operation = "<span class='btn'><em><i class='fa fa-cog'></i>" . L('nc_set') . " <i class='arrow'></i></em><ul><li><a href='index.php?app=store&wwi=store_edit&store_id=" . $value['store_id'] . "'>编辑店铺信息</a></li><li><a href='index.php?app=store&wwi=del&store_id=" . $value['store_id'] . "'>删除该店铺</a></li>";
     ...
     * 店铺分销申请表
     */
    public function distribution_listWwi(){
     ...
     * 输出XML数据
     */
    public function get_distribution_xmlWwi() {
        ...
        foreach ($distribution_list as $value) {
            ...
            if($value['distri_state'] == 0) {
                $operation .= "<a class='btn orange' href=\"javascript:void(0);\" onclick=\"distribution_check('" . $value['distri_id'] . "')\"><i class=\"fa fa-check-tyq-o\"></i>审核</a>";
            }
     ...
     * 审核店铺分销申请
     */
    public function distribution_checkWwi() {
        $id = intval($_GET['id']);
        if ($id > 0) {
            $model_store_distribution = Model('store_distribution');
            $condition = array();
     ...
     * 删除店铺分销申请
     */
    public function distribution_delWwi() {
        $id = intval($_GET['id']);
        if ($id > 0) {
            $model_store_distribution = Model('store_distribution');
            $condition = array();
