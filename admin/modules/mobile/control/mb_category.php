...
class mb_categoryControl extends SystemControl{
    ...
    public function indexWwi() {
        $this->mb_category_listWwi();
    }
    /**
     *
     */
    public function mb_category_listWwi(){
...
Tpl::showpage('mb_category.list');
     ...
     * 删除
     */
    public function mb_category_delWwi(){
        $lang   = Language::getLangContent();
        if (intval($_GET['gc_id']) > 0){
            ...
            showMessage($lang['link_index_del_succ'],'index.php?app=mb_category&wwi=mb_category_list');
        }else {
            showMessage($lang['link_index_choose_del'],'index.php?app=mb_category&wwi=mb_category_list');
        }
    }
     ...
     * 添加
     */
    public function mb_category_addWwi(){
        ...
        if ($_POST['form_submit'] == 'ok'){
            ...
            if ($result){
                $url = array(
                    array(
                        'url'=>'index.php?app=mb_category&wwi=mb_category_add',
                        'msg'=>$lang['link_add_again'],
                    ),
                    array(
                        'url'=>'index.php?app=mb_category&wwi=mb_category_list',
...
Tpl::showpage('mb_category.add');
     ...
     * 编辑
     */
    public function mb_category_editWwi(){
        $lang   = Language::getLangContent();
        $model_link = Model('mb_category');
        if ($_POST['form_submit'] == 'ok'){
            ...
            if ($result){
                ...
                $url = array(
                    array(
                        'url'=>'index.php?app=mb_category&wwi=mb_category_edit&gc_id='.intval($_POST['gc_id']),
