...
class tyq_memberlevelControl extends SystemControl{
    public function __construct(){
        parent::__construct();
        Language::read('tyq_memberlevel');
    }
    /**
    ...
    public function indexWwi(){
        ...
        if(chksubmit()){
            ...
            $rs = $model->table('tyq_mldefault')->insertAll($insert_all,array(),true);
            if($rs){
        ...
        $mld_list = $model->table('tyq_mldefault')->select();
        $mld_list = array_under_reset($mld_list, 'mld_id');
        Tpl::output('mld_list', $mld_list);
        Tpl::setDirquna('tyq');
Tpl::showpage('tyq_memberlevel');
    }
    /**
     * Reference Title list
     */
    public function refWwi(){
        $model = Model();
        if(chksubmit()){
            ...
            $rs = $model->table('tyq_mlref')->where(array('mlref_id'=>array('in', $mlrefid_array)))->delete();
            if($rs){
                showMessage(L('nc_common_op_succ'));
            }else{
                showMessage(L('nc_common_op_fail'));
        ...
        $mlref_list = $model->table('tyq_mlref')->select();
        Tpl::output('mlref_list', $mlref_list);
        Tpl::setDirquna('tyq');
Tpl::showpage('tyq_memberlevel.ref');
    }
    /**
     * Add the Reference Title
     */
    public function ref_addWwi(){
        if(chksubmit()){
            $insert_array = array();
            ...
            $rs = Model()->table('tyq_mlref')->insert($insert_array);
            if($rs){
                $url = array(
                        array(
                                'url'=>'index.php?app=tyq_memberlevel&wwi=ref_add',
                                'msg'=>L('tyq_continue_add'),
                        ),
                        array(
                                'url'=>'index.php?app=tyq_memberlevel&wwi=ref',
                                'msg'=>L('nc_backlist'),
                        )
                );
        ...
        Tpl::setDirquna('tyq');
Tpl::showpage('tyq_memberlevel.ref_add');
    }
    /**
     * Edit the Reference Title
     */
    public function ref_editWwi(){
        $model = Model();
        if(chksubmit()){
            $update_array = array();
            ...
            $rs = $model->table('tyq_mlref')->where(array('mlref_id'=>intval($_POST['mlref_id'])))->update($update_array);

            if($rs){
                showMessage(L('nc_common_op_succ'), 'index.php?app=tyq_memberlevel&wwi=ref');
            }else{
                showMessage(L('nc_common_op_fail'));
            }
        }
        $mlref_info = $model->table('tyq_mlref')->where(array('mlref_id'=>intval($_GET['mlref_id'])))->find();
        if(empty($mlref_info)){
            showMessage(L('param_error'));
        }
        Tpl::output('mlref_info', $mlref_info);
        Tpl::setDirquna('tyq');
Tpl::showpage('tyq_memberlevel.ref_edit');
    }
    /**
     * elete the Reference Title
     */
    public function ref_delWwi(){
        $mlref_id = intval($_GET['mlref_id']);
        if($mlref_id <= 0){
            showMessage(L('param_error'));
        }
        $rs = Model()->table('tyq_mlref')->where(array('mlref_id'=>$mlref_id))->delete();
    ...
    public function ajaxWwi(){
        switch ($_GET['branch']){
            case 'status':
                ...
                Model()->table('tyq_mlref')->where(array('mlref_id'=>intval($_GET['id'])))->update($update_array);
                echo 'true';exit;
                break;
        }
    }
     ...
     * 更新圈子成员等级缓存
     */
    public function update_cacheWwi() {
        dkcache('tyq_level');
        showMessage(L('nc_common_op_succ'), 'index.php?app=tyq_memberlevel');
