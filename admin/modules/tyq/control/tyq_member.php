...
class tyq_memberControl extends SystemControl{
    ...
    public function member_listWwi(){
        ...
        if($_GET['tyqname'] != ''){
            $where['tyq_name'] = array('like', '%'.$_GET['tyqname'].'%');
        }
        ...
        $member_list = $model->table('tyq_member')->where($where)->page(10)->order($order)->select();
        Tpl::output('show_page', $model->showpage('2'));
        Tpl::output('member_list', $member_list);
        Tpl::setDirquna('tyq');
Tpl::showpage('tyq_member.list');
    }
    ...
    public function get_xmlWwi() {
        ...
        $order = '';
        $param = array('member_id', 'member_name', 'tyq_id', 'tyq_name', 'is_recommend', 'cm_jointime', 'is_identity', 'is_star', 'cm_thcount'
        ...
        $page = $_POST['rp'];
        $member_list = $model->table('tyq_member')->where($condition)->page($page)->order($order)->select();
        ...
        foreach ($member_list as $value) {
            $param = array();
            $operation = "<a class='btn red' href='javascript:void(0);' onclick=\"fg_del('".$value['member_id']."|".$value['tyq_id']."')\"><i class='fa fa-list-alt'></i>删除</a>";
            $operation .= "<span class='btn'><em><i class='fa fa-cog'></i>" . L('nc_set') . " <i class='arrow'></i></em><ul>";
            $operation .= "<li><a href='".MALL_SITE_URL."/index.php?app=sns_tyq&mid=".$value['member_id']."' target=\"_blank\">查看成员信息</a></li>";
            if ($value['is_recommend'] == 1) {
                $operation .= "<li><a href='javascript:void(0);' onclick=\"fg_recommend('".$value['member_id']."|".$value['tyq_id']."', 0)\">取消成员推荐</a></li>";
            } else {
                $operation .= "<li><a href='javascript:void(0);' onclick=\"fg_recommend('".$value['member_id']."|".$value['tyq_id']."', 1)\">推荐优秀成员</a></li>";
            }
            $operation .= "</ul></span>";
            ...
            $data['list'][$value['member_id']."|".$value['tyq_id']] = $param;
        }
    ...
    public function member_delWwi(){
        ...
        foreach ($param as $value) {
            ...
            list($member_id, $tyq_id) = $tpl_param;
            $where['member_id'] = $member_id;
            $where['tyq_id'] = $tyq_id;
            Model()->table('tyq_member')->where($where)->delete();
     ...
     * 推荐/取消会员
     */
    public function member_recommendWwi(){
        $array = explode('|', $_GET['id']);
        list($member_id, $tyq_id) = $array;
        $where = array();
        $where['member_id'] = $member_id;
        $where['tyq_id'] = $tyq_id;
