...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?app=tyq_manage&wwi=tyq_list" title="返回圈子列表"><i class="fa fa-arrow-tyq-o-left"></i></a>
      <div class="subject">
        <h3><?php echo $lang['nc_tyq_manage'];?> - <?php echo $lang['nc_new'];?>圈子</h3>
...
function call_back(picname){
	$('#c_img').val(picname);
	$('#view_img').attr('src','<?php echo UPLOAD_SITE_URL.'/'.ATTACH_TYQ;?>/group/'+picname);
}
// 选择圈主
...
$(function(){
		...
		(
			{
				...
				{
					if (data.status == 1){
						ajax_form('cutpic','<?php echo $lang['nc_cut'];?>','<?php echo ADMIN_SITE_URL?>/index.php?app=common&wwi=pic_cut&type=tyq&x=120&y=120&resize=1&ratio=1&url='+data.url,690);
					}else{
						alert(data.msg);
	...
	$('#mastername').focus(function(){
		ajax_form('choose_master', '<?php echo $lang['tyq_choose_master'];?>', 'index.php?app=tyq_manage&wwi=choose_master', 640);
	});
	...
	$('#tyq_form').validate({
        ...
        rules : {
        	c_name : {
        		...
        		maxlength : 12,
            	remote : {
            		url : 'index.php?app=tyq_manage&wwi=check_tyq_name',
                    type: 'get',
                    data:{
                    	name : function(){
                            return $('#c_name').val();
            ...
            mastername : {
            	required : true,
                remote   : {
                    url :'index.php?app=tyq_manage&wwi=check_member&branch=ok',
                    type:'get',
                    data:{
                        name : function(){
