...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      <div class="subject">
        <h3><?php echo $lang['nc_tyq_membermanage'];?></h3>
        <h5><?php echo $lang['nc_tyq_membermanage_subhead'];?></h5>
      </div>
    </div>
  </div>
  <div class="explanation" id="explanation">
    ...
    <ul>
      <li><?php echo $lang['tyq_member_prompts_one'];?></li>
      <li><?php echo $lang['tyq_member_prompts_two'];?></li>
      <li><?php echo $lang['tyq_member_prompts_three'];?></li>
    </ul>
  </div>
...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?app=tyq_member&wwi=get_xml',
        colModel : [
            ...
            {display: '圈子名称', name : 'tyq_name', width : 120, sortable : true, align: 'left'},
...
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?app=tyq_member&wwi=member_del', {id:id}, function(data){
            if (data.state) {
...
function fg_recommend(id, value) {
    $.getJSON('index.php?app=tyq_member&wwi=member_recommend', {id:id, value:value}, function(data){
        if (data.state) {
            $("#flexigrid").flexReload();
        } else {
            showError(data.msg)
