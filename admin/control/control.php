...
class SystemControl{
    ...
    protected function __construct() {
        ...
        if (($_GET['branch']!='' || $_GET['wwi']=='ajax') && strtoupper(CHARSET) == 'GBK'){
    ...
    protected final function systemLogin() {
        ...
        if (!key_exists('gid',(array)$user) || !isset($user['sp']) || (empty($user['name']) || empty($user['id']))){
            @header('Location: '. ADMIN_SITE_URL .'/index.php?app=login&wwi=login');exit;
        }else {
    ...
    protected final function sublink($links = array(), $actived = '', $file='index.php') {
        ...
        foreach ($links as $k=>$v) {
            ...
            if (empty($array['wwi'])) $array['wwi'] = 'index';
            if (!$this->checkPermission($array)) continue;
            $href = ($array['wwi'] == $actived ? null : "href=\"{$file}?{$v['url']}\"");
            $class = ($array['wwi'] == $actived ? "class=\"current\"" : null);
    ...
    protected final function log($lang = '', $state = 1, $admin_name = '', $admin_id = 0) {
        ...
        $data['ip']         = getIp();
        $data['url']        = $_REQUEST['app'].'&'.$_REQUEST['wwi'];
