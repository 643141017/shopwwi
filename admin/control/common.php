...
// 				$save_file2 = BASE_UPLOAD_PATH.'/'.$_POST['filename'];
     ...
     * 查询每月的周数组
     */
    public function getweekofmonthWwi(){
     ...
     * 设置常用菜单
     */
    public function common_operationsWwi() {
        $type = $_GET['type'];
        $value = $_GET['value'];
     ...
     * 代办事项
     */
    public function pending_mattersWwi() {
     ...
     * 代办事项ajax数据
     */
    public function ajax_pending_mattersWwi() {
        $statistics  = $this->get_pending_matters();
        $count = 0;
    ...
    private function get_pending_matters() {
        ...
        if (C('tzx_isuse')) {
            // 文章审核
            $statistics['tzx_article_verify'] = Model('tzx_article')->getTzxArticleCount(array('article_state' => 2));
            // 画报审核
            $statistics['tzx_picture_verify'] = Model('tzx_picture')->getTzxPictureCount(array('picture_state' => 2));
        }
        /**
         * 圈子
         */
        if (C('tyq_isuse')) {
            $statistics['tyq_verify'] = Model('tyq')->getTyqUnverifiedCount();
        }
        return $statistics;
    }
}
