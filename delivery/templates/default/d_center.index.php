    ...
    <tbody>
      ...
      <tr class="hover">
        ...
        <td><dl>
            ...
            <dd><a href="javascript:void(0);" class="link" onclick="javascript:ajax_form('get_express', '查看物流', 'index.php?app=d_center&wwi=get_express&e_code=<?php echo $v['express_code'];?>&shipping_code=<?php echo $v['shipping_code'];?>')">查看物流跟踪</a></dd>
        ...
        <td class="tc handle"><?php if ($v['dlyo_state'] == DELIVERY_ORDER_DEFAULT) {?>
          <a href="javascript:void(0);" class="btn" onclick="javascript:ajax_get_confirm('请确认包裹已经到站，提醒买家取件？','<?php echo DELIVERY_SITE_URL;?>/index.php?app=d_center&wwi=arrive_point&order_id=<?php echo $v['order_id'];?>');">到站</a>
          <?php } else if ($v['dlyo_state'] == DELIVERY_ORDER_ARRIVE) {?>
          <a href="javascript:void(0);" class="btn" onclick="javascript:ajax_form('pickup_parcel', '取货', 'index.php?app=d_center&wwi=pickup_parcel&order_id=<?php echo $v['order_id'];?>')">取货</a>
          <?php } else {?>
          --
          <?php }?></td>
      </tr>
