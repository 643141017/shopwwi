...
$(document).ready(function(){
    ...
    //摘要计数
    $("[name='picture_image_abstrapp[]']").nc_text_count({max:140});
    ...
    $("#picture_image_upload").fileupload({
        dataType: 'json',
            url: "<?php echo TZX_SITE_URL.DS.'index.php?app=publish&wwi=article_image_upload';?>",
            add: function(e,data) {
                ...
                if(result.status == "success") {
                    ...
                    var image_handle = '<a nctype="btn_set_picture_image" image_name="'+result.file_name+'"><?php echo $lang['tzx_cover'];?></a>';
                    ...
                    var image_content = '<dd class="picture-abstract"><h4><?php echo $lang['tzx_image_abstract'];?></h4>';
    ...
    $("[nctype='btn_drop_picture_image']").live("click",function(){
        ...
        $.getJSON("<?php echo TZX_SITE_URL.DS.'index.php?app=publish&wwi=article_image_drop';?>", { image_name: image_name }, function(result){
                if(result.status == "success") {
                    image_object.remove();
                } else {
                    showError(result.error);
    ...
    $("[nctype='btn_goods_add_commit']").live('click', function() {
        ...
        if(goods_list.find("dl").length < 3) {
            ...
            var image_name = $(this).attr("image_name");
            if(url != '') {
                $.getJSON("<?php echo TZX_SITE_URL.DS.'index.php?app=api&wwi=goods_info_by_url';?>", { url: url}, function(data){
                    if(data.result == "true") {
                        ...
                        url_item += '<dd class="taobao-item-delete" nctype="btn_image_goods_delete"><?php echo $lang['tzx_delete'];?></dd>';
    ...
    //标签操作
    $("[nctype='tzx_tag']").live("click",function(){
        var current_css = $(this).attr("class");
        if(current_css == "btn-tzx-tag") {
            $(this).attr("class","btn-tzx-tag-selected");
        } else {
            $(this).attr("class","btn-tzx-tag");
        }
        var tzx_tag_selected = '';
        $(".btn-tzx-tag-selected").each(function(){
            tzx_tag_selected += $(this).attr("tag_id") + ",";
        });
        $("#picture_tag").val(tzx_tag_selected.substring(0, tzx_tag_selected.length-1));
    });

});
</script>
<div class="tzx-content mt10">
  <form id="add_form" method="post" action="index.php?app=publish&wwi=publish_picture_save">
    ...
    <div class="tzx-publish-content">
      <table class="tzx-publish-table tzx-publish-table-picture">
        <tbody>
          <tr>
            <th><?php echo $lang['tzx_article_class'];?><?php echo $lang['nc_colon'];?></th>
            ...
            <td rowspan="3" class="w200"><input id="picture_image" name="picture_image" type="hidden" value="" />
              <div class="edit-cover-pic">
                ...
                <?php if(!empty($output['picture_detail']['picture_image'])) { ?>
                 <img alt="" src="<?php echo getTZXArticleImageUrl($output['picture_detail']['picture_attachment_path'],$output['picture_detail']['picture_image'], 'list');?>">
                 <?php } ?>
             </div>
          ...
          <tr>
            <th><?php echo $lang['tzx_image_upload'];?><?php echo $lang['nc_colon'];?></th>
            <td colspan="2">
                ...
                <li>
                  <dl class="picture-content">
                    <dt class="picture-img">
                      <div class="upload-thumb"><a href="Javascript: void(0);"><img nctype="picture_image_item" alt="" src="<?php echo getTZXArticleImageUrl(empty($value['image_path'])?$output['picture_detail']['picture_attachment_path']:$value['image_path'], $value['image_name'], 'list');?>"></a></div>
                      <div class="handle"><a image_name="<?php echo $value['image_name'];?>" nctype="btn_set_picture_image"><?php echo $lang['tzx_cover'];?></a><a image_name="<?php echo $value['image_name'];?>" nctype="btn_drop_picture_image"><?php echo $lang['nc_delete'];?></a></div>
                    </dt>
                    <input type="hidden" value="<?php echo $value['image_name'].','.$value['image_path'];?>" name="picture_image_all[]">
                    <dd class="picture-abstract">
                      <h4><?php echo $lang['tzx_image_abstract'];?></h4>
                    ...
                    <dd class="picture-add-goods">
                      <input class="btn-type-s" nctype="btn_add_image_goods" image_name="<?php echo $value['image_name'];?>" type="button" value="<?php echo $lang['tzx_goods_add'];?>" />
                    </dd>
                  </dl>
          ...
          <tr>
            <th><?php echo $lang['tzx_text_publisher'];?></th>
          ...
          <tr>
            <th><?php echo $lang['tzx_publish_time'];?></th>
          ...
          <tr>
            ...
            <td colspan="2"><input id="picture_keyword" class="text w380" name="picture_keyword" type="text" value="<?php if(!empty($output['picture_detail'])) echo $output['picture_detail']['picture_keyword'];?>"/><div class="hint"><?php echo $lang['tzx_keyword_expalin'];?></div></td>
          </tr>
        </tbody>
        <tfoot>
          <tr>
            <td colspan="21"><a id="btn_draft" class="btn-draft" href="javascript:void(0);"><?php echo $lang['tzx_article_save_draft'];?></a> <a id="btn_publish" class="btn-publish" href="javascript:void(0);"><?php echo $lang['tzx_text_publish'];?></a></td>
          </tr>
        </tfoot>
      </table>
    </div>
