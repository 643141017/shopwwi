...
<link href="<?php echo TZX_TEMPLATES_URL;?>/css/layout.css" rel="stylesheet" type="text/css">
...
<script type="text/javascript" src="<?php echo TZX_RESOURCE_SITE_URL;?>/js/common.js" charset="utf-8"></script>
...
$(function(){
	//search
	$("#searchTZX").children('ul').children('li').click(function(){
		$(this).parent().children('li').removeClass("current");
		$(this).addClass("current");
        $("#form_search").attr("action", $(this).attr("action"));
        $("#app").val($(this).attr("app"));
        $("#wwi").val($(this).attr("wwi"));
	});
    var search_current_item = $("#searchTZX").children('ul').children('li.current');
    $("#form_search").attr("action", search_current_item.attr("action"));
    $("#app").val(search_current_item.attr("app"));
    $("#wwi").val(search_current_item.attr("wwi"));
});
//登录开关状态
...
<div class="headNew-bg">
    <div class="head-wrap">
        <div class="index wwi-left">
            <a href="<?php echo MALL_SITE_URL;?>" target="_blank">运维首页</a>
            <em class="download"></em>
        </div>
        <div class="box-xx box_xx-line wwi-left">|</div>
        <div class="phone">
            <a href="<?php echo WAP_SITE_URL;?>" target="_blank">运维移动APP</a>
        ...
        <div class="box-xx box_xx-line wwi-left">|</div>
        <div class="weixin">
            <a href="javascript:;" target="_blank">运维微信号</a>
            <em class="download"></em>
        </div>
      ...
      <?php } ?><?php echo $_SESSION['member_name'];?></a></span><div class="hover-show">
        ...
        <div id="notLogin" class="sign-box to-sign">
            ...
            <div class="box-xx box_xx-line">|</div>
            <div class="sign-right"><span class="callUs">联系客服<em><?php echo $output['setting_config']['shopwwi_phone']; ?></em></span></div>
        </div>
        <!--//未登录-->
       <?php }?>
    </div>
...
$(function(){
	...
	$('#keyword').focus(function(){
        ...
        source: function (request, response) {
            $.getJSON('<?php echo MALL_SITE_URL;?>/index.php?app=search&wwi=auto_complete', request, function (data, status, xhr) {
	...
	$('#button').click(function(){
      if ($('#keyword').val() == '') {
        ...
        window.location.href="<?php echo MALL_SITE_URL?>/index.php?app=search&wwi=index&keyword="+$('#keyword').attr('data-value');
          return false;
      }
      }
  });
  ...
  <header class="public-head-layout wrapper">
    <h1 class="site-logo"><a href="<?php echo MALL_SITE_URL;?>" class="new-logo"><img src="<?php echo UPLOAD_SITE_URL.DS.ATTACH_COMMON.DS.$output['setting_config']['site_logo']; ?>" class="pngFix"></a>    <div class="cityCont">
                <span class="cityNow"><em><?php if($_COOKIE['city'] !=null){ echo  $_COOKIE['city']; } else{?>
		    		<?php echo $output['setting_config']['shopwwi_default_city']; ?>
		    		<?php }?></em></span>
                <div class="cityItem">
                    <p>
    ...
    <div class="navCont">
           ...
           <!-- <a href="<?php echo urlMall('consult', 'index');?>" <?php if($output['index_sign'] == 'consult' && $output['index_sign'] != '0') {echo 'class="now"';} ?>>商家应答</a>
            ...
            <a href="<?php echo urlMall('store_list', 'index');?>" <?php if($output['index_sign'] == 'store_list' && $output['index_sign'] != '0') {echo 'class="now"';} ?>>淘店铺</a>
                        <?php if (C('tzx_isuse')){ ?>
            <a href="<?php echo TZX_SITE_URL;?>" <?php if($output['index_signs'] == 'tzx' && $output['index_sign'] != '0') {echo 'class="now"';} ?>>淘资讯</a><?php } ?>
            	<?php if (C('tyq_isuse')){ ?>
...
<header id="topHeader">
  <div class="warp-all">
    <div class="tzx-logo"><a href="<?php echo TZX_SITE_URL;?>">
