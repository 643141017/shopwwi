...
$(document).ready(function(){
    var url_comment_list = "<?php echo TZX_SITE_URL.DS;?>index.php?app=comment&wwi=comment_list&type=<?php echo $_GET['app'];?>&comment_object_id=<?php echo $output['detail_object_id'];?>&comment_all=<?php echo $output['comment_all'];?>";
    $("#btn_comment_submit").click(function(){
        if($("#input_comment_message").val() != '') {
        $.post("<?php echo TZX_SITE_URL.DS.'index.php?app=comment&wwi=comment_save';?>", $("#add_form").serialize(),
            function(data){
                if(data.result == 'true') {
    ...
    $("[nctype=comment_drop]").live('click',function(){
        if(confirm('<?php echo $lang['nc_ensure_del'];?>')) {
            ...
            $.post("index.php?app=comment&wwi=comment_drop&type=<?php echo $_GET['app'];?>", { comment_id: $(this).attr("comment_id")}, function(json){
                if(json.result == "true") {
                    item.remove();
                } else {
    ...
    $('#comment_list').on('click', '[nctype="btn_comment_quote_publish"]', function() {
        ...
        $.post("<?php echo TZX_SITE_URL.DS.'index.php?app=comment&wwi=comment_save';?>", {comment_id:comment_id, comment_object_id:comment_object_id, comment_type:comment_type, comment_message:comment_message},
            function(data){
                if(data.result == 'true') {
                    $("#input_comment_message").val("");
                    $("#comment_list").load(url_comment_list);
    ...
    $('#comment_list').on('click', '[nctype="comment_up"]', function() {
        ...
        $.post("<?php echo TZX_SITE_URL.DS.'index.php?app=comment&wwi=comment_up';?>", {comment_id:comment_id},
            function(data){
                if(data.result == 'true') {
...
<div id="tzx_comment_flag" class="article-comment-title">
  <h3></h3>
  <span><?php echo $lang['tzx_comment1'];?><a href="<?php echo TZX_SITE_URL.DS;?>index.php?app=<?php echo $_GET['app'];?>&wwi=<?php echo $_GET['app'];?>_comment_detail&<?php echo $_GET['app'];?>_id=<?php echo $output['detail_object_id'];?>"><em><?php echo $output[$_GET['app'].'_detail'][$_GET['app'].'_comment_count'];?></em></a><?php echo $lang['tzx_comment2'];?><em><?php echo $output[$_GET['app'].'_detail'][$_GET['app'].'_click'];?></em><?php echo $lang['tzx_comment3'];?></span> </div>
<form id="add_form" action="" class="article-comment-form">
  ...
  <?php if($_SESSION['is_login'] != '1'){?>
  <div class="article-comment-login"><?php echo $lang['tzx_comment4'];?><a id="btn_login" href="###">[<?php echo $lang['tzx_login'];?>]</a><?php echo $lang['tzx_comment5'];?></div>
  <?php }?>
</form>
