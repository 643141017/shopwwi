...
class TZXControl{
    ...
    const PICTURE = 2;
    //用户中心文章列表页
    const TZX_MEMBER_ARTICLE_URL = 'index.php?app=member_article&wwi=article_list';
    const TZX_MEMBER_PICTURE_URL = 'index.php?app=member_picture&wwi=picture_list';
    ...
    public function __construct(){
         ...
         * seo
         */
        Tpl::output('html_title',C('tzx_seo_title').'-'.C('site_name').' - Powered by ShopWWI完美S5版');
        Tpl::output('seo_keywords',C('tzx_seo_keywords'));
        Tpl::output('seo_description',C('tzx_seo_description'));
        ...
        if(intval(C('tzx_submit_verify_flag')) === 1) {
    ...
    protected function check_login() {
        if(!isset($_SESSION['is_login'])) {
            $ref_url = TZX_SITE_URL.request_uri();
    ...
    protected function get_article_state_list() {
        $array = array();
        $array[self::ARTICLE_STATE_DRAFT] = Language::get('tzx_text_draft');
        $array[self::ARTICLE_STATE_VERIFY] = Language::get('tzx_text_verify');
        $array[self::ARTICLE_STATE_PUBLISHED] = Language::get('tzx_text_published');
        $array[self::ARTICLE_STATE_RECYCLE] = Language::get('tzx_text_recycle');
        return $array;
    }
    ...
    protected function share_app_publish($publish_info=array()) {
        $param = array();
        $param['comment'] = "'".$_SESSION['member_name']."'".Language::get('tzx_text_zai').C('tzx_seo_title').Language::get('share_article');
        $param['title'] = "'".$_SESSION['member_name']."'".Language::get('tzx_text_zai').C('tzx_seo_title').Language::get('share_article');
...
class TZXHomeControl extends TZXControl{

    public function __construct() {
        parent::__construct();
        $model_navigation = Model('tzx_navigation');
        $navigation_list = $model_navigation->getList(TRUE, null, 'navigation_sort asc');
        Tpl::output('navigation_list', $navigation_list);

        $model_article_class = Model('tzx_article_class');
        ...
        $model_picture_class = Model('tzx_picture_class');
...
class TZXMemberControl extends TZXControl{
    ...
    protected function drop_image($attachment_path, $image_name) {
        $image = BASE_UPLOAD_PATH.DS.ATTACH_TZX.DS.$attachment_path.DS.$image_name;
        if(is_file($image)) {
            unlink($image);
        }
    }
