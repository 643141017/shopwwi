...
class pictureControl extends TZXHomeControl{
     ...
     * 画报详细页
     */
    public function picture_detailWwi() {
        ...
        if(empty($picture_detail)) {
            showMessage(Language::get('picture_not_exist'), TZX_SITE_URL, '', 'error');
        }
     ...
     * 画报评论
     */
    public function picture_comment_detailWwi() {
        $picture_id = intval($_GET['picture_id']);
        if($picture_id <= 0) {
            showMessage(Language::get('wrong_argument'),'','','error');
        }
     ...
     * 画报详细页(图片列表)
     */
    public function picture_detail_imageWwi() {
        $picture_id = intval($_GET['picture_id']);
        if($picture_id <= 0) {
            showMessage(Language::get('wrong_argument'),'','','error');
        }
     ...
     * 画报搜索
     */
    public function picture_searchWwi() {
        $condition = array();
     ...
     * 根据标签搜索
     */
    public function picture_tag_searchWwi() {
        $picture_list = array();
