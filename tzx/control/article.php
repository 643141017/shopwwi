...
class articleControl extends TZXHomeControl{
     ...
     * 文章列表
     */
    public function article_detailWwi() {
        ...
        if(empty($article_detail)) {
            showMessage(Language::get('article_not_exist'), TZX_SITE_URL, '', 'error');
        }
     ...
     * 文章评论
     */
    public function article_comment_detailWwi() {
        $article_id = intval($_GET['article_id']);
        if($article_id <= 0) {
            showMessage(Language::get('wrong_argument'),'','','error');
        }
     ...
     * 文章列表
     */
    public function article_searchWwi() {
        $condition = array();
     ...
     * 根据标签搜索
     */
    public function article_tag_searchWwi() {
        $article_list = array();
