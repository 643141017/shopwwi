...
class commentControl extends TZXHomeControl{

    public function __construct() {
        parent::__construct();
        //是否关闭投稿功能
        if(intval(C('tzx_comment_flag')) !== 1) {
     ...
     * 评论保存
     **/
    public function comment_saveWwi() {
     ...
     * 评论列表
     **/
    public function comment_listWwi() {
        $page_count = 5;
        ...
        if($comment_object_id > 0 && $comment_type > 0) {
            ...
            $model_tzx_comment = Model('tzx_comment');
            $comment_list = $model_tzx_comment->getListWithUserInfo($condition, $page_count, $order);
            Tpl::output('comment_list', $comment_list);
            if($_GET['comment_all'] === 'all') {
                Tpl::output('show_page', $model_tzx_comment->showpage(2));
            }
            ...
            if(!empty($comment_quote_id)) {
                $comment_quote_list = $model_tzx_comment->getListWithUserInfo(array('comment_id'=>array('in', $comment_quote_id)));
            }
    ...
    public function comment_upWwi() {
        ...
        $comment_id = intval($_POST['comment_id']);
        if($comment_id > 0) {
            $model_comment_up = Model('tzx_comment_up');
            $param = array();
