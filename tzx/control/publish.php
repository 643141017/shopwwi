...
class publishControl extends TZXMemberControl{
    ...
    public function indexWwi() {
        $this->publish_articleWwi();
    }

    /**
     * 文章发布
     */
    public function publish_articleWwi(){
        ...
        Tpl::showpage('publish_article','tzx_member_layout');
    }

    /**
     * 文章发布
     */
    public function publish_article_saveWwi(){
        ...
        if(!empty($_POST['article_image_all'])) {
            ...
            foreach ($_POST['article_image_all'] as $value) {
                ...
                $article_image_url = getTZXArticleImageUrl($file_path, $file_name, 'max');
        ...
        $model_tag_relation = Model('tzx_tag_relation');
        if(!empty($_POST['article_id'])) {
        ...
        if($article_id) {
            showMessage(Language::get('nc_common_save_succ'),self::TZX_MEMBER_ARTICLE_URL);
        } else {
            showMessage(Language::get('nc_common_save_fail'),self::TZX_MEMBER_ARTICLE_URL,'','error');
        }
    }
     ...
     * 画报发布
     */
    public function publish_pictureWwi(){
        ...
        Tpl::showpage('publish_picture','tzx_member_layout');
     ...
     * 画报保存
     */
    public function publish_picture_saveWwi() {
        ...
        $model_tag_relation = Model('tzx_tag_relation');
        $model_picture_image = Model('tzx_picture_image');
        ...
        if(!empty($_POST['picture_image_all'])) {
            ...
            for ($i = 0; $i < $image_count; $i++) {
                ...
                $picture_image_url = getTZXArticleImageUrl($file_path, $file_name, 'max');
        ...
        if($picture_id) {
            showMessage(Language::get('nc_common_save_succ'),self::TZX_MEMBER_PICTURE_URL);
        } else {
            showMessage(Language::get('nc_common_save_fail'),self::TZX_MEMBER_PICTURE_URL,'','error');
        }
     ...
     * 文章图片上传
     */
    public function article_image_uploadWwi() {
        $data = array();
        $data['status'] = 'success';
        if(!empty($this->publisher_name)) {
            if(!empty($_FILES['article_image_upload']['name'])) {
                $upload = new UploadFile();
                $upload->set('default_dir',ATTACH_TZX.DS.'article'.DS.$this->attachment_path);
                ...
                $data['file_url'] = getTZXArticleImageUrl($this->attachment_path, $upload->file_name, 'max');
     ...
     * 文章图片删除
     */
    public function article_image_dropWwi() {
        $data = array();
        $data['status'] = 'success';
