 ...
 //图片上传
    $("#article_image_upload").fileupload({
        dataType: 'json',
            url: "index.php?app=publish&wwi=article_image_upload",
            add: function(e,data) {
    ...
    $("[nctype='btn_drop_article_image']").live("click",function(){
        ...
        $.getJSON("index.php?app=publish&wwi=article_image_drop", { image_name: image_name }, function(result){
            if(result.status == "success") {
                image_object.remove();
            } else {
                showError(result.error);
    ...
    $("[nctype='tzx_tag']").live("click",function(){
        ...
        $("#article_tag").val(tzx_tag_selected.substring(0, tzx_tag_selected.length-1));
    });
        ...
        if($("#article_goods_list li").length < 3) { 
            ...
            if(search_keyword != "") {
                if(search_type == "goods_url") {
                    ...
                    $.getJSON("index.php?app=api&wwi=goods_info_by_url", { url: url}, function(data){
                        if(data.result == "true") {
                    ...
                    $("#div_goods_select").load("index.php?app=api&wwi=goods_list&search_type="+search_type+"&search_keyword="+search_keyword);
                }
            }
        }
    });
    ...
    $("#btn_article_search").click(function(){
        ...
        if(search_keyword != "") {
            $("#div_article_select").load("index.php?app=api&wwi=article_list&search_type="+search_type+"&search_keyword="+search_keyword);
        }
    });
    ...
    function refresh_article_link() {
        var tzx_article_selected = '';
        $("#article_link_list [nctype='btn_article_select']").each(function(){
            tzx_article_selected += $(this).attr("article_id") + ",";
        });
        $("#article_link").val(tzx_article_selected.substring(0, tzx_article_selected.length-1));
    } 
