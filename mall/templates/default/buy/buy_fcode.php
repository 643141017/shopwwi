...
$(function(){
    $('#fcode_submit').on('click',function(){
        ...
        $('#fcode_callback').val('');
		$.get("index.php?app=buy&wwi=check_fcode", {'fcode':$('#fcode').val(),'goods_id':$('#goods_id').val()}, function(data){
            if (data == '1') {
            	$('#fcode_callback').val('1');
            	$('#fcode_submit').hide();
            	$('#fcode').hide();
            	$('#fcode_showmsg').append('<i class="icon-ok-tyq"></i>'+$('#fcode').val()+'为有效的F码，您可以继续完成下单购买。');
            } else {
