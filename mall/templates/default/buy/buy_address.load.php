...
function delAddr(id){
    $('#addr_list').load(SITEURL+'/index.php?app=buy&wwi=load_addr&ifchain=<?php echo $_GET['ifchain'];?>&id='+id);
}
$(function(){
    function addAddr() {
        $('#add_addr_box').load(SITEURL+'/index.php?app=buy&wwi=add_addr');
    }
    $('input[nc_type="addr"]').on('click',function(){
        ...
        if ($(this).val() == '0') {
        	...
        	if ($(this).val() == '-1') {
                $('.address_item').removeClass('ncc-selected-item');
                $('#add_addr_box').load(SITEURL+'/index.php?app=buy&wwi=add_chain');
        	} else {
                $('.address_item').removeClass('ncc-selected-item');
