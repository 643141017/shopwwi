...
<div class="ncm-flow-layout">
  <div class="ncm-flow-container">
    ...
    <form id="evalform" method="post" action="index.php?app=member_evaluate&wwi=<?php echo $_GET['wwi'];?>&order_id=<?php echo $_GET['order_id'];?>">
        ...
        <tbody>
          ...
          <tr class="bd-line">
            ...
            <td valign="top" class="tl"><div class="ncgeval">商品评分：
                <div class="raty vm">
                  <input nctype="score" name="goods[<?php if ($_GET['wwi'] != 'add_vr') { echo $goods['rec_id'];} else { echo $goods['goods_id'];}?>][score]" type="hidden">
                </div>
              </div>
              <textarea name="goods[<?php if ($_GET['wwi'] != 'add_vr') { echo $goods['rec_id'];} else { echo $goods['goods_id'];}?>][comment]" cols="150" class="w450 mt10 mb10 h40" placeholder="请输入要评价的内容，不要超过150个字符。"></textarea>
              <div class="show-pic">
                <?php if ($_GET['wwi'] != 'add_vr') {?>
                ...
                <input type="checkbox" class="checkbox vm" name="goods[<?php if ($_GET['wwi'] != 'add_vr') { echo $goods['rec_id'];} else { echo $goods['goods_id'];}?>][anony]">
      ...
      <?php if (!$output['store_info']['is_own_mall'] && $_GET['wwi'] != 'add_vr') { ?>
      <div class="tabmenu">
        <ul class="tab">
...
$(function(){
    ...
    <?php if(!empty($output['order_goods'])){?>
    <?php foreach($output['order_goods'] as $goods){?>
    <?php $param_id = $_GET['wwi'] != 'add_vr' ? $goods['rec_id'] : $goods['goods_id'];?>
    // 图片上传
