...
$(document).ready(function() {
    ...
    function change_seccode() {
        $('#codeimage').attr('src', 'index.php?app=seccode&wwi=makecode&nchash=<?php echo $output['nchash'];?>&t=' + Math.random());
        $('#captcha').select();
    }
    ...
    $("#form_login").validate({
        ...
        rules:{
            ...
            captcha:{
                required:true,
                remote:{
                    url:"index.php?app=seccode&wwi=check&nchash=<?php echo $output['nchash'];?>",
                    type:"get",
                    data:{
                        captcha:function() {
                            return $("#captcha").val();
...
<body>
<div id="loginBG01" class="wwist-login-bg">
  <p class="pngFix"></p>
</div>
<div id="loginBG02" class="wwist-login-bg">
  <p class="pngFix"></p>
</div>
<div class="wwist-login-container">
  <div class="wwist-login-title">
    <h2>商家管理中心</h2>
    <h4><a href="<?php echo urlChain('login');?>">门店管理登录</a></h4>
    <span>请使用入驻申请时填写的“商家用户名”作为登录用户名<br/>
    登录密码则商城用户密码一致</span></div>
  <form id="form_login" action="index.php?app=seller_login&wwi=login" method="post" >
    ...
    <div class="input">
      ...
      <div class="code">
        <div class="arrow"></div>
        <div class="code-img"><a href="javascript:void(0)" nctype="btn_change_seccode"><img src="index.php?app=seccode&wwi=makecode&nchash=<?php echo $output['nchash'];?>" name="codeimage" border="0" id="codeimage"></a></div>
