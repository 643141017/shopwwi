...
<form id="spec_form" method="post" action="<?php echo MALL_SITE_URL;?>/index.php?app=store_spec&wwi=save_spec">
  ...
  <div class="wwist-spec-info" style=" width: 960px;"><span>规格名称：<?php echo $output['sp_info']['sp_name'];?></span><span>所属分类：<?php echo $output['gc_info']['gc_name'];?></span><a href="javascript:void(0);" nctype="add_sv" class="ncbtn"><i class="icon-th-list"></i>添加规格值</a></div>
  <table class="wwist-default-table" style="width: 960px;">
    <thead>
      <tr>
        <th class="w80">排序</th>
...
$(function(){
    ...
    $('a[nctype="submit-btn"]').click(function(){
        ...
        if (_submit) {
            ajaxpost('spec_form', '<?php echo MALL_SITE_URL;?>/index.php?app=store_spec&wwi=save_spec', '', 'onerror');
        }
    });
    ...
    $('a[nctype="spec_del"]').click(function(){
        ...
        $.getJSON('index.php?app=store_spec&wwi=ajax_delspec&id='+data_str.id, function(data){
            if(data){
                $this.parents('tr:first').remove();
            }
        });
