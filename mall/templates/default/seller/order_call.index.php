...
<table class="wwist-default-table order" id="new_order" style="border-top:solid 1px #E6E6E6">
  <thead>
    <tr>
      <th class="w10"></th>
...
<span class="fr mr5"> <a href="index.php?app=store_order_print&order_id=<?php echo $order_id;?>" class="wwist-btn-mini" target="_blank" title="打印发货单"/><i class="icon-print"></i>A5打印</a></span>
<span class="fr mr5"> <a href="index.php?app=store_order_print&wwi=small&order_id=<?php echo $order_id;?>" class="wwist-btn-mini" target="_blank" title="打印发货单"/><i class="icon-print"></i>小票打印</a></span>
 </th>
    </tr>
    <?php $i = 0;?>
    ...
    <tr>
      <td class="bdl"></td>
      <td class="w70"><div class="wwist-goods-thumb"><a href="<?php echo $goods['goods_url'];?>" target="_blank"><img src="<?php echo $goods['image_60_url'];?>" onMouseOver="toolTip('<img src=<?php echo $goods['image_240_url'];?>>')" onMouseOut="toolTip()"/></a></div></td>
      ...
      <td class="bdl" rowspan="<?php echo $order['goods_count'];?>"><p class="wwist-order-amount"><?php echo $order['order_amount']; ?></p>
      ...
      <td class="bdl bdr" rowspan="<?php echo $order['goods_count'];?>"><p><?php echo $order['state_desc']; ?>
        ...
        <!-- 订单查看 -->
        <p><a href="index.php?app=store_order&wwi=show_order&order_id=<?php echo $order_id;?>" target="_blank"><?php echo $lang['store_order_view_order'];?></a></p>
        
        <!-- 物流跟踪 -->
        <p>
          <?php if ($order['if_deliver']) { ?>
          <a href='index.php?app=store_deliver&wwi=search_deliver&order_sn=<?php echo $order['order_sn']; ?>'><?php echo $lang['store_order_show_deliver'];?></a>
          <?php } ?>
        </p>
	...
	<td><?php if($order['is_print'] == 1){ echo "已打印"; }else{ echo "未打印";} ?></td>
      ...
      <td class="bdl bdr" rowspan="<?php echo $order['goods_count'];?>">
        ...
        <p><a href="javascript:void(0)" class="wwist-btn wwist-btn-red mt5" nc_type="dialog" uri="index.php?app=store_order&wwi=change_state&state_type=order_cancel&order_sn=<?php echo $order['order_sn']; ?>&order_id=<?php echo $order['order_id']; ?>" dialog_title="<?php echo $lang['store_order_cancel_order'];?>" dialog_id="seller_order_cancel_order" dialog_width="400" id="order<?php echo $order['order_id']; ?>_action_cancel" /><i class="icon-remove-tyq"></i><?php echo $lang['store_order_cancel_order'];?></a></p>
        <?php } ?>
        
        <!-- 修改运费 -->
        <?php if ($order['if_modify_price']) { ?>
        <p><a href="javascript:void(0)" class="wwist-btn-mini wwist-btn-orange mt10" uri="index.php?app=store_order&wwi=change_state&state_type=modify_price&order_sn=<?php echo $order['order_sn']; ?>&order_id=<?php echo $order['order_id']; ?>" dialog_width="480" dialog_title="<?php echo $lang['store_order_modify_price'];?>" nc_type="dialog"  dialog_id="seller_order_adjust_fee" id="order<?php echo $order['order_id']; ?>_action_adjust_fee" /><i class="icon-pencil"></i>修改运费</a></p>
        <?php }?>
        <!-- 修改价格 -->
		<?php if ($order['if_spay_price']) { ?>
        <p><a href="javascript:void(0)" class="wwist-btn-mini wwist-btn-green mt10" uri="index.php?app=store_order&wwi=change_state&state_type=spay_price&order_sn=<?php echo $order['order_sn']; ?>&order_id=<?php echo $order['order_id']; ?>" dialog_width="480" dialog_title="<?php echo $lang['store_order_modify_price'];?>" nc_type="dialog"  dialog_id="seller_order_adjust_fee" id="order<?php echo $order['order_id']; ?>_action_adjust_fee" /><i class="icon-pencil"></i>修改价格</a></p>
		<?php }?>
        
        <!-- 发货 -->
        <?php if ($order['if_send']) { ?>
        <p><a class="wwist-btn wwist-btn-green mt10" href="index.php?app=store_deliver&wwi=send&order_id=<?php echo $order['order_id']; ?>"/><i class="icon-truck"></i><?php echo $lang['store_order_send'];?></a></p>
        <?php } ?>
    ...
    <tr>
      <td class="bdl"></td>
      <td colspan="4" class="tl"><div class="wwist-goods-gift">赠品：
...
function refresh()
{
	$.ajax({
		type: "GET",
		url: MALL_SITE_URL+'/index.php?app=order_call&wwi=getajaxorder',
		dataType:"json",
		async: true,
		success: function(data){
		    ...
		    {	
				for(var i = 0; i < order_list.length; i++)
				{
					...
					html_str += '<tr><th colspan="20"><span class="ml10">订单编号：<em>'+order_list[i].order_sn+'</em></span> <span>下单时间：<em class="goods-time">'+order_list[i].add_time+'</em></span> <span class="fr mr5"> <a href="index.php?app=store_order_print&amp;order_id='+order_list[i].order_id+'" class="wwist-btn-mini" target="_blank" title="打印发货单"><i class="icon-print"></i>打印发货单</a></span></th></tr>';
					...
					for(var k = 0; k < order_list[i].goods_list.length; k++)
					{
						...
						html_str += '<td class="w70"><div class="wwist-goods-thumb"><a href="'+order_list[i].goods_list[k].goods_url+'" target="_blank"><img src="'+order_list[i].goods_list[k].image_60_url+'" onMouseOver="toolTip(\'<img src='+order_list[i].goods_list[k].image_240_url+'>\')" onMouseOut="toolTip()"/></a></div></td>';
						...
						if ((order_list[i].goods_count > 0 && k==0) || (order_list[i].goods_count) == 1 )
						{
							...
							html_str += '<td class="bdl" rowspan="'+order_list[i].goods_count+'"><p class="wwist-order-amount">'+order_list[i].order_amount+'</p>';
							...
							//订单查看
							html_str += '	<p><a href="index.php?app=store_order&wwi=show_order&order_id='+order_list[i].order_id+'" target="_blank">订单详情</a></p>';
								
							//物流跟踪
							html_str += '		<p>';
							if(order_list[i].if_deliver)
							{
								html_str += '<a href=\'index.php?app=store_deliver&wwi=search_deliver&order_sn='+order_list[i].order_sn+'\'>查看物流</a>';
							}
							...
							{
								html_str += '<p><a href="javascript:void(0)" class="wwist-btn wwist-btn-red mt5" nc_type="dialog" uri="index.php?app=store_order&wwi=change_state&state_type=order_cancel&order_sn='+order_list[i].order_sn+'&order_id='+order_list[i].order_id+'" dialog_title="取消订单" dialog_id="seller_order_cancel_order" dialog_width="400" id="order'+order_list[i].order_id+'_action_cancel" /><i class="icon-remove-tyq"></i>取消订单</a></p>';
							}
							...
							if(order_list[i].if_send){
								html_str += '<p>';
								html_str += '<a class="wwist-btn wwist-btn-green mt10" href="index.php?app=store_deliver&wwi=send&order_id='+order_list[i].order_id+'">';
					   ...
					   //E 合并TD
						...
						if(order_list[i].zengpin_list && j == order_list[i].goods_list.length)
						{
							html_str += '<tr>';
							html_str += '  <td class="bdl"></td>';
							html_str += '  <td colspan="4" class="tl"><div class="wwist-goods-gift">赠品：';
							html_str += '  <ul>';
...
{
    if(length > 0){
		var url = MALL_SITE_URL+'/index.php?app=order_call_print&wwi=index';
...
{
	if(length > 0){
		var url = MALL_SITE_URL+'/index.php?app=order_call_print&wwi=small';
	    window.open(url,'_blank');
	}
}
...
function select_type(obj)
{
	...
	$.ajax({
		type: "GET",
		url: MALL_SITE_URL+'/index.php?app=order_call&wwi=select_type&type='+type,
		dataType:"json",
		async: true,
		success: function(data){
			if(data == 1)
