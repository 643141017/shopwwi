...
<div class="wwist-account-container">
  <h4>
  ...
  <ul class="wwist-account-container-list">
...
$(document).ready(function(){
    ...
    $('input[deep_type="2"]').on('click',function(){
        if ($(this).attr('checked') == 'checked') {
            ...
            //父级选框是否也选中功能注释
//             tmp = $(this).parents('.wwist-account-container').find('input[deep_type="2"]');
...
//         	$('#'+$(this).attr('parent_id')).prop('checked',false);
	...
	$('body').on('click','#cate_3_submit', function(){
		...
		//父级选框是否也选中功能注释
//         tmp = $('#'+cur_edit_id).parents('.wwist-account-container').find('input[deep_type="2"]');
//         if (tmp.size() == tmp.filter(':checked').size()){
...
//         }
    ...
    $('[nctype="btn_select_module_class"]').on('click', function() {
        if($(this).prop('checked')) {
            $(this).parents('.wwist-account-container').find('input:checkbox').prop('checked', true);
            $(this).parents('.wwist-account-container').find('span[id^="count_"]').each(function(){
                $(this).html("("+$(this).attr('childcount')+")");
            });
        } else {
            $(this).parents('.wwist-account-container').find('input:checkbox').prop('checked', false);
            $(this).parents('.wwist-account-container').find('span[id^="count_"]').html('(0)');
            $('#gc_select_all').prop('checked',false);
        }
    });
});
