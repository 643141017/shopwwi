...
<form method="post" id="goods_combo" action="<?php echo urlMall('store_promotion_combo', 'save_combo');?>">
  ...
  <div class="wwist-form-goods-combo-default" data-gid="<?php echo $output['goods_info']['goods_id'];?>">
  ...
  <div nctype="wwist-goods-combo" class="wwist-goods-combo" id="ncscGoodsCombo">
    <?php if (!empty($output['combo_list'])) {$i = 0;?>
    <?php foreach ($output['combo_list'] as $key => $value) {?>
    <div class="wwist-form-goods-combo">
      <div class="combo-title">分类名称
        <input type="text" name="class[s<?php echo $i;?>]" value="<?php echo $value['name'];?>" >
        <a href="javascript:void(0);" nctype="wwist-goods-combo-del" class="ncbtn ncbtn-grapefruit"><i class="icon-trash"></i>删除</a>
      </div>
      <div class="combo-goods" nctype="choose_goods_list">
        <ul>
          ...
          <li>
            ...
            <a class="ncbtn-mini ncbtn-bittersweet" nctype="del_choosed" href="javascript:void(0);"><i class="icon-ban-tyq"></i>取消推荐</a></li>
          <?php }?>
          <?php }?>
        </ul>
      </div>
...
$(function(){	
    ...
    $('a[nctype="select_goods"]').click(function(){
        $('div[nctype="wwist-goods-combo"]').append('<div class="wwist-form-goods-combo"><div class="combo-title">分类名称<input type="text" name="class['+ _i +']" value="默认' + _i + '" ><a href="javascript:void(0);" nctype="wwist-goods-combo-del" class="ncbtn ncbtn-grapefruit"><i class="icon-trash"></i>删除</a></div><div class="combo-goods" nctype="choose_goods_list" data-i="' + _i + '"><ul></ul></div></div>');
        selected_box($('.wwist-form-goods-combo', '#goods_combo').last());
        _i++;
    });
    ...
    // 删除按钮
    $('div[nctype="wwist-goods-combo"]').on('click', 'a[nctype="wwist-goods-combo-del"]', function(){
        $(this).parents('.wwist-form-goods-combo:first').remove();
    });
    ...
    $('#goods_combo')
    // 选择分类组
    .on('click', '.wwist-form-goods-combo', function(){
        selected_box($(this));
    })
    // 删除推荐商品
    ...
    $('div[nctype="combo_goods_list"]').on('click', 'a[nctype="a_choose_goods"]', function(){
        ...
        _li = $('<li></li>')
            ...
            .append('<a class="ncbtn-mini ncbtn-bittersweet" nctype="del_choosed" href="javascript:void(0);"><i class="icon-ban-tyq"></i>取消推荐</a>');
...
function selected_box($this) {
	$('.wwist-form-goods-combo', '#goods_combo').removeClass('selected');
