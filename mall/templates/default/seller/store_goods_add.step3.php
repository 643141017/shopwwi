...
<form method="post" id="goods_image" action="<?php if ($output['edit_goods_sign']) { echo urlMall('store_goods_online', 'edit_save_image'); } else { echo urlMall('store_goods_add', 'save_image');}?>">
  ...
  <div class="wwist-form-goods-pic">
    <div class="container">
      <?php foreach ($output['value_array'] as $value) {?>
      <div class="wwist-goodspic-list">
        <div class="title">
        ...
        <ul class="goods-pic-list" nctype="ul<?php echo $value['sp_value_id'];?>">
          <?php for ($i = 0; $i < 5; $i++) {?>
          <li class="wwist-goodspic-upload">
            ...
            <div class="wwist-upload-btn"><a href="javascript:void(0);"><span><input type="file" hidefocus="true" size="1" class="input-file" name="file_<?php echo $value['sp_value_id'] . $i;?>" id="file_<?php echo $value['sp_value_id'] . $i;?>"></span><p><i class="icon-upload-alt"></i>上传</p>
              </a></div>
            
          </li>
          <?php }?>
        </ul>
        <div class="wwist-select-album">
          <a class="ncbtn" href="index.php?app=store_album&wwi=pic_list&item=goods_image&color_id=<?php echo $value['sp_value_id'];?>" nctype="select-<?php echo $value['sp_value_id'];?>"><i class="icon-picture"></i>从图片空间选择</a>
          <a href="javascript:void(0);" nctype="close_album" class="ncbtn ml5" style="display: none;"><i class=" icon-tyq-arrow-up"></i>关闭相册</a>
        </div>
