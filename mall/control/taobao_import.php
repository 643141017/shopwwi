 ...
 *
 *
 **你正在使用的程序由网店运维技术交流中心www.shopwwi.com提供！这里有专业化shopwwi技术指导！以便你更好的了解这个程序*/
		   ...
		   if (strtoupper(CHARSET) == 'GBK'){
			...
			if(intval($store_grade['sg_goods_limit']) != 0) {
				if($goods_num >= $store_grade['sg_goods_limit']) {
					showMessage($lang['store_goods_index_goods_limit'].$store_grade['sg_goods_limit'].$lang['store_goods_index_goods_limit1'],'index.php?app=store_goods&wwi=goods_list','html','error');
				}
			...
			if(intval($store_info['store_end_time']) != 0) {
				if(time() >= $store_info['store_end_time']) {
					showMessage($lang['store_goods_index_time_limit'],'index.php?app=store_goods&wwi=goods_list','html','error');
				}
			}
			/**
			 * 循环添加数据
			...
			if(is_array($records) and count($records) > 0){
				foreach($records as $k=>$record){
					if($remain_num>0 and $k>=$remain_num){
						showMessage($lang['store_goods_index_goods_limit'].$store_grade['sg_goods_limit'].$lang['store_goods_index_goods_limit1'].$lang['store_goods_import_end'].(count($records)-$remain_num).$lang['store_goods_import_products_no_import'],'index.php?app=store_goods&wwi=taobao_import&step=4','html','error');
					}
