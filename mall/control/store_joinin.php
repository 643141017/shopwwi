...
class store_joininControl extends BaseHomeControl {
    ...
    public function __construct() {
        ...
        if($_GET['wwi'] != 'check_seller_name_exist' && $_GET['wwi'] != 'checkname') {
        ...
        if(!empty($joinin_detail)) {
            ...
            switch (intval($joinin_detail['joinin_state'])) {
                ...
                case STORE_JOIN_STATE_VERIFY_SUCCESS:
                    if(!in_array($_GET['wwi'], array('pay', 'pay_save'))) {
                        $this->payWwi();
                    }
                    break;
                case STORE_JOIN_STATE_VERIFY_FAIL:
                    if(!in_array($_GET['wwi'], array('step1', 'step2', 'step3', 'step4','ajax_upload_image'))) {
                        $this->show_join_message('审核失败:'.$joinin_detail['joinin_message'], MALL_SITE_URL.DS.'index.php?app=store_joinin&wwi=step1');
                    }
                    break;
                case STORE_JOIN_STATE_PAY_FAIL:
                    if(!in_array($_GET['wwi'], array('pay', 'pay_save'))) {
                        $this->show_join_message('付款审核失败:'.$joinin_detail['joinin_message'], MALL_SITE_URL.DS.'index.php?app=store_joinin&wwi=pay');
                    }
                    break;
    ...
    public function check_seller_name_existWwi() {
        $condition = array();
    ...
    public function ajax_upload_imageWwi() {
