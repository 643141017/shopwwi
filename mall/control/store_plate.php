...
class store_plateControl extends BaseSellerControl {
    ...
    public function plate_addWwi() {
        if (chksubmit()) {
            ...
            if ($result) {
                showDialog(L('nc_common_op_succ'), urlMall('store_plate', 'index'),'succ');
            } else {
                showDialog(L('nc_common_op_fail'), urlMall('store_plate', 'index'));
            }
        }
        // 是否能使用编辑器
