...
class store_deliverControl extends BaseSellerControl {
     ...
     * @return boolean
     */
    public function buyer_address_editWwi() {
        $order_id = intval($_GET['order_id']);
     ...
     * 收货地址保存
     */
    public function buyer_address_saveWwi() {
        $model_order = Model('order');
        $data = array();
     ...
     * @return boolean
     */
    public function send_address_selectWwi() {
     ...
     * 保存发货地址修改
     */
    public function send_address_saveWwi() {
     ...
     * 物流跟踪
     */
    public function search_deliverWwi(){
     ...
     * 延迟收货
     */
    public function delay_receiveWwi(){
        $order_id = intval($_GET['order_id']);
     ...
     * 运单打印
     */
    public function waybill_printWwi() {
        $order_id = intval($_GET['order_id']);
        if($order_id <= 0) {
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'deliver':
                $menu_array = array(
                array('menu_key'=>'deliverno',          'menu_name'=>Language::get('nc_member_path_deliverno'), 'menu_url'=>'index.php?app=store_deliver&wwi=index&state=deliverno'),
                array('menu_key'=>'delivering',         'menu_name'=>Language::get('nc_member_path_delivering'),    'menu_url'=>'index.php?app=store_deliver&wwi=index&state=delivering'),
                array('menu_key'=>'delivered',      'menu_name'=>Language::get('nc_member_path_delivered'), 'menu_url'=>'index.php?app=store_deliver&wwi=index&state=delivered'),
                );
                break;
            case 'search':
                $menu_array = array(
                1=>array('menu_key'=>'nodeliver',           'menu_name'=>Language::get('nc_member_path_deliverno'), 'menu_url'=>'index.php?app=store_deliver&wwi=index&state=nodeliver'),
                2=>array('menu_key'=>'delivering',          'menu_name'=>Language::get('nc_member_path_delivering'),    'menu_url'=>'index.php?app=store_deliver&wwi=index&state=delivering'),
                3=>array('menu_key'=>'delivered',       'menu_name'=>Language::get('nc_member_path_delivered'), 'menu_url'=>'index.php?app=store_deliver&wwi=index&state=delivered'),
