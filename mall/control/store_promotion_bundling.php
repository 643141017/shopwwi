...
class store_promotion_bundlingControl extends BaseSellerControl {
    ...
    public function indexWwi() {
        $this->bundling_listWwi();
    }

    /**
     * 发布的活动列表
     */
    public function bundling_listWwi() {
     ...
     * 套餐购买
     */
    public function bundling_quota_addWwi() {
        if (chksubmit()) {
     ...
     * 套餐续费
     */
    public function bundling_renewWwi() {
        if (chksubmit()) {
            $model_bundling = Model('p_bundling');
            $quantity = intval($_POST['bundling_quota_quantity']); // 购买数量（月）
            $price_quantity = $quantity * intval(C('promotion_bundling_price')); // 扣款数
     ...
     * 套餐活动添加
     */
    public function bundling_addWwi() {
        /**
         * 实例化模型
         */
        $model_bundling = Model('p_bundling');
        ...
        if (chksubmit()) {
            ...
            showDialog(L('nc_common_op_succ'), urlMall('store_promotion_bundling', 'bundling_list'), 'succ');
        }
     ...
     * 套餐活动添加商品
     */
    public function bundling_add_goodsWwi() {
        /**
         * 实例化模型
         */
        $model_goods =Model('goods');
     ...
     * 删除优惠套装活动
     */
    public function drop_bundlingWwi(){
        /**
         * 参数验证
         */
