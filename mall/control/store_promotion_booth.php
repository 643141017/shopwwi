...
class store_promotion_boothControl extends BaseSellerControl {
    ...
    public function indexWwi() {
        $this->booth_goods_listWwi();
    }

    /**
     * 套餐商品列表
     */
    public function booth_goods_listWwi() {
     ...
     * 选择商品
     */
    public function booth_select_goodsWwi() {
     ...
     * 购买套餐
     */
    public function booth_quota_addWwi() {
        if (chksubmit()) {
     ...
     * 套餐续费
     */
    public function booth_renewWwi() {
        if (chksubmit()) {
            $model_booth = Model('p_booth');
            $quantity = intval($_POST['booth_quota_quantity']); // 购买数量（月）
            $price_quantity = $quantity * intval(C('promotion_booth_price')); // 扣款数
