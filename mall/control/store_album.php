...
//      Tpl::output('show_page',$page->show());
     ...
     *
     */
    public function album_add_saveWwi(){
        if (chksubmit()){
            /**
             * 实例化相册模型
             */
     ...
     * 相册分类编辑保存
     */
    public function album_edit_saveWwi(){
        $param = array();
        $param['aclass_name']   = $_POST['name'];
        $param['aclass_des']    = $_POST['description'];
        $param['aclass_sort']   = intval($_POST['sort']);
     ...
     * 图片列表
     */
    public function album_pic_listWwi(){
        if(empty($_GET['id'])) {
            showMessage(Language::get('album_parameter_error'),'','html','error');
        }
     ...
     * 修改相册封面
     */
    public function change_album_coverWwi(){
        if(empty($_GET['id'])) {
     ...
     * ajax修改图名称
     */
    public function change_pic_nameWwi(){
     ...
     * 图片详细页
     */
    public function album_pic_infoWwi(){
     ...
     * 图片 ajax
     */
    public function album_ad_ajaxWwi(){
        if(empty($_GET['class_id']) && empty($_GET['id'])){
            exit();
        }
     ...
     * 图片删除
     */
    public function album_pic_delWwi(){
     ...
     * 移动相册
     */
    public function album_pic_moveWwi(){
        /**
         * 实例化相册类
         */
        $model_album = Model('album');
     ...
     * 替换图片
     */
    public function replace_image_uploadWwi() {
        $file = $_GET['id'];
	 ...
	 * 添加水印
	 */
	public function album_pic_watermarkWwi(){
		if(empty($_POST['id']) && !is_array($_POST['id'])) {
			showDialog(Language::get('album_parameter_error'));
		}
		...
		if ($store_wm_info['wm_image_name'] == '' && $store_wm_info['wm_text'] == ''){
			showDialog(Language::get('album_class_setting_wm'),"index.php?app=store_album&wwi=store_watermark",'error','CUR_DIALOG.close();');//"请先设置水印"
		}
     ...
     * 水印管理
     */
    public function store_watermarkWwi(){
        /**
         * 读取语言包
         */
    ...
    private function profile_menu($menu_type,$menu_key=''){
        ...
        switch ($menu_type) {
            case 'album':
                $menu_array = array(
                1=>array('menu_key'=>'album','menu_name'=>Language::get('nc_member_path_my_album'),'menu_url'=>'index.php?app=store_album'),
                2=>array('menu_key'=>'watermark','menu_name'=>Language::get('nc_member_path_watermark'),'menu_url'=>'index.php?app=store_album&wwi=store_watermark')
                );
                break;
            case 'album_pic':
                $menu_array = array(
                1=>array('menu_key'=>'album','menu_name'=>Language::get('nc_member_path_my_album'),'menu_url'=>'index.php?app=store_album'),
                3=>array('menu_key'=>'pic_list','menu_name'=>Language::get('nc_member_path_album_pic_list'),'menu_url'=>'index.php?app=store_album&wwi=album_pic_list&id='.intval($_GET['id'])),
                2=>array('menu_key'=>'watermark','menu_name'=>Language::get('nc_member_path_watermark'),'menu_url'=>'index.php?app=store_album&wwi=store_watermark')
                );
                break;
            case 'album_pic_info':
                $menu_array = array(
                1=>array('menu_key'=>'album','menu_name'=>Language::get('nc_member_path_my_album'),'menu_url'=>'index.php?app=store_album'),
                3=>array('menu_key'=>'pic_info','menu_name'=>Language::get('nc_member_path_album_pic_info'),'menu_url'=>'index.php?app=store_album&wwi=album_pic_info&id='.intval($_GET['id']).'&class_id='.intval($_GET['class_id'])),
                2=>array('menu_key'=>'watermark','menu_name'=>Language::get('nc_member_path_watermark'),'menu_url'=>'index.php?app=store_album&wwi=store_watermark')
                );
                break;
        }
     ...
     * ajax返回图片信息
     */
    public function ajax_change_imgmessageWwi(){
     ...
     * ajax验证名称时候重复
     */
    public function ajax_check_class_nameWwi(){
