...
class store_infoControl extends BaseSellerControl {
     ...
     * 申请新的经营类目
     */
    public function bind_class_addWwi() {
     ...
     * 申请新经营类目保存
     */
    public function bind_class_saveWwi() {
        if (!chksubmit()) exit();
        ...
        if($result) {
            showDialog('申请成功，请等待系统审核','index.php?app=store_info&wwi=bind_class','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
        }else {
            showDialog($lang['nc_common_save_fail']);
        }
    }
     ...
     * 删除申请的经营类目
     */
    public function bind_class_delWwi() {
...
//              $model_store->editStore(array('store_end_time'=>$data['re_end_time']),array('store_id'=>$_SESSION['store_id']));
    ...
    //上传付款凭证
    public function reopen_uploadWwi() {
        if (!chksubmit()) exit();
    ...
    private function profile_menu($menu_type,$menu_key='',$array=array()) {
        ...
        switch ($menu_type) {
            case 'index':
                $menu_array[] = array('menu_key'=>'bind_class', 'menu_name'=>$lang['nc_member_path_bind_class'], 'menu_url'=>'index.php?app=store_info&wwi=bind_class');
                if (!checkPlatformStore()) {
                    $menu_array[] = array('menu_key'=>'index', 'menu_name'=>$lang['nc_member_path_store_info'], 'menu_url'=>'index.php?app=store_info&wwi=index');
                    $menu_array[] = array('menu_key'=>'reopen', 'menu_name'=>$lang['nc_member_path_store_reopen'], 'menu_url'=>'index.php?app=store_info&wwi=reopen');
                }
                break;
            case 'bind_class':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>$lang['nc_member_path_bind_class'], 'menu_url'=>'index.php?app=store_bind_class&wwi=index'),
                );
                break;
            case 'add':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>$lang['nc_member_path_bind_class'], 'menu_url'=>'index.php?app=store_bind_class&wwi=index'),
                array('menu_key'=>'add', 'menu_name'=>$lang['nc_member_path_bind_class_add'], 'menu_url'=>'index.php?app=store_bind_class&wwi=add')
                );
                break;
        }
        if(!empty($array)) {
