...
class paymentControl extends BaseHomeControl{
    ...
    public function real_orderWwi(){
        ...
        if (empty($api_pay_amount)) {
            redirect(MALL_SITE_URL.'/index.php?app=buy&wwi=pay_ok&pay_sn='.$order_pay_info['data']['pay_sn'].'&pay_amount='.ncPriceFormat($pay_amount));
        }
    ...
    public function vr_orderWwi(){
        ...
        //如果所需支付金额为0，转到支付成功页
        if (empty($api_pay_amount)) {
            redirect('index.php?app=buy_virtual&wwi=pay_ok&order_sn='.$order_info['order_sn'].'&order_id='.$order_info['order_id'].'&order_amount='.ncPriceFormat($order_info['order_amount']));
        }
    ...
    public function returnWwi(){
        ...
        //支付成功后跳转
        if ($order_type == 'real_order') {
            $pay_ok_url = MALL_SITE_URL.'/index.php?app=buy&wwi=pay_ok&pay_sn='.$out_trade_no.'&pay_amount='.ncPriceFormat($api_pay_amount);
        } elseif ($order_type == 'vr_order') {
            $pay_ok_url = MALL_SITE_URL.'/index.php?app=buy_virtual&wwi=pay_ok&order_sn='.$out_trade_no.'&order_id='.$order_pay_info['order_id'].'&order_amount='.ncPriceFormat($api_pay_amount);
        } elseif ($order_type == 'pd_order') {
     ...
     */
    public function wxpay_returnWwi() {
     ...
     * 支付成功，更新订单状态
     */
    public function wxpay_notifyWwi() {
        $result = Logic('payment')->getPaymentInfo('wxpay');
        if (!$result['state']) {
            Log::record('wxpay not found','RUN');
        }
    ...
    public function query_stateWwi() {
