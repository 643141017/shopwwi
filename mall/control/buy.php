...
class buyControl extends BaseBuyControl {
    ...
    public function buy_step2Wwi() {
        ...
        //转向到商城支付页面
        redirect('index.php?app=buy&wwi=pay&pay_sn='.$result['data']['pay_sn']);
    }

    /**
     * 下单时支付页面
     */
    public function payWwi() {
        $pay_sn = $_GET['pay_sn'];
        ...
        if (empty($pay['pay_amount_online']) && empty($pay['pay_amount_offline'])) {
            redirect('index.php?app=buy&wwi=pay_ok&pay_sn='.$pay_sn.'&is_chain='.$flag_chain.'&pay_amount='.ncPriceFormat($pay_amount));
        }
     ...
     */
    public function change_addrWwi() {
        $logic_buy = Logic('buy');
    ...
    //根据门店自提站ID计算商品库存
    public function change_chainWwi() {
        $logic_buy = Logic('buy');
     ...
     * AJAX验证支付密码
     */
    public function check_pd_pwdWwi(){
     ...
     * F码验证
     */
    public function check_fcodeWwi() {
    ...
    private function _buy_branch($post) {
        if (!$post['ifcart']) {
            ...
            if ($goods_info['is_virtual']) {
                redirect('index.php?app=buy_virtual&wwi=buy_step1&goods_id='.$goods_id.'&quantity='.$quantity);
            }
        }
    }
