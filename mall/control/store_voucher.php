...
class store_voucherControl extends BaseSellerControl{
     ...
     * 代金券模版列表
     */
    public function voucherlistWwi(){
     ...
     * 导出
     */
    public function voucher_exportWwi(){
        $t_id = intval($_GET['tid']);
        if($t_id <= 0){
            showDialog('参数错误');
        }
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){//显示下载链接
                ...
                Tpl::output('murl','index.php?app=store_voucher&wwi=voucher_export');
    ...
    public function quotaaddWwi(){
        if (chksubmit()){
            ...
            if($reault){
                showDialog(Language::get('voucher_apply_buy_succ'),'index.php?app=store_voucher&wwi=quotalist','succ');
            } else {
                showDialog(Language::get('nc_common_op_fail'),'index.php?app=store_voucher&wwi=quotalist');
            }
        }else {
            //输出导航
     ...
     * 代金券模版添加
     */
    public function templateaddWwi(){
        $model = Model('voucher');
        if ($isOwnMall = checkPlatformStore()) {
            ...
            if(empty($quotainfo)){
                showMessage(Language::get('voucher_template_quotanull'),'index.php?app=store_voucher&wwi=quotaadd','html','error');
            }
            ...
            if ($count >= C('promotion_voucher_storetimes_limit')){
                ...
                showMessage($message,'index.php?app=store_voucher&wwi=templatelist','html','error');
            }
        }
        ...
        if(empty($pricelist)){
            showMessage(Language::get('voucher_template_pricelisterror'),'index.php?app=store_voucher&wwi=templatelist','html','error');
        }
        //获取领取方式
        ...
        if(chksubmit()){
            ...
            if ($error){
                ...
                if($rs){
                    ...
                    showDialog(Language::get('nc_common_save_succ'),'index.php?app=store_voucher&wwi=templatelist','succ');
                }else{
                    showDialog(Language::get('nc_common_save_fail'),'index.php?app=store_voucher&wwi=templatelist','error');
                }
            }
        }else{
            //店铺分类
    ...
    public function templateeditWwi(){
        ...
        if ($t_id <= 0){
            showMessage(Language::get('wrong_argument'),'index.php?app=store_voucher&wwi=templatelist','html','error');
        }
        $model = Model('voucher');
        //查询模板信息
        $param = array();
        ...
        if(chksubmit()){
            ...
            if ($error){
                ...
                if($rs){
                    ...
                    showDialog(Language::get('nc_common_op_succ'),'index.php?app=store_voucher&wwi=templatelist','succ');
                }else{
                    showDialog(Language::get('nc_common_op_fail'),'index.php?app=store_voucher&wwi=templatelist','error');
                }
            }
        }else{
     ...
     * 删除代金券
     */
    public function templatedelWwi(){
        $t_id = intval($_GET['tid']);
        if ($t_id <= 0){
            showMessage(Language::get('wrong_argument'),'index.php?app=store_voucher&wwi=templatelist','html','error');
        }
     ...
     * 查看代金券详细
     */
    public function templateinfoWwi(){
        $t_id = intval($_GET['tid']);
        if ($t_id <= 0){
            showMessage(Language::get('wrong_argument'),'index.php?app=store_voucher&wwi=templatelist','html','error');
        }
     ...
     * 生成卡密代金券
     */
    public function bulidvoucherWwi(){
        $t_id = intval($_GET['tid']);
    	if ($t_id <= 0){
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'voucher':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('nc_member_path_store_voucher'), 'menu_url'=>'index.php?app=store_voucher&wwi=templatelist'),
                );
                break;
            case 'quota_add':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('nc_member_path_store_voucher'), 'menu_url'=>'index.php?app=store_voucher&wwi=templatelist'),
                4=>array('menu_key'=>'quotaadd','menu_name'=>Language::get('voucher_applyadd'), 'menu_url'=>'index.php?app=store_voucher&wwi=quotaadd')
                );
                break;
            case 'template':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('nc_member_path_store_voucher'), 'menu_url'=>'index.php?app=store_voucher&wwi=templatelist'),
                2=>array('menu_key'=>'templateadd','menu_name'=>Language::get('voucher_templateadd'),   'menu_url'=>'index.php?app=store_voucher&wwi=templateadd'),
                );
                break;
            case 'templateedit':
                $menu_array = array(
