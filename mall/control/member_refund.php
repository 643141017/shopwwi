...
class member_refundControl extends BaseMemberControl {
    ...
    public function add_refundWwi(){
        ...
        if ($order_id < 1 || $goods_id < 1) {//参数验证
            showDialog(Language::get('wrong_argument'),'index.php?app=member_order&wwi=index','error');
        }
        $condition = array();
        ...
        if (chksubmit() && $goods_id > 0){
            ...
            $show_url = 'index.php?app=member_return&wwi=index';
            ...
            if ($refund_array['refund_type'] != '2') {
                ...
                $show_url = 'index.php?app=member_refund&wwi=index';
            }
     ...
     *
     */
    public function add_refund_allWwi(){
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                $model_refund->editOrderLock($order_id);
                showDialog(Language::get('nc_common_save_succ'),'index.php?app=member_refund&wwi=index','succ');
            } else {
                showDialog(Language::get('nc_common_save_fail'),'reload','error');
            }
        }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'member_order':
                $menu_array = array(
                array('menu_key'=>'buyer_refund','menu_name'=>Language::get('nc_member_path_buyer_refund'), 'menu_url'=>'index.php?app=member_refund'),
                array('menu_key'=>'buyer_return','menu_name'=>Language::get('nc_member_path_buyer_return'), 'menu_url'=>'index.php?app=member_return'),
