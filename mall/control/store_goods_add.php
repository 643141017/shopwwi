...
class store_goods_addControl extends BaseSellerControl {
    ...
    private function checkStore(){
        ...
        if ($goodsLimit > 0) {
            ...
            if ($goods_num >= $goodsLimit) {
                showMessage(L('store_goods_index_goods_limit') . $goodsLimit . L('store_goods_index_goods_limit1'), 'index.php?app=store_goods_online&wwi=goods_list', 'html', 'error');
            }
        }
    }
    ...
    public function indexWwi() {
        $this->checkStore();
        $this->add_step_oneWwi();
    }

    /**
     * 添加商品
     */
    public function add_step_oneWwi() {
        // 实例化商品分类模型
     ...
     * 添加商品
     */
    public function add_step_twoWwi() {
        // 实例化商品分类模型
        $model_goodsclass = Model('goods_class');
     ...
     * 第三步添加颜色图片
     */
    public function add_step_threeWwi() {
     ...
     * 商品发布第四步
     */
    public function add_step_fourWwi() {
        // 单条商品信息
     ...
     * ajax获取商品分类的子级数据
     */
    public function ajax_goods_classWwi() {
     ...
     * ajax删除常用分类
     */
    public function ajax_stapledelWwi() {
        Language::read ( 'member_store_goods_index' );
     ...
     * ajax选择常用商品分类
     */
    public function ajax_show_commWwi() {
     ...
     * AJAX添加商品规格值
     */
    public function ajax_add_specWwi() {
        $name = trim($_GET['name']);
        $gc_id = intval($_GET['gc_id']);
     ...
     * AJAX查询品牌
     */
    public function ajax_get_brandWwi() {
