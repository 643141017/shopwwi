...
class store_goods_onlineControl extends BaseSellerControl {
     ...
     * 编辑商品保存
     */
    public function edit_save_goodsWwi() {
     ...
     * 保存商品图片
     */
    public function edit_save_imageWwi() {
        if (chksubmit()) {
            ...
            if ($rs['state']) {
                ...
                showDialog(L('nc_common_save_fail'), urlMall('store_goods_online', 'index'));
            }
        }
    }
     ...
     * 商品下架
     */
    public function goods_unshowWwi() {
        $common_id = $this->checkRequestCommonId($_GET['commonid']);
        $commonid_array = explode(',', $common_id);
        $result = Logic('goods')->goodsUnShow($commonid_array, $this->store_info['store_id'], $_SESSION['seller_id'], $_SESSION['seller_name']);
        if ($result['state']) {
            showDialog(L('store_goods_index_goods_unshow_success'), getReferer() ? getReferer() : 'index.php?app=store_goods_online&wwi=goods_list', 'succ', '', 2);
        } else {
     ...
     * 设置广告词
     */
    public function edit_jingleWwi() {
        if (chksubmit()) {
   ...
   * 设置商品佣金
     */
    public function edit_invite_priceWwi() {
		$model_goods = Model('goods');
        if (chksubmit()) {
			$where = array();
     ...
     * ajax获取商品列表
     */
    public function get_goods_list_ajaxWwi() {
        $common_id = $_GET['commonid'];
        if ($common_id <= 0) {
            echo 'false';exit();
        }
    ...
    public function edit_iframe_ajaxWwi() {

        Tpl::showpage('store_goods_list.edit_iframe', 'null_layout');
    }
    
    public function edit_body_ajaxWwi() {
        $model_goods = Model('goods');
        if (chksubmit()) {
            $where = array();
     ...
     * @return
     */
	public function maker_qrcodeWwi()
	{
