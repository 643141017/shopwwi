...
class pointredpacketControl extends BasePointMallControl {
    ...
    public function indexWwi(){
        $this->pointredpacketWwi();
    }
    /**
     * 红包列表
     */
    public function pointredpacketWwi(){
        //查询会员及其附属信息
     ...
     * 兑换红包
     */
    public function rptexchangeWwi(){
        $tid = intval($_GET['tid']);
        if($tid <= 0){
        ...
        if($_SESSION['is_login'] != '1'){
            ...
            $js = "CUR_DIALOG = ajax_form('rptexchange', '您要兑换的红包', 'index.php?app=pointredpacket&wwi=rptexchange&tid={$tid}', 550);";
            showDialog('','','js',$js);
            die;
        }
        $result = true;
     ...
     * 兑换红包保存信息
     */
    public function rptexchange_saveWwi(){
        if($_SESSION['is_login'] != '1'){
            $js = "login_dialog();";
            showDialog('','','js',$js);
        }
