...
class store_promotion_fcodeControl extends BaseSellerControl {
     ...
     * 购买套餐
     */
    public function fcode_quota_addWwi() {
        if (chksubmit()) {
     ...
     * 套餐续费
     */
    public function fcode_renewWwi() {
        if (chksubmit()) {
            $model_fcode = Model('p_fcode');
            $quantity = intval($_POST['fcode_quota_quantity']); // 购买数量（月）
            $price_quantity = $quantity * intval(C('promotion_fcode_price')); // 扣款数
            ...
            if ($return) {
                ...
                showDialog('购买成功', urlMall('store_promotion_fcode', 'fcode_list'), 'succ');
            } else {
                showDialog('购买失败', urlMall('store_promotion_fcode', 'fcode_quota_add'));
            }
        }
     ...
     * 选择商品
     */
    public function fcode_select_goodsWwi() {
     ...
     * 选择商品
     */
    public function choosed_goodsWwi() {
        $model_fcode = Model('p_fcode');
        if (!checkPlatformStore()) {
            // 验证套餐时候过期
     ...
     * 删除选择商品
     */
    public function del_choosed_goodsWwi() {
        $gid = intval($_GET['gid']);
        if ($gid <= 0) {
