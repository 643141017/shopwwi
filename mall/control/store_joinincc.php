        ...
        if(!empty($joinin_detail)) {
            ...
            switch (intval($joinin_detail['joinin_state'])) {
                ...
                case STORE_JOIN_STATE_VERIFY_FAIL:
                    if(!in_array($_GET['wwi'], array('step1', 'step2', 'step3', 'step4'))) {
                        $this->show_join_message('审核失败:'.$joinin_detail['joinin_message'], MALL_SITE_URL.DS.'index.php?app=store_joinincc&wwi=step1');
                    }
                    break;
                case STORE_JOIN_STATE_PAY_FAIL:
                    if(!in_array($_GET['wwi'], array('pay', 'pay_save'))) {
                        $this->show_join_message('付款审核失败:'.$joinin_detail['joinin_message'], MALL_SITE_URL.DS.'index.php?app=store_joinincc&wwi=pay');
                    }
                    break;
