...
class pointcartControl extends BasePointMallControl {
    ...
    public function addWwi() {
        ...
        if (!$data['state']){
            switch ($data['error']){
                case 'ParameterError':
                    echo json_encode(array('done'=>false,'msg'=>$data['msg'],'url'=>'index.php?app=pointprod&wwi=plist')); die;
                    break;
                default:
    ...
    public function step2Wwi() {
        ...
        if (!$data['state']){
            showDialog($data['msg'],'index.php?app=pointcart&wwi=step1','error');
        }
        $order_id = $data['data']['order_id'];
        @header("Location:index.php?app=pointcart&wwi=step3&order_id=".$order_id);
    }
    /**
     * 流程第三步
     */
