...
class sns_albumControl extends BaseSNSControl {
     ...
     * 图片详细页
     */
    public function album_pic_scroll_ajaxWwi(){
        $class_id = intval($_GET['class_id']); $id = intval($_GET['id']);
        if($class_id <= 0 && $id <= 0){
            exit();
        }
    ...
    private function profile_menu($menu_type,$menu_key=''){
        ...
        switch ($menu_type) {
            ...
            case 'album_pic':
                $menu_array = array(
                    1=>array('menu_key'=>'album','menu_name'=>$album_menuname,'menu_url'=>'index.php?app=sns_album&mid='.$this->master_id),
                    2=>array('menu_key'=>'pic_list','menu_name'=>Language::get('nc_sns_path_album_pic_list'),'menu_url'=>'index.php?app=sns_album&wwi=album_pic_list&id='.intval($_GET['id']).'&mid='.$this->master_id),
                );
                break;
            case 'album_pic_info':
                $menu_array = array(
                    1=>array('menu_key'=>'album','menu_name'=>$album_menuname,'menu_url'=>'index.php?app=sns_album&mid='.$this->master_id),
                    2=>array('menu_key'=>'pic_info','menu_name'=>Language::get('nc_sns_path_album_pic_info'),'menu_url'=>'index.php?app=sns_album&wwi=album_pic_info&id='.intval($_GET['id']).'&class_id='.intval($_GET['class_id']).'$mid='.$this->master_id),
                );
                break;
        }
