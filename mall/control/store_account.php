...
class store_accountControl extends BaseSellerControl {
    ...
    public function account_listWwi() {
    ...
    public function account_addWwi() {
        $model_seller_group = Model('seller_group');
        $seller_group_list = $model_seller_group->getSellerGroupList(array('store_id' => $_SESSION['store_id']));
    ...
    public function account_editWwi() {
    ...
    public function account_saveWwi() {
        ...
        if($result) {
            ...
            showDialog(Language::get('nc_common_op_succ'), urlMall('store_account', 'account_list'), 'succ');
        } else {
            $this->recordSellerLog('添加账号失败');
            showDialog(Language::get('nc_common_save_fail'), urlMall('store_account', 'account_list'), 'error');
        }
    }

    public function account_edit_saveWwi() {
        // 客户端登陆选项判断
        $is_client = 0;
        if(intval($_POST['is_client']) > 0) {
            $is_client = 1;
    ...
    public function account_delWwi() {
        $seller_id = intval($_POST['seller_id']);
        if($seller_id > 0) {
            $condition = array();
    ...
    public function check_seller_memberWwi() {
