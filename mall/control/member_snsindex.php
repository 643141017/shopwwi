...
class member_snsindexControl extends BaseMemberControl {
    ...
    public function indexWwi(){
        ...
        //信息输出
        Tpl::output('nchash',substr(md5(MALL_SITE_URL.$_GET['app'].$_GET['wwi']),0,8));
    ...
    public function addtraceWwi(){
        ...
        if ($result){
            ...
            $js .= "$(\"#content_weibo\").val('');$(\"#privacy\").val('0');$('#friendtrace').lazyshow({url:\"index.php?app=member_snshome&wwi=tracelist&curpage=1\",'iIntervalId':true});";
    ...
    public function sharegoodsWwi(){
        ...
        if ($_POST['form_submit'] == 'ok'){
            ...
            if ($result){
                ...
                $url = '';
                if ($_GET['irefresh']){
                    $js .= "$('#friendtrace').lazyshow({url:\"index.php?app=member_snsindex&wwi=tracelist&curpage=1\",'iIntervalId':true});";
                }else{
                    $url = 'reload';
                }
    ...
    public function deltraceWwi(){
        ...
        if ($result){
            ...
            if ($_GET['type'] == 'href'){
                showDialog(Language::get('nc_common_del_succ'),'index.php?app=member_snshome&wwi=trace&mid='.$_SESSION['member_id'],'succ');
            }else {
                $js = "location.reload();";
     ...
     * 编辑分享商品的可见权限(主人登录后操作)
     */
    public function editprivacyWwi(){
        $id = intval($_GET['id']);
        if ($id <= 0){
            showDialog(Language::get('wrong_argument'),'','error');
        }
     ...
     * 编辑分享店铺的可见权限(主人登录后操作)
     */
    public function storeprivacyWwi(){
        $id = intval($_GET['id']);
        if ($id <= 0){
            showDialog(Language::get('wrong_argument'),'','error');
        }
    ...
    public function addcommentWwi(){
        ...
        if ($result){
            ...
            if ($_POST['showtype'] == 1){
                $js .="$(\"#tracereply_{$originalid}\").load('index.php?app=member_snshome&wwi=commenttop&mid={$owner_id}&id={$originalid}&type={$originaltype}');";
            }else {
                $js .="$(\"#tracereply_{$originalid}\").load('index.php?app=member_snshome&wwi=commentlist&mid={$owner_id}&id={$originalid}&type={$originaltype}');";
            }
    ...
    public function addforwardWwi(){
        ...
        if ($result){
            ...
            if ($_GET['irefresh']){
                ...
                if ($_GET['type']=='home'){
                    $js = "$('#friendtrace').lazyshow({url:\"index.php?app=member_snshome&wwi=tracelist&mid={$tracelog_info['trace_memberid']}&curpage=1\",'iIntervalId':true});";
                }else if ($_GET['type']=='snshome'){
                    $js = "$('#forward_".$originalid."').hide();$('#friendtrace').lazyshow({url:\"index.php?app=member_snshome&wwi=tracelist&mid={$tracelog_info['trace_memberid']}&curpage=1\",'iIntervalId':true});";
                }else {
                    $js = "$('#friendtrace').lazyshow({url:\"index.php?app=member_snsindex&wwi=tracelist&curpage=1\",'iIntervalId':true});";
                }
     ...
     * 商品收藏页面和商品详细页面分享商品
     */
    public function sharegoods_oneWwi(){
        Language::read('member_sharemanage');
        $gid = intval($_GET['gid']);
        if ($gid<=0){
            showDialog(Language::get('wrong_argument'),'','error');
        ...
        if ($_GET['dialog']){
            $js = "CUR_DIALOG = ajax_form('sharegoods', '".Language::get('sns_sharegoods_tofriend')."', 'index.php?app=member_snsindex&wwi=sharegoods_one&gid={$gid}', 480);";
            showDialog('','','js',$js);
        }
        //查询商品信息
     ...
     * 店铺收藏页面分享店铺
     */
    public function sharestore_oneWwi(){
        Language::read('member_sharemanage');
        $sid = intval($_GET['sid']);
        if ($sid<=0){
            showDialog(Language::get('wrong_argument'),'','error');
        ...
        if ($_GET['dialog']){
            $js = "ajax_form('sharestore', '".Language::get('sns_sharestore')."', 'index.php?app=member_snsindex&wwi=sharestore_one&sid={$sid}', 480);";
            showDialog('','','js',$js);
        }
        //查询店铺信息
        $store_model = Model('store');
