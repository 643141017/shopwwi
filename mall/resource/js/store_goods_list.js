$(function(){
    ...
    $('i[nctype="ajaxGoodsList"]').toggle(
        function(){
            ...
            var _div = _parenttr.next().find('.wwist-goods-sku');
            if (_div.html() == '') {
                $.getJSON('index.php?app=store_goods_online&wwi=get_goods_list_ajax' , {commonid : _commonid}, function(date){
                    if (date != 'false') {
                        var _ul = $('<ul class="wwist-goods-sku-list"></ul>');
                        $.each(date, function(i, o){
                            ...
                            $('<div><a href="javascript:;" class="ncbtn-mini ncbtn-lavander mt5" onclick="ajax_form(\'ajax_jingle\', \'编辑独立描述\', \'index.php?app=store_goods_online&wwi=edit_iframe_ajax&goods_id=' + o.goods_id + '\', \'915\')">编辑独立描述</a></div>').appendTo(_li);
							if (o.is_distribution != '0') {$('<div><a href="javascript:;" class="ncbtn-mini ncbtn-aqua mt5" onclick="ajax_form(\'ajax_jingle\', \'编辑分销佣金\', \'index.php?app=store_goods_online&wwi=edit_invite_price&goods_id=' + o.goods_id + '\', \'650\')">编辑分销佣金</a></div>').appendTo(_li);}
                            _li.appendTo(_ul);
                        });
                        _ul.appendTo(_div);
                        _parenttr.next().show();
