 ...
 * @return void
 */
function requestNotFound($app = null, $wwi = null) {
 ...
 * @return string 字符串类型的返回结果
 */
function getNchash($app = '', $wwi = ''){
    $app = $app ? $app : $_GET['app'];
    $wwi = $wwi ? $wwi : $_GET['wwi'];
    if (C('captcha_status_login')){
        return substr(md5(MALL_SITE_URL.$app.$wwi),0,8);
    } else {
        return '';
    }
}
 ...
 * 获取文章URL
 */
function getTZXArticleUrl($article_id) {
    if(URL_MODEL) {
        // 开启伪静态
        return TZX_SITE_URL.DS.'article-'.$article_id.'.html';
    } else {
        return TZX_SITE_URL.DS.'index.php?app=article&wwi=article_detail&article_id='.$article_id;
    }
}
 ...
 * 获取画报URL
 */
function getTZXPictureUrl($picture_id) {
    if(URL_MODEL) {
        // 开启伪静态
        return TZX_SITE_URL.DS.'picture-'.$picture_id.'.html';
    } else {
        return TZX_SITE_URL.DS.'index.php?app=picture&wwi=picture_detail&picture_id='.$picture_id;
    }
}
 ...
 * 获取文章图片URL
 */
function getTZXArticleImageUrl($image_path, $image_name, $type='list') {
    if(empty($image_name)) {
        return UPLOAD_SITE_URL.DS.ATTACH_TZX.DS.'no_cover.png';
    } else {
        ...
        $file_path = ATTACH_TZX.DS.'article'.DS.$image_path.DS.str_ireplace('.', '_'.$type.'.', $image_name);
        if(file_exists(BASE_PATH.DS.$file_name)) {
            $image_name = UPLOAD_SITE_URL.DS.$file_path;
        } else {
            $image_name = UPLOAD_SITE_URL.DS.ATTACH_TZX.DS.'no_cover.png';
        }
        return $image_name;
    }
}
 ...
 * 获取文章图片URL
 */
function getTZXImageName($image_name_string) {
 ...
 * 获取TZX专题图片
 */
function getTZXSpecialImageUrl($image_name='') {
    return UPLOAD_SITE_URL.DS.ATTACH_TZX.DS.'special'.DS.$image_name;
}

/**
 * 获取TZX专题路径
 */
function getTZXSpecialImagePath($image_name='') {
    return BASE_UPLOAD_PATH.DS.ATTACH_TZX.DS.'special'.DS.$image_name;
}

/**
 * 获取TZX首页图片
 */
function getTZXIndexImageUrl($image_name='') {
    return UPLOAD_SITE_URL.DS.ATTACH_TZX.DS.'index'.DS.$image_name;
}

/**
 * 获取TZX首页图片路径
 */
function getTZXIndexImagePath($image_name='') {
    return BASE_UPLOAD_PATH.DS.ATTACH_TZX.DS.'index'.DS.$image_name;
}

/**
 * 获取TZX专题Url
 */
function getTZXSpecialUrl($special_id) {
    return TZX_SITE_URL.DS.'index.php?app=special&wwi=special_detail&special_id='.$special_id;
}
...
function getMallSpecialUrl($special_id) {
    return MALL_SITE_URL.DS.'index.php?app=special&wwi=special_detail&special_id='.$special_id;
}
 ...
 * 获取TZX专题静态文件
 */
function getTZXSpecialHtml($special_id) {
    $special_file = BASE_UPLOAD_PATH.DS.ATTACH_TZX.DS.'special_html'.DS.md5('special'.intval($special_id)).'.html';
 ...
 * 获取微商城个人秀图片地址
 */
function getTfxPersonalImageUrl($personal_info,$type=''){
    ...
    if(!empty($personal_image_list)){
        foreach ($personal_image_list as $value) {
            ...
            if(is_file(BASE_UPLOAD_PATH.DS.ATTACH_TFX.DS.$file_path)) {
                $personal_image_array[] = UPLOAD_SITE_URL.DS.ATTACH_TFX.DS.$file_path;
            } else {
                $personal_image_array[] = getTfxDefaultImage();
            }
        }
    } else {
        $personal_image_array[] = getTfxDefaultImage();
...
function ubb($ubb){
	$ubb = str_replace(array(
			...
			"<span style=\"color:#$1\">",
			"<img src=\"".TYQ_SITE_URL.'/templates/'.TPL_TYQ_NAME."/images/smilier/$1.png\">",
...
function themeImagePath($param){
	return BASE_UPLOAD_PATH.'/'.ATTACH_TYQ.'/theme/'.$param;
}
/**
 * 话题图片url
 *
...
function themeImageUrl($param){
	return UPLOAD_SITE_URL.'/'.ATTACH_TYQ.'/theme/'.$param;
}
/**
 * 圈子logo
 *
 ...
 */
function tyqLogo($id){
	if(file_exists(BASE_UPLOAD_PATH.'/'.ATTACH_TYQ.'/group/'.$id.'.jpg')){
		return UPLOAD_SITE_URL.'/'.ATTACH_TYQ.'/group/'.$id.'.jpg';
	}else{
		return UPLOAD_SITE_URL.'/'.ATTACH_TYQ.'/default_group_logo.gif';
	}
}
/**
 * sns 来自
...
function snsShareFrom($sign) {
    switch ($sign) {
        ...
        case '2' :
            ...
            break;
        case '3' :
            return L('sns_from') . '<a target="_black" href="' . TFX_SITE_URL . '">' . L('nc_modules_tfx') . '</a>';
            break;
        case '4' :
            return L('sns_from') . '<a target="_black" href="' . TZX_SITE_URL . '">TZX</a>';
            break;
        case '5' :
            return L('sns_from') . '<a target="_black" href="' . TYQ_SITE_URL . '">' . L('nc_tyq') . '</a>';
            break;
    }
}
 ...
 *
 * url('search,'index','array('cate_id'=>2)); 实际指向 index.php?app=search&wwi=index&cate_id=2
 * 传递数组参数时，若app（或wwi）值为index,则可以省略
 * 上面示例等同于
 * url('search','',array('app'=>'search','cate_id'=>2));
 *
 * @param string $app control文件名
 * @param string $wwi op方法名
 * @param array $args URL其它参数
 * @param boolean $model 默认取当前系统配置
 * @param string $site_url 生成链接的网址，默认取当前网址
 * @return string
 */
function url($app = '', $wwi = '', $args = array(), $model = false, $site_url = ''){
    //伪静态文件扩展名
    $ext = '.html';
    //入口文件名
    $file = 'index.php';
//    $site_url = empty($site_url) ? MALL_SITE_URL : $site_url;
    ...
    //定义变量存放返回url
    $url_string = '';
    if (empty($app) && empty($wwi) && empty($args)) {
        return $site_url;
    }
    $app = !empty($app) ? $app : 'index';
    $wwi = !empty($wwi) ? $wwi : 'index';
 ...
 * @return string
 */
function urlMall($app = '', $wwi = '', $args = array(), $store_domain = ''){

    // 如何使自营店则返回javascript:;

//    if ($app == 'show_store' && $wwi != 'goods_all') {
...
//    }
    ...
    if(URL_MODEL && in_array($app.':'.$wwi, $rewrite_item)) {
        $rewrite_flag = true;
    ...
    return url($app, $wwi, $args, $rewrite_flag, MALL_SITE_URL);
}
 ...
 * @return string
 */
function urlAdmin($app = '', $wwi = '', $args = array()){
    return url($app, $wwi, $args, false, ADMIN_SITE_URL);
}
 ...
 * @return string
 */
function urlAdminMall($app = '', $wwi = '', $args = array()){
    return url($app, $wwi, $args, false, ADMIN_modules_URL);
}
/**
 * 商城后台Tyq使用的URL链接函数，强制使用动态传参数模式
 *
 * @param string $app control文件名
 * @param string $wwi op方法名
 * @param array $args URL其它参数
 * @return string
 */
function urlAdminTyq($app = '', $wwi = '', $args = array()){
    return url($app, $wwi, $args, false, TYQ_modules_URL);
}
/**
 ...
 */
function urlAdminMobile($app = '', $wwi = '', $args = array()){
    return url($app, $wwi, $args, false, MOBILE_modules_URL);
}
/**
 * 商城后台Tzx使用的URL链接函数，强制使用动态传参数模式
 *
 * @param string $app control文件名
 * @param string $wwi op方法名
 * @param array $args URL其它参数
 * @return stringurlAdminMobile
 */
function urlAdminTzx($app = '', $wwi = '', $args = array()){
    return url($app, $wwi, $args, false, TZX_modules_URL);
}
/**
 * 商城后台TFX使用的URL链接函数，强制使用动态传参数模式
 *
 * @param string $app control文件名
 * @param string $wwi op方法名
 * @param array $args URL其它参数
 * @return stringurlAdminMobile
 */
function urlAdminTfx($app = '', $wwi = '', $args = array()){
    return url($app, $wwi, $args, false, TFX_modules_URL);
}
/**
 * TZX使用的URL链接函数，强制使用动态传参数模式
 *
 * @param string $app control文件名
 * @param string $wwi op方法名
 * @param array $args URL其它参数
 * @return string
 */
function urlTZX($app = '', $wwi = '', $args = array()){
	// 如果平台开启伪静态开关，并且为伪静态模块，修改标志为开启伪静态
    $rewrite_item = array(
    ...
    return url($app, $wwi, $args, $rewrite_flag, TZX_SITE_URL);
}
/**
 ...
 * @return string
 */
function urlTyq($app = '', $wwi = '', $args = array()){
	// 如果平台开启伪静态开关，并且为伪静态模块，修改标志为开启伪静态
    $rewrite_item = array(
	        'index:index',
	        'theme:theme_detail'
    ...
    return url($app, $wwi, $args, $rewrite_flag, TYQ_SITE_URL);
}
/**
 ...
 * @return string
 */
function urlTfx($app = '', $wwi = '', $args = array()){
    return url($app, $wwi, $args, false, TFX_SITE_URL);
}
/**
 ...
 * @return string
 */
function urlMember($app = '', $wwi = '', $args = array()) {
    // 默认标志为不开启伪静态
    $rewrite_flag = false;
    ...
    return url($app, $wwi, $args, $rewrite_flag, MEMBER_SITE_URL);
}
/**
 ...
 * @return string
 */
function urlLogin($app = '', $wwi = '', $args = array()) {
	    // 如果平台开启伪静态开关，并且为伪静态模块，修改标志为开启伪静态
    $rewrite_item = array(
	        'login:index',
			'login:register'
    ...
    return url($app, $wwi, $args, false, LOGIN_SITE_URL);
}
/**
 ...
 * @return string
 */
function urlChain($app = '', $wwi = '', $args = array()){
    return url($app, $wwi, $args, false, CHAIN_SITE_URL);
}
/**
 * 验证是否为平台店铺
 *
