...
<div class="choose-image">
  <ul class="tabs-nav">
    <li <?php if($_GET['curpage'] == ''){?>class="tabs-selected"<?php }?>><a href="javascript:void(0)"><?php echo $lang['tyq_network_image'];?></a></li>
    <?php if($_GET['type'] != 'm'){?><li <?php if($_GET['curpage'] != ''){?>class="tabs-selected"<?php }?>><a href="javascript:void(0)"><?php echo $lang['tyq_album_image'];?></a></li><?php }?>
  </ul>
  <div class="url-image tabs-panel <?php if($_GET['curpage'] != ''){?>tabs-hide<?php }?>">
    <label><i></i><?php echo $lang['tyq_insert_image_url'];?></label>
    <input name="" type="text" class="text w400" nctype="imageurl" />
    <div class="handle-bar" style="padding-left:0px;"><a href="Javascript: void(0)" class="button" nctype="imageurl"><?php echo $lang['tyq_insert_theme'];?></a></div>
  </div>
  <div class="goods-gallery tabs-panel <?php if($_GET['curpage'] == ''){?>tabs-hide<?php }?>">
    <div class="nav"><i></i><?php echo $lang['tyq_select_image_from_album'];?>
    ...
    <ul class="list">
      ...
      <?php }?>
      <?php }else{?>
      <span><?php echo $lang['tyq_upload_image_null'];?></span>
      <?php }?>
    </ul>
    <div class="ubb-pagination"><?php echo $output['show_page']; ?></div>
    <div class="handle-bar" style="padding-left:0px;"><a href="Javascript: void(0)" class="button" nctype="imagealbum"><?php echo $lang['tyq_insert_theme'];?></a></div>
  </div>
</div>
<script>
...
$(function() {
	...
	$('.choose-image').find('a[class="demo"]').click(function(){
		...
		CUR_DIALOG = ajax_form("uploadimage", '<?php echo $lang['tyq_select_image'];?>', _uri, 480);
		return false;
	});
	// 选择分类跳转
	$('#jumpMenu').change(function(){
		DialogManager.close('uploadimage');
	    var _uri = TYQ_SITE_URL + '/index.php?app=theme&wwi=choose_image&curpage=1&c_id='+c_id+'&class_id='+$(this).val();
		CUR_DIALOG = ajax_form("uploadimage", '<?php echo $lang['tyq_select_image'];?>', _uri, 480);
	});
	// 选择相册图片
