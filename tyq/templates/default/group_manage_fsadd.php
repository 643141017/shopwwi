...
<div class="eject_con">
  ...
  <form id="fsadd_form" action="<?php echo TYQ_SITE_URL;?>/index.php?app=manage&wwi=friendship_add&c_id=<?php echo $output['c_id'];?>" method="post" class="base-form-style">
    <input type="hidden" value="ok" name="form_submit">
    <dl>
      <dt><?php echo $lang['tyq_name'].$lang['nc_colon'];?></dt>
      <dd>
    ...
    <dl>
      <dt><?php echo $lang['ftyq_sort'].$lang['nc_colon'];?></dt>
      <dd>
        <input type="text" name="sort" class="w50 text" value="255" />
      </dd>
    </dl>
    <dl>
      <dt><?php echo $lang['tyq_tclass_sort'].$lang['nc_colon'];?></dt>
      <dd>
...
$(function(){
    ...
    $('#fsadd_form').validate({
    	...
    	submitHandler:function(form){
    		ajaxpost('fsadd_form', '<?php echo TYQ_SITE_URL;?>/index.php?app=manage&wwi=friendship_add&c_id='+c_id, '', 'onerror');
    	},
        rules : {
        	cid : {
            	min : 1
        ...
        messages : {
        	cid : {
            	min : '<?php echo $lang['ftyq_please_choose'];?>'
        	},
            sort  : {
                required : '<?php echo $lang['ftyq_sort_not_null'];?>',
                digits : '<?php echo $lang['tyq_tclass_sort_is_digits'];?>',
                max : '<?php echo $lang['tyq_tclass_sort_max'];?>'
            }
        }
    });
	...
	$('a[nctype="fsadd_search"]').click(function(){
		...
		$.getJSON(TYQ_SITE_URL+'/index.php?app=manage&wwi=search_tyq&c_id='+c_id+'&name='+name, function(data){
			if(data){
				...
				$.each(data, function(e, d){
					$('<option value="'+d.tyq_id+'">'+d.tyq_name+'</option>').appendTo(select);
				});
