...
<div class="theme-detail-poll-content">
  <form method="post" id="poll_form" <?php if(!$output['vote_end'] && !$output['partake']){?>action="<?php echo TYQ_SITE_URL;?>/index.php?app=theme&wwi=save_votepoll&c_id=<?php echo $output['c_id'];?>&t_id=<?php echo $output['t_id'];?>"<?php }?>>
    <input type="hidden" name="form_submit" value="ok" />
    <div class="poll-option-info">
      <h4>
        <?php if($output['poll_info']['poll_multiple'] == 1){echo $lang['tyq_checkbox_poll'];}else{echo $lang['tyq_radio_poll'];}?>
      </h4>
      <?php echo $lang['tyq_owned_by_all'];?><em><?php echo $output['poll_info']['poll_voters'];?></em><?php echo $lang['tyq_participate_in_the_vote'];?>
      <h5>
        <?php if($output['vote_end']){echo $lang['tyq_poll_ends'];}else if($output['partake']){echo $lang['tyq_have_to_vote'];}?>
      </h5>
    </div>
    <table width="100%" border=0 cellpadding="0" cellspacing="0">
      ...
      <tr>
        ...
        <td><?php if($val['pollop_votername'] != ''){?>
          ...
          <?php }?>
          <em><?php echo $val['pollop_votes'];?></em><?php echo $lang['tyq_have_poll'];?></td>
      </tr>
      <?php }?>
    </table>
	...
	$('a[nctype="poll_submit"]').click(function(){
		if(_ISLOGIN){
			if($('input[name="pollopid[]"]:checked').length == 0){
				$('span[nctype="poll-warning"]').html('<?php echo $lang['tyq_vote_option_not_null'];?>').show();
				window.setTimeout(pollWarningHide,5000);	// 5 seconds after the hidden message
				return false;
			}
			ajaxpost('poll_form', TYQ_SITE_URL+'/index.php?app=theme&wwi=save_votepoll&c_id='+c_id+'&t_id='+t_id, '', 'onerror');
		}else{
			login_dialog();		
		}
	});
