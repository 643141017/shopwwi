    ...
    <table class="base-table-style">
      <thead>
        ...
        <tr>
          ...
          <?php }?>
          <td colspan="20" class="tr"><?php if(count($output['thclass_list']) <10){?><a href="javascript:void(0);" nctype="add_thclass" class="add-group-class-btn mr10"><?php echo $lang['tyq_tclass_add'];?></a><?php }else{echo $lang['tyq_tclass_add_max_10'];}?></td>
        </tr>
      </thead>
      <tbody>
        <?php if(!empty($output['thclass_list'])){?>
      ...
      <tfoot>
        <tr>
          ...
          <th colspan="20"><label for="all" class="handle-btn"><i class="ac0"></i><?php echo $lang['nc_check_all'];?></label>
            <a href="javascript:void(0);" class="handle-btn" nctype="batchbutton" uri="<?php echo TYQ_SITE_URL;?>/index.php?app=manage&wwi=class_del&c_id=<?php echo $output['c_id'];?>" name="thc_id" confirm="<?php echo $lang['nc_ensure_del'];?>"><i class="ac5"></i><?php echo $lang['nc_delete'];?></a> </th>
        </tr>
      </tfoot>
      <?php }?>
    </table>
...
$(function(){
    ...
    $('a[nctype="add_thclass"]').click(function(){
    	_uri = "<?php echo TYQ_SITE_URL;?>/index.php?app=manage&wwi=class_add&inajax=1&c_id=<?php echo $output['c_id'];?>";
    	CUR_DIALOG = ajax_form('add_thclass', '<?php echo $lang['tyq_tclass_add'];?>', _uri, 520);
	});
});
function edit_thclass(id){
	_uri = "<?php echo TYQ_SITE_URL;?>/index.php?app=manage&wwi=class_edit&inajax=1&c_id=<?php echo $output['c_id'];?>&thc_id="+id;
	CUR_DIALOG = ajax_form('edit_thclass', '<?php echo $lang['tyq_tclass_edit'];?>', _uri, 520);
}
function del_thclass(id){
	showDialog('<?php echo $lang['nc_ensure_del'];?>', 'confirm', '', function(){
		_uri = "<?php echo TYQ_SITE_URL;?>/index.php?app=manage&wwi=class_del&inajax=1&c_id=<?php echo $output['c_id'];?>&thc_id="+id;
		ajaxget(_uri);
	});
}
</script>