...
<div class="choose-goods">
	<ul class="tabs-nav">
		<li class="tabs-selected"><a href="javascript:void(0);"><?php echo $lang['tyq_goods_link'];?></a></li>
		<li><a href="javascript:void(0)"><?php echo $lang['tyq_bought_goods'];?></a></li>
		<li><a href="javascript:void(0)"><?php echo $lang['tyq_favorite_goods'];?></a></li>
	</ul>
	<div class="goods-api tabs-panel">
		<label><i></i><?php echo $lang['tyq_goods_link_tips1'];if(C('taobao_api_isuse')){echo $lang['tyq_goods_link_tips2'];}echo $lang['tyq_goods_link_tips3'];?></label>
		<p>
    ...
    <ul id="mycarousel1" class="jcarousel-skin-tango">
      ...
      <li><a href="javascript:void(0);"
				data-param="{'id':'<?php echo $v['goods_id'];?>','name':'<?php echo $v['goods_name'];?>','price':'<?php echo $v['goods_price'];?>','storeid':'<?php echo $v['store_id'];?>','img':'<?php echo thumb($v, 60);?>','image':'<?php echo $v['goods_image'];?>','uri':'<?php echo urlMall('goods', 'index', array('goods_id'=>$v['goods_id']));?>','type':0}">
					...
					<p class="extra"><?php echo $lang['tyq_selected'];?></p>
			</a></li>
      <?php }?>
    </ul>
    <?php } else{?>
    <div class="norecord"><?php echo $lang['tyq_bought_goods_null'];?></div>
    <?php }?>
  </div>
	<div class="tabs-panel tabs-hide">
		<!-- 商品图片start -->
    ...
    <ul id="mycarousel2" class="snsgoodsimglist jcarousel-skin-tango">
      ...
      <li><a href="javascript:void(0);"
				data-param="{'id':'<?php echo $v['goods_id'];?>','name':'<?php echo $v['goods_name'];?>','price':'<?php echo $v['goods_price'];?>','storeid':'<?php echo $v['store_id'];?>','img':'<?php echo thumb($v, 60);?>','image':'<?php echo $v['goods_image'];?>','uri':'<?php echo urlMall('goods', 'index', array('goods_id'=>$v['goods_id']));?>','type':0}"><img
					...
					src="<?php echo thumb($v, 240);?>" />
					<p class="extra"><?php echo $lang['tyq_selected'];?></p> </a></li>
      <?php }?>
    </ul>
    <?php } else{?>
    <div class="norecord"><?php echo $lang['tyq_favorite_goods_null'];?></div>
    <?php }?>
  </div>
	<div class="" nctype="cgError"></div>
	<dl class="selected-goods">
		<dt><?php echo $lang['tyq_selected_goods'];?></dt>
	</dl>
	<div class="handle-bar">
		<a href="Javascript: void(0)" class="button" nctype="insertGoods"><?php echo $lang['tyq_insert_theme'];?></a>
	</div>
</div>
<script
...
$(function(){
	...
	$('a[nctype="goodsLinkAdd"]').click(function(){
		...
		if(count <= 0){
			// 这里怎么加提示？ 不能在继续选择商品
			$('div[nctype="cgError"]').html('<span><?php echo $lang['tyq_goods_error1'];?></span>');
    		window.setTimeout(hideError,5000);	// 5 seconds after the hidden message
			return false;
		}
		
		var link = encodeURIComponent($('#goods_link').val());
		var _uri = TYQ_SITE_URL+'/index.php?app=theme&wwi=check_link&link='+link+'&c_id='+c_id;
		$.getJSON(_uri, function(data){
			if (data){
        		...
        		// 这里怎么加提示？ 不能在继续选择商品
        		$('div[nctype="cgError"]').html('<span><?php echo $lang['tyq_goods_error2'];?></span>');
        		window.setTimeout(hideError,5000);	// 5 seconds after the hidden message
        		return;
            }
		});
...
function chooseGoods(o){
	...
	if(count <= 0){
		// 这里怎么加提示？ 不能在继续选择商品
		$('div[nctype="cgError"]').html('<?php echo $lang['tyq_goods_error1'];?>');
		return false;
	}
