...
class BaseTyqControl{
    ...
    protected function showLayout() {
		...
		//城市切换
		 if (C('shopwwi_city') != '') {
            $citys_list = @unserialize(C('shopwwi_city'));
        }
   ...
   //文章输出
    ...
    private function checkSuper() {
        if($_SESSION['is_login']){
            $super = Model('tyq_member')->getSuperInfo(array('member_id' => $_SESSION['member_id']));
     ...
     * 圈子信息
     */
    protected function tyqInfo(){
        $this->tyq_info = Model()->table('tyq')->where(array('tyq_id'=>$this->c_id))->find();

        if(empty($this->tyq_info)){
            showMessage(L('tyq_group_not_exists'), '', '', 'error');
        }
        Tpl::output('tyq_info', $this->tyq_info);
    }
    /**
     * 圈主和管理信息
     */
    protected function manageList(){
        $prefix = 'tyq_managelist';
        $info = rcache($this->c_id, $prefix);
        if (empty($info)) {
            $manager_list = Model()->table('tyq_member')->where(array('tyq_id'=>$this->c_id, 'is_identity'=>array('in', array(1,2))))->select();
    ...
    protected function memberInfo(){
        if($_SESSION['is_login']){
            $this->cm_info = Model()->table('tyq_member')->where(array('tyq_id'=>$this->c_id, 'member_id'=>$_SESSION['member_id']))->find();
    ...
    protected function sidebar(){
        ...
        if (empty($data)) {
            // 圈子所属分类
            $data['class_info'] = Model()->table('tyq_class')->where(array('class_id'=>$this->tyq_info['class_id']))->find();

            // 明星圈友
            $data['star_member'] = Model()->table('tyq_member')->where(array('cm_state'=>1, 'tyq_id'=>$this->c_id, 'is_star'=>1))->order('rand()')->limit(5)->select();

            // 最新加入
            $data['newest_member'] = Model()->table('tyq_member')->where(array('cm_state'=>1, 'tyq_id'=>$this->c_id))->order('cm_jointime desc')->limit(5)->select();
			
			// 热门话题
            $data['hots_themelist'] = Model()->table('tyq_theme')->where(array('is_closed'=>0,'tyq_id'=>$this->c_id))->order('theme_browsecount desc')->limit(10)->select();
        ...
        $cmid_lists  = Model()->table('tyq_member')->field('member_id')->where(array('tyq_id'=>$this->c_id, 'cm_state'=>1))->select();
        ...
        if(!empty($goods_lists)){
            ...
            // 友情圈子
            $data['friendship_list'] = Model()->table('tyq_fs')->where(array('tyq_id'=>$this->c_id, 'friendship_status'=>1))->order('friendship_sort asc')->select();
			
			foreach ($data['friendship_list'] as $k=>$v){
				$tyqsinfo = Model()->table('tyq')->where(array('tyq_id'=>$this->c_id))->find();
                $data['friendship_list'][$k]['friendship_desc']=$tyqsinfo['tyq_desc'];
				$data['friendship_list'][$k]['friendship_count']=Model()->table('tyq_theme')->where(array('theme_addtime'=>array('gt', $now), 'tyq_id'=>$this->c_id, 'is_closed'=>0))->count();
        }

		//	foreach ($data['friendship_list'] as $val){
		//		$tyqsinfo = $model->table('tyq')->where(array('tyq_id'=>$this->c_id))->find();
			//	$data['friendship_list']['friendship_desc']=$tyqsinfo['tyq_desc'];
		//	}
        }
    ...
    protected function themeTop(){
        $prefix = 'tyq_themetop';
        $info = rcache('tyq', $prefix);
        if (empty($info)) {
            $model = Model();
            // 最新话题
            $data['new_themelist'] = $model->table('tyq_theme')->where(array('is_closed'=>0))->order('theme_id desc')->limit(10)->select();
            // 热门话题
            $data['hot_themelist'] = $model->table('tyq_theme')->where(array('is_closed'=>0))->order('theme_browsecount desc')->limit(10)->select();
            // 人气回复
            $data['reply_themelist'] = $model->table('tyq_theme')->where(array('is_closed'=>0))->order('theme_commentcount desc')->limit(10)->select();
            $info['info'] = serialize($data);
            wcache('tyq', $info, $prefix, 60);
        }
     ...
     * SEO
     */
    protected function tyqSEO($title= '') {
        Tpl::output('html_title',$title.' '.C('tyq_seotitle').' - Powered by ShopWWI 完美S5版');
        Tpl::output('seo_keywords',C('tyq_seokeywords'));
        Tpl::output('seo_description',C('tyq_seodescription'));
    }
    ...
    protected function breadcrumd($param = ''){
        $crumd = array(
            ...
            1=>array(
                'link'=>TYQ_SITE_URL.'/index.php?app=group&c_id='.$this->c_id,
                'title'=>$this->tyq_info['tyq_name']
            ),
        );
        if(!empty($this->theme_info)){
            $crumd[2] = array(
                'link'=>TYQ_SITE_URL.'/index.php?app=theme&wwi=theme_detail&c_id='.$this->c_id.'&t_id='.$this->t_id,
...
class BaseTyqThemeControl extends BaseTyqControl{
    protected $tyq_info = array();   // 圈子详细信息
    ...
    protected function themeInfo(){
        ...
        $this->theme_info = Model()->table('tyq_theme')->where(array('tyq_id'=>$this->c_id, 'theme_id'=>$this->t_id))->find();
        if(empty($this->theme_info)){
            showMessage(L('tyq_theme_not_exists'), '', '', 'error');
        }
    ...
    protected function checkReplySelf(){
        ...
        $this->reply_info = Model()->table('tyq_threply')->where(array('theme_id'=>$this->t_id, 'reply_id'=>$this->r_id, 'member_id'=>$_SESSION['member_id']))->find();
    ...
    protected function checkThemeSelf(){
        ...
        $this->theme_info = Model()->table('tyq_theme')->where(array('theme_id'=>$this->t_id, 'member_id'=>$_SESSION['member_id']))->find();
        if(empty($this->theme_info)){
            showDialog(L('wrong_argument'));
        }
        Tpl::output('theme_info', $this->theme_info);
...
class BaseTyqManageControl extends BaseTyqControl{
    protected $tyq_info = array();   // 圈子详细信息
    protected $t_id = 0;        // 话题id
    protected $theme_info = array();    // 话题详细信息
    ...
    protected function tyqInfo(){
        // 圈子信息
        $this->tyq_info = Model()->table('tyq')->where(array('tyq_id'=>$this->c_id))->find();
        if(empty($this->tyq_info)) @header("location: ".TYQ_SITE_URL);
        Tpl::output('tyq_info', $this->tyq_info);
    }
    /**
     * 会员信息
     */
    protected function tyqMemberInfo(){
        // 会员信息
        $this->cm_info = Model()->table('tyq_member')->where(array('tyq_id'=>$this->c_id, 'member_id'=>$_SESSION['member_id']))->find();
        if(!empty($this->cm_info)){
    ...
    protected function removeManager($array){
        ...
        $cm_info = Model()->table('tyq_member')->where($where)->select();
        if(empty($cm_info)){
            return $array;
        }
    ...
    protected function checkIdentity($type){        // c圈主 m管理 cm圈主和管理
        ...
        if($sign){
            return L('tyq_permission_denied');
        }
    }
    /**
     * 会员加入的圈子
     */
    protected function memberJoinTyq(){
        // 所属圈子信息
        $tyq_array = Model()->table('tyq,tyq_member')->field('tyq.*,tyq_member.is_identity')
                        ->join('inner')->on('tyq.tyq_id=tyq_member.tyq_id')
                        ->where(array('tyq_member.member_id'=>$_SESSION['member_id']))->select();
        Tpl::output('tyq_array', $tyq_array);
    }
    /**
     * Top Navigation
     */
    protected  function sidebar_menu($sign, $child_sign=''){
        $menu = array(
                    'index'=>array('menu_name'=>L('tyq_basic_setting'), 'menu_url'=>'index.php?app=manage&c_id='.$this->c_id),
                    'member'=>array('menu_name'=>L('tyq_member_manage'), 'menu_url'=>'index.php?app=manage&wwi=member_manage&c_id='.$this->c_id),
                    'applying'=>array('menu_name'=>L('tyq_wait_apply'), 'menu_url'=>'index.php?app=manage&wwi=applying&c_id='.$this->c_id),
                    'level'=>array('menu_name'=>L('tyq_member_level'), 'menu_url'=>'index.php?app=manage_level&wwi=level&c_id='.$this->c_id),
                    'class'=>array('menu_name'=>L('tyq_tclass'), 'menu_url'=>'index.php?app=manage&wwi=class&c_id='.$this->c_id),
                    'inform'=>array(
                                'menu_name'=>L('tyq_inform'),
                                'menu_url'=>'index.php?app=manage_inform&wwi=inform&c_id='.$this->c_id,
                                'menu_child'=>array(
                                            'untreated'=>array('name'=>L('tyq_inform_untreated'), 'url'=>'index.php?app=manage_inform&wwi=inform&c_id='.$this->c_id),
                                            'treated'=>array('name'=>L('tyq_inform_treated'), 'url'=>'index.php?app=manage_inform&wwi=inform&type=treated&c_id='.$this->c_id)
                                        ),
                            ),
                    'managerapply'=>array('menu_name'=>L('tyq_mapply'), 'menu_url'=>'index.php?app=manage_mapply&c_id='.$this->c_id),
                    'friendship'=>array('menu_name'=>L('ftyq'), 'menu_url'=>'index.php?app=manage&wwi=friendship&c_id='.$this->c_id)
                );
        if($this->identity == 2){
...
class BaseTyqPersonalControl extends BaseTyqControl{
    ...
    protected function tyqMemberInfo(){
        ...
        $tyqmember_list = Model()->table('tyq_member')->where(array('member_id'=>$this->m_id))->select();
        ...
        if(!empty($tyqmember_list)){
            foreach ($tyqmember_list as $val){
