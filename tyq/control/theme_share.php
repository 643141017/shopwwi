...
class theme_shareControl extends BaseTyqControl{
    ...
    public function indexWwi(){
        ...
        if(!empty($theme_info)){
            $affix_list = Model()->table('tyq_affix')->where(array('affix_type'=>1, 'tyq_id'=>$this->c_id, 'theme_id'=>$t_id))->limit(1)->select();
        ...
        if(chksubmit()){
            ...
            if ($result){
                ...
                if (C('share_isuse') == 1){
                    ...
                    $params['url'] = TYQ_SITE_URL."/index.php?app=theme&wwi=theme_detail&c_id=".$this->c_id."&t_id=".$theme_info['theme_id'];
                ...
                Model()->table('tyq_theme')->where(array('theme_id'=>$t_id))->update(array('theme_sharecount'=>array('exp', 'theme_sharecount+1')));
    ...
    private function traceContent($theme_info, $affix_list){
        ...
        $url = TYQ_SITE_URL."/index.php?app=theme&wwi=theme_detail&c_id=".$this->c_id."&t_id=".$theme_info['theme_id'];
        if(!empty($affix_list[0])){
        ...
        $content .= "<div class=\"goodsinfo\"><p>".$_SESSION['member_name'].L('tyq_at,nc_quote1').$theme_info['tyq_name'].L('nc_quote2').L('tyq_share,sharebind_theme').L('nc_colon').'</p><p>'.L('nc_quote1').$theme_info['theme_name'].L('nc_quote2')."&nbsp;&nbsp;<a href='".$url."'>".L('sharebind_go_and_see')."</a></p></div></div>";
        return $content;
    }
}
