...
class manage_levelControl extends BaseTyqManageControl{
    ...
    public function levelWwi(){
        ...
        if(chksubmit()){
            ...
            if($_POST['levelset'] == 'custom'){
                ...
                if($levelset) $mlref_info = Model()->table('tyq_mlref')->where(array('mlref_id'=>$levelset))->find();
                if(!empty($mlref_info)){
            ...
            $rs = Model('tyq_level')->levelInsert($insert, true);
            showDialog(L('nc_common_op_succ'), 'reload', 'succ');
        }
        $model = Model();
        // Defaule Member Title list
        $mldefault_list = rkcache('tyq_level', true);
        if (!empty($mldefault_list)){
            ...
            $mlref_list = $model->table('tyq_mlref')->where(array('mlref_status'=>1))->select();
