...
class themeControl extends BaseTyqThemeControl{
     ...
     */
    public function ajax_themeinfoWwi(){
        // 话题信息
        $this->themeInfo();
        ...
        $model = Model();
        // 话题商品
        $goods_list = $model->table('tyq_thg')->where(array('theme_id'=>$this->t_id, 'reply_id'=>0))->select();
        $goods_list = tidyThemeGoods($goods_list, 'themegoods_id');
        $data['goods_list'] = $goods_list;
        // 附件
        $affix_list = $model->table('tyq_affix')->where(array('affix_type'=>1, 'theme_id'=>$this->t_id))->select();
        if(!empty($affix_list)){
        ...
        // 访问数增加
        $model->table('tyq_theme')->where(array('theme_id'=>$this->t_id))->update(array('theme_browsecount'=>array('exp', 'theme_browsecount+1')));
        ...
        if (isset($_SESSION['member_id'])) {
            // 是否赞过话题
            $like_info = $model->table('tyq_like')->where(array('theme_id'=>$this->t_id, 'member_id'=>$_SESSION['member_id']))->find();
            if(empty($like_info)){
                $data['theme_nolike'] = 1;
            }else{
                $data['theme_nolike'] = 0;
     ...
     */
    public function ajax_quickreplyWwi(){
        // 话题信息
        $this->themeInfo();

        $data = array();
        $data['form_action'] = TYQ_SITE_URL.'/index.php?app=theme&wwi=save_reply&type=quick&c_id='.$this->c_id.'&t_id='.$this->t_id;
        $data['member_avatar'] = getMemberAvatarForID($_SESSION['member_id']); // 头像
        // 回复
        $reply_list = Model()->table('tyq_threply')->where(array('theme_id'=>$this->t_id, 'tyq_id'=>$this->c_id))->order('reply_id desc')->limit(5)->select();
        if(!empty($reply_list)){
        ...
        $data['c_istalk']   = intval(C('tyq_istalk'));
        $data['c_contentleast'] = intval(C('tyq_contentleast'));
        if(intval(C('tyq_contentleast')) > 0){
            $data['c_contentmsg']   = sprintf(L('nc_content_min_length'), intval(C('tyq_contentleast')));
        }else{
     ...
     * 保存话题
     */
    public function save_themeWwi(){
        if(chksubmit()){
            // Reply function does close,throw error.
            if(!intval(C('tyq_istalk'))){
                showDialog(L('tyq_theme_cannot_be_published'));
            }
            // checked cookie of SEC
            if(cookie(tyq_intervaltime)){
                showDialog(L('tyq_operation_too_frequent'));
            }
            // 会员信息
            $this->memberInfo();
            // 圈子信息
            ...
            if(!in_array($this->identity, array(1,2,3))){
                showDialog(L('tyq_no_join_ban_release'));
            }
            ...
            $thclass_name = '';
            if($thclass_id > 0){
                $thclass_info = $model->table('tyq_thclass')->where(array('thclass_id'=>$thclass_id))->find();
            ...
            if(intval(C('tyq_contentleast')) > 0) $validate_arr[] = array("input"=>$_POST["themecontent"],"validator"=>'Length',"min"=>intval(C('tyq_contentleast')),"message"=>Language::get('tyq_contentleast'));
            ...
            $insert = array();
            $insert['theme_name']   = tyqCenterCensor($_POST['name']);
            $insert['theme_content']= tyqCenterCensor($_POST['themecontent']);
            $insert['tyq_id']    = $this->c_id;
            $insert['tyq_name']  = $this->tyq_info['tyq_name'];
            ...
            $themeid = $model->table('tyq_theme')->insert($insert);
            if($themeid){
                $has_goods = 0; // 存在商品标记
                $has_affix = 0;// 存在附件标记
                // 插入话题商品
                if(!empty($_POST['goods'])){
                    ...
                    $rs = $model->table('tyq_thg')->insertAll($goods_insert);
                    $has_goods = 1;
                }
                // 更新话题附件
                $model->table('tyq_affix')->where(array('affix_type'=>1, 'member_id'=>$_SESSION['member_id'], 'theme_id'=>0))->update(array('theme_id'=>$themeid, 'tyq_id'=>$this->c_id));

                // 更新话题信息
                $affixe_count = $model->table('tyq_affix')->where(array('affix_type'=>1, 'member_id'=>$_SESSION['member_id'], 'theme_id'=>$themeid))->count();
                if($affixe_count > 0){
                    $has_affix = 1;
                }
                if($has_goods || $has_affix){
                    ...
                    $model->table('tyq_theme')->where(array('theme_id'=>$themeid))->update($update);
                }

                // 更新圈子表话题数
                $update = array(
                            'tyq_thcount'=>array('exp', 'tyq_thcount+1')
                        );
                $model->table('tyq')->where(array('tyq_id'=>$this->c_id))->update($update);
                ...
                $model->table('tyq_member')->where(array('member_id'=>$_SESSION['member_id'], 'tyq_id'=>$this->c_id))->update($update);
                ...
                if($_GET['sp'] == 1){
                    ...
                    $model->table('tyq_thpoll')->insert($insert);
                    if(!empty($_POST['polloption'])){
                        ...
                        $model->table('tyq_thpolloption')->insertAll($insert_array);
                    }
                }

                // set cookie of SEC
                if(intval(C('tyq_intervaltime')) > 0){
                    setNcCookie('tyq_intervaltime', true, intval(C('tyq_intervaltime')));
                }
                ...
                $param['tyq_id']     = $this->c_id;
                $param['type']          = 'release';
                $param['itemid']        = $themeid;
                Model('tyq_exp')->saveExp($param);
                showDialog(L('nc_release_op_succ'), TYQ_SITE_URL.'/index.php?app=theme&wwi=theme_detail&c_id='.$this->c_id.'&t_id='.$themeid, 'succ');
            }else{
     ...
     */
    public function save_votepollWwi(){
        if(chksubmit()){
            $model = Model();
            // check theme
            $this->themeInfo();
            ...
            $poll_info = $model->table('tyq_thpoll')->where(array('theme_id'=>$this->t_id))->find();
            if(empty($poll_info)){
                showDialog(L('wrong_argument'));
            }
            if($poll_info['poll_endtime'] != 0 && time() > $poll_info['poll_endtime']){
                showDialog(L('tyq_poll_has_end'));
            }
            $voter_info = $model->table('tyq_thpollvoter')->where(array('theme_id'=>$this->t_id, 'member_id'=>$_SESSION['member_id']))->find();
            if(!empty($voter_info)){
                showDialog(L('tyq_poll_has_join'));
            }
            ...
            if(empty($pollopid)){
                showDialog(L('tyq_poll_choose_options'));
            }
            // Verify the voting options exist
            $option_list = $model->table('tyq_thpolloption')->where(array('pollop_id'=>array('in', $pollopid), 'theme_id'=>$this->t_id))->select();
            ...
            foreach ($option_list as $val){
                ...
                $model->table('tyq_thpolloption')->where(array('pollop_id'=>$val['pollop_id']))->update($update);        //
            }
            ...
            $model->table('tyq_thpollvoter')->insert($insert);

            // Update the total number of votes
            $update = array();
            $update['poll_voters']  = array('exp', 'poll_voters+'.count($option_list));
            $model->table('tyq_thpoll')->where(array('theme_id'=>$this->t_id))->update($update);
            showDialog(L('tyq_poll_success'), 'reload', 'succ');
        }
    }
    /**
     * 选择商品
     */
    public function choose_goodsWwi(){
        $model = Model();
        // 三个月内 已购买的商品
     ...
     * 上传图片
     */
    public function choose_imageWwi(){
        $model = Model();
        $where = array();
     ...
     * 附件批量上传 话题
     */
    public function image_uploadWwi(){
        $data['msg']                = 'error';
        ...
        $count = Model()->table('tyq_affix')->where($where)->count();
        if($count < 10){
            $partpath = themePartPath($_SESSION['member_id']);
            $upload = new UploadFile();
            $upload->set('default_dir', ATTACH_TYQ.'/theme/'.$partpath);
            ...
            if ($result){
                ...
                $insert['reply_id']         = 0;
                $insert['tyq_id']        = $this->c_id;
                $id = Model()->table('tyq_affix')->insert($insert);
     ...
     * 附件删除
     */
    public function delimgWwi(){
        $id = intval($_GET['id']);
        if($id > 0){
            $affix_info = Model()->table('tyq_affix')->where(array('member_id'=>$_SESSION['member_id'], 'affix_id'=>$id))->find();
            if(!empty($affix_info)){
                @unlink(themeImagePath($affix_info['affix_filename']));
                @unlink(themeImagePath($affix_info['affix_filethumb']));
                Model()->table('tyq_affix')->where(array('affix_id'=>$id))->delete();
                echo true;
            }
        }
    }
     ...
     * 获得未使用附件
     */
    public function unused_imgWwi(){
        $affix_list = Model()->table('tyq_affix')->field('affix_id,affix_filename,affix_filethumb')->where(array('member_id'=>$_SESSION['member_id'], 'affix_type'=>1, 'theme_id'=>0))->select();
        if(!empty($affix_list)){
            $affix_array = array();
            foreach($affix_list as $key=>$val){
     ...
     * 话题详细页
     */
    public function theme_detailWwi(){
        ...
        if($this->m_readperm < $this->theme_info['theme_readperm']){
            showMessage(L('tyq_Insufficient_permissions'), TYQ_SITE_URL, '', 'error');
        }
        ...
        $reply_info = $model->table('tyq_threply')->where($where)->page(15)->order('reply_id asc')->select();
        ...
        // goods
        $goods_array = $model->table('tyq_thg')->where($where)->select();
        ...
        // affix
        $affix_array = $model->table('tyq_affix')->where($where)->select();
        ...
        // member
        $member_list = $model->table('tyq_member')->field('member_id,tyq_id,cm_level,cm_levelname,is_identity')->where(array('tyq_id'=>$this->c_id, 'member_id'=>array('in', $memberid_array)))->select();
        ...
        $this->tyqSEO($this->theme_info['theme_name']);

        // Special theme
        if($this->theme_info['theme_special'] == 1){
            $poll_info = $model->table('tyq_thpoll')->where(array('theme_id'=>$this->t_id))->find();
            Tpl::output('poll_info', $poll_info);
            $option_list = $model->table('tyq_thpolloption')->where(array('theme_id'=>$this->t_id))->order('pollop_sort asc')->select();
    ...
    public function new_themeWwi(){
        ...
        $thclass_list = Model()->table('tyq_thclass')->where($where)->select();
        ...
        $this->tyqSEO(L('tyq_release_theme'));

        if($_GET['sp'] == 1){
            // breadcrumb navigation
            $this->breadcrumd(L('tyq_new_poll'));
            Tpl::showpage('theme.new_poll');
        }else{
            // breadcrumb navigation
            $this->breadcrumd(L('tyq_new_theme'));
    ...
    public function edit_themeWwi(){
        ...
        if(chksubmit()){
            ...
            $model = Model();
            $update = array();
            $update['theme_name']       = tyqCenterCensor($_POST['name']);
            $update['theme_content']    = tyqCenterCensor($_POST['themecontent']);
            ...
            $rs = $model->table('tyq_theme')->where(array('theme_id'=>$this->t_id))->update($update);
            if($rs){
                $has_goods = 0; // 存在商品标记
                $has_affix = 0;// 存在附件标记
                // 删除原有商品
                $goods_list = Model()->table('tyq_thg')->where(array('theme_id'=>$this->t_id, 'reply_id'=>0))->delete();
                // 插入话题商品
                if(!empty($_POST['goods'])){
                    $goods_insert = array();
                    foreach ($_POST['goods'] as $key=>$val){
                ...
                // 更新话题附件
                $model->table('tyq_affix')->where(array('affix_type'=>1, 'member_id'=>$_SESSION['member_id'], 'theme_id'=>0))->update(array('theme_id'=>$this->t_id, 'tyq_id'=>$this->c_id));

                // 更新话题信息
                $affixe_count = $model->table('tyq_affix')->where(array('affix_type'=>1, 'member_id'=>$_SESSION['member_id'], 'theme_id'=>$this->t_id))->count();
                if($affixe_count > 0){
                    $has_affix = 1;
                }
                if($has_goods || $has_affix){
                    ...
                    $update['has_goods']    = $has_goods;
                    $update['has_affix']    = $has_affix;
                    $model->table('tyq_theme')->where(array('theme_id'=>$this->t_id))->update($update);
                }
                // Special theme
                if($_GET['sp'] == 1){
                    // Update the vote
                    ...
                    if(!empty($_POST['polloption'])){
                        ...
                        foreach ($_POST['polloption'] as $key=>$val){
                            $option_info = $model->table('tyq_thpolloption')->where(array('pollop_id'=>$key, 'theme_id'=>$this->t_id))->find();
                            if(!empty($option_info)){
                                $update = array();
                                $update['pollop_option']= $val;
                                $update['pollop_sort']  = $_POST['pollsort'][$key];
                                $model->table('tyq_thpolloption')->where(array('pollop_id'=>$key))->update($update);
                            }else{
                                if ($val == '') continue;
                                $i = array();
                        ...
                        if(!empty($insert_array)) $model->table('tyq_thpolloption')->insertAll($insert_array);
                    }
                }
                showDialog(L('nc_deit_op_succ'), TYQ_SITE_URL.'/index.php?app=theme&wwi=theme_detail&c_id='.$this->c_id.'&t_id='.$this->t_id, 'succ');
            }else{
     ...
     * 赞
     */
    public function ajax_likeyesWwi(){
        // 话题信息
        $this->themeInfo();
        ...
        if(empty($like_info)){
            // 插入话题赞表
            Model()->table('tyq_like')->insert(array('theme_id'=>$this->t_id, 'member_id'=>$_SESSION['member_id'], 'tyq_id'=>$this->c_id));
            // 更新赞数量
            Model()->table('tyq_theme')->where(array('theme_id'=>$this->t_id))->update(array('theme_likecount'=>array('exp', 'theme_likecount+1')));
            echo 'true';
        }else{
            echo 'false';
        }
     ...
     * 取消赞
     */
    public function ajax_likenoWwi(){
        // 话题信息
        $this->themeInfo();
        ...
        if(empty($like_info)){
            ...
            // 删除话题赞表信息
            Model()->table('tyq_like')->where(array('theme_id'=>$this->t_id, 'member_id'=>$_SESSION['member_id']))->delete();
            // 更新赞数量
            Model()->table('tyq_theme')->where(array('theme_id'=>$this->t_id))->update(array('theme_likecount'=>array('exp', 'theme_likecount-1')));
            echo 'true';
        }
    ...
    public function replyWwi(){
        ...
        // 不是圈子成员不能发帖
        if(!in_array($this->identity, array(1,2,3))){
            showDialog(L('tyq_no_join_ban_reply'));
        }
        // 话题信息
        $this->themeInfo();

        if($_GET['answer_id'] != ''){
            $reply_info = Model()->table('tyq_threply')->where(array('theme_id'=>$this->t_id, 'reply_id'=>intval($_GET['answer_id'])))->find();
        ...
        $where['affix_type']= 2;
        $where['reply_id']  = 0;
        $affix_list = Model()->table('tyq_affix')->where($where)->select();
        Tpl::output('affix_list', $affix_list);

        $this->tyqSEO(L('nc_reply_theme'));
    ...
    public function edit_replyWwi(){
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = array(
                    array("input"=>$_POST["replycontent"], "require"=>"true", "message"=>L('tyq_reply_not_null')),
            );
            $error = $obj_validate->validate();
            if($error != ''){
                showDialog($error);
                ...
                $update['reply_content']= tyqCenterCensor($_POST['replycontent']);
                $update['reply_addtime']= time();
                $update['is_closed']    = 0;
                $rs = $model->table('tyq_threply')->where(array('theme_id'=>$this->t_id, 'reply_id'=>$this->r_id))->update($update);
                if($rs){
                    // 删除原有商品
                    $goods_list = Model()->table('tyq_thg')->where(array('theme_id'=>$this->t_id, 'reply_id'=>$this->r_id))->delete();
                    // 插入话题商品
                    if(!empty($_POST['goods'])){
                        $goods_insert = array();
                        foreach ($_POST['goods'] as $key=>$val){
                    ...
                    // 更新话题附件
                    $model->table('tyq_affix')->where(array('affix_type'=>2, 'member_id'=>$_SESSION['member_id'], 'reply_id'=>0))->update(array('theme_id'=>$this->t_id, 'reply_id'=>$this->r_id, 'tyq_id'=>$this->c_id));

                    showDialog(L('nc_common_op_succ'), 'index.php?app=theme&wwi=theme_detail&c_id='.$this->c_id.'&t_id='.$this->t_id, 'succ');
                }
            }
        }
        ...
        $goods_list = Model()->table('tyq_thg')->where($where)->select();
        $goods_list = tidyThemeGoods($goods_list, 'themegoods_id');
        Tpl::output('goods_list', $goods_list);
     ...
     * 话题回复保存
     */
    public function save_replyWwi(){
        // Reply function does close,throw error.
        if(!intval(C('tyq_istalk'))){
            showDialog(L('tyq_has_been_closed_reply'));
        }
        // checked cookie of SEC
        if(cookie(tyq_intervaltime)){
            showDialog(L('tyq_operation_too_frequent'));
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                $insert['member_id']    = $_SESSION['member_id'];
                $insert['member_name']  = $_SESSION['member_name'];
                $insert['reply_content']= tyqCenterCensor($_POST['replycontent']);
                $insert['reply_addtime']= time();
                $insert['is_closed']    = 0;

                // 回复楼层验证
                if($_POST['answer_id'] != ''){
                    $reply_info = Model()->table('tyq_threply')->where(array('theme_id'=>$this->t_id, 'reply_id'=>intval($_POST['answer_id'])))->find();
                    if(!empty($reply_info)) {
                ...
                $reply_id = $model->table('tyq_threply')->insert($insert);
                if($reply_id){
                    if($_GET['type'] == 'adv'){
                        // 插入话题商品
                        if(!empty($_POST['goods'])){
                        ...
                        // 更新话题附件
                        $model->table('tyq_affix')->where(array('affix_type'=>2, 'member_id'=>$_SESSION['member_id'], 'reply_id'=>0))->update(array('theme_id'=>$this->t_id, 'reply_id'=>$reply_id, 'tyq_id'=>$this->c_id));
                    }
                    ...
                    // 成员回复数增加 最后回复时间
                    $model->table('tyq_member')->where(array('member_id'=>$_SESSION['member_id'], 'tyq_id'=>$this->c_id))->update(array('cm_comcount'=>array('exp', 'cm_comcount+1'), 'cm_lastspeaktime'=>time()));
                    // set cookie of SEC
                    if(intval(C('tyq_intervaltime')) > 0){
                        setNcCookie('tyq_intervaltime', true, intval(C('tyq_intervaltime')));
                    }
    ...
    public function del_replyWwi(){
        ...
        if($affix_list){
            ...
            $model->table('tyq_affix')->where($where)->delete();
        }
        ...
        $param['type']      = 'reply';
        Model('tyq_recycle')->saveRecycle($param);

        // 删除回复
        $model->table('tyq_threply')->where(array('theme_id'=>$this->t_id, 'reply_id'=>$this->r_id, 'member_id'=>$_SESSION['member_id']))->delete();
