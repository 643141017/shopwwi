...
class groupControl extends BaseTyqThemeControl{
    ...
    public function indexWwi(){
        ...
        $theme_list = $model->table('tyq_theme')->where($where)->order('is_stick desc,lastspeak_time desc')->page(20)->select();
        ...
        if(!empty($theme_list)){
            ...
            $affix_list = $model->table('tyq_affix')->where(array('affix_type'=>1,'theme_id'=>array('in', $themeid_array)))->select();
        ...
        $todaythcount = $model->table('tyq_theme')->where(array('theme_addtime'=>array('egt',$dayBegin), 'tyq_id'=>$this->c_id))->count();
        Tpl::output('todaythcount', $todaythcount);

        //展示形式，默认以图文展示 list/preview
        if($_GET['type'] != ''){
            $display_mode = ($_GET['type'] == 'list')?'list':'preview';
            setNcCookie('tyqDisplayMode', $display_mode, 30*24*60*60);
        }else{
            $display_mode = cookie('tyqDisplayMode') ? cookie('tyqDisplayMode') : 'preview';
        }
        ...
        $thclass_list = $model->table('tyq_thclass')->where($where)->order('thclass_sort asc')->select();
        ...
        $this->tyqSEO($this->tyq_info['tyq_name']);
    ...
    public function applyWwi(){
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = array(
                array("input"=>$_POST["apply_content"], "require"=>"true", "message"=>L('tyq_apply_content_null')),
                array("input"=>$_POST["intro"], "require"=>"true", "message"=>L('tyq_introduction_not_null')),
            );
            ...
            if($error != ''){
                ...
                $insert['cm_state']         = intval($this->tyq_info['tyq_joinaudit']) == 0 ? 1 : 0;
                $insert['is_identity']      = 3;
                $model->table('tyq_member')->insert($insert, true);
                if(intval($this->tyq_info['tyq_joinaudit']) == 0){
                    // Update the number of members
                    $update = array(
                                'tyq_mcount'=>array('exp', 'tyq_mcount+1')
                            );
        ...
        $this->tyqSEO(L('tyq_apply_join').$this->tyq_info['tyq_name']);
        Tpl::showpage('group_apply', 'null_layout');
    }
    /**
     * 退出圈子
     */
    public function quitWwi(){
        // 圈子信息
        $this->tyqInfo();
        // 会员信息
        $this->memberInfo();
        if(in_array($this->identity, array(2,3))){
            // 删除会员
            Model()->table('tyq_member')->where(array('tyq_id'=>$this->c_id, 'member_id'=>$_SESSION['member_id']))->delete();

            $update = array();
            $update['tyq_mcount']= array('exp','tyq_mcount-1');

            // Whether to apply for management
            $rs = Model()->table('tyq_mapply')->where(array('tyq_id'=>$this->c_id, 'member_id'=>$_SESSION['member_id']))->find();
            if($rs){
                Model()->table('tyq_mapply')->where(array('tyq_id'=>$this->c_id, 'member_id'=>$_SESSION['member_id']))->delete();
     ...
     * 圈子成员列表
     */
    public function group_memberWwi(){
        // 圈子信息
        $this->tyqInfo();
        ...
        $cm_list = $model->table('tyq_member')->where($where)->order('is_identity asc,cm_jointime desc')->page(15)->select();
        Tpl::output('show_page', $model->showpage(2));
        Tpl::output('cm_list', $cm_list);

        $this->tyqSEO(L('tyq_member_list').$this->tyq_info['tyq_name']);

        // breadcrumb navigation
        $this->breadcrumd(L('tyq_firend'));
        Tpl::showpage('group.member');
    }
    /**
     * 圈子成员编辑
     */
    public function group_membereditWwi(){
        $model = Model();
        $where = array();
        $where['tyq_id'] = $this->c_id;
        ...
        if(chksubmit()){
            ...
            if(!in_array($this->identity, array(1,2,3,6))){
                showDialog(L('tyq_member_no_join'), 'reload', 'error', 'CUR_DIALOG.close();');
            }
            $update = array();
        ...
        $member_info = $model->table('tyq_member')->where($where)->find();
     ...
     * 圈子商品列表
     */
    public function group_goodsWwi(){
        // 圈子信息
        $this->tyqInfo();
        ...
        // 成员商品列表
        $model = Model();
        $cmid_list  = $model->table('tyq_member')->field('member_id')->where(array('tyq_id'=>$this->c_id, 'cm_state'=>1))->select();
        ...
        $this->tyqSEO(L('tyq_member_like_and_show_goods').$this->tyq_info['tyq_name']);
     ...
     */
    public function manage_applyWwi(){
        $model = Model();
        // Tyq information
        $this->tyqInfo();
        // Verify membership
        $cm_info = $model->table('tyq_member')->where(array('tyq_id'=>$this->c_id, 'cm_state'=>1, 'is_identity'=>3))->find();
        if(empty($cm_info) || $this->tyq_info['mapply_open'] == 0 || $this->tyq_info['mapply_ml'] > $cm_info['cm_level'] || $cm_info['is_identity'] == 1){
            if(chksubmit()){
                showDialog(L('tyq_apply_error'), '', 'error', 'DialogManager.close(\'manage_apply\')');
            }else{
                echo '<script>showError("'.L('tyq_apply_error').'");DialogManager.close("manage_apply");</script>';exit;
            }
        }
        // Ban repeated application
        $mapply_info = $model->table('tyq_mapply')->where(array('tyq_id'=>$this->c_id, 'member_id'=>$_SESSION['member_id']))->find();
        if(!empty($mapply_info)){
            if(chksubmit()){
                showDialog(L('tyq_repeat_apply_error'), '', 'error', 'DialogManager.close(\'manage_apply\')');
            }else{
                echo '<script>showError("'.L('tyq_repeat_apply_error').'");DialogManager.close("manage_apply");</script>';exit;
            }
        }
        ...
        if(chksubmit()){
            ...
            $model->table('tyq_mapply')->insert($update);

            // Update the application for membership
            $model->table('tyq')->where(array('tyq_id'=>$this->c_id))->update(array('new_mapplycount'=>array('exp', 'new_mapplycount+1')));
