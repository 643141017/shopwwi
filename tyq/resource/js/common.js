$(function(){
	$('.my-group').mouseover(function(){
		...
		if(!$this.hasClass('t')){
			$this.addClass('t');
			if(_ISLOGIN){
				$.getJSON(TYQ_SITE_URL+'/index.php?app=index&wwi=myjoinedtyq', function(data){
					if(data){
						$.each(data, function(e,d){
							...
							$('<a href="'+TYQ_SITE_URL+'/index.php?app=group&c_id='+d.tyq_id+'">'+d.tyq_name+$i+'</a>').appendTo('span[nctype="span-mygroup"]');
						});
					}else{
	...
	// 创建圈子
	$('a[nctype="create_tyq"]').click(function(){
		if(_ISLOGIN == 0){
			login_dialog();
		}else{
			window.location.href=TYQ_SITE_URL+"/index.php?app=index&wwi=add_group";
		}
	});
					...
					if(options.identity  == 1 || options.identity == 2 || options.identity == 3){ 	// 成员点击展开回复
						if(options.reply_box.css('display') == 'none'){
							if(!options.reply.hasClass('t')){
								...
								// 快速回复
								$.getJSON(TYQ_SITE_URL+'/index.php?app=theme&wwi=ajax_quickreply&c_id='+options.c_id+'&t_id='+options.id, function(data){
						...
						// 点击展开申请
						CUR_DIALOG = ajax_form('apply_join','申请加入','index.php?app=group&wwi=apply&c_id='+options.c_id,520,1);
					}
				}else{
					login_dialog();		
				}
	...
	// Membership card
	$('[nctype="mcard"]').membershipCard({type:'tyq'});
});
//弹出框登录
function login_dialog(){
		$.show_nc_login({
			nchash:NC_HASH,
			formhash:NC_TOKEN,
			anchor:'tyq_comment_flag'
		});	
}

//赞
function likeYes(o,options){
	$.getJSON(TYQ_SITE_URL+'/index.php?app=theme&wwi=ajax_likeyes&c_id='+options.c_id+'&t_id='+options.t_id, function(data){
		if(data){
...
function likeNo(o,options){
	$.getJSON(TYQ_SITE_URL+'/index.php?app=theme&wwi=ajax_likeno&c_id='+options.c_id+'&t_id='+options.t_id, function(data){
		if(data){
			var likeCount = parseInt(o.find('em[nctype="like"]').html())-1;
