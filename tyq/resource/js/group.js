$(function(){
	...
	$('a[nctype="apply"]').click(function(){
		if(_ISLOGIN == 0){
			...
			CUR_DIALOG = ajax_form('apply_join','申请加入','index.php?app=group&wwi=apply&c_id='+c_id,520,1);
		}
	});
	// 退出圈子
	$('a[nctype="quitGroup"]').click(function(){
		showDialog('确定要退出圈子吗？', 'confirm', '', function(){
			var _uri = TYQ_SITE_URL+"/index.php?app=group&wwi=quit&c_id="+c_id;
			ajaxget(_uri);
		});
	});
	//横高局中比例缩放隐藏显示图片
	...
	$('a[nctype="theme_read"],span[nctype="theme_read"]').click(function(){
		...
		if(!$this.hasClass('t')){
			...
			$.getJSON(TYQ_SITE_URL+'/index.php?app=theme&wwi=ajax_themeinfo&c_id='+c_id+'&t_id='+$id, function(data){
				// 内容部分
	...
	$('a[nctype="themeCloseYes"]').click(function(){
		...
		var _uri = TYQ_SITE_URL+'/index.php?app=manage&wwi=ajax_nospeak&value=0&c_id='+c_id+'&m_id='+m_id;
		ajaxget(_uri);
	});
	// 解除禁言
	$('a[nctype="themeCloseNo"]').click(function(){
		var m_id = $(this).attr('data-param');
		var _uri = TYQ_SITE_URL+'/index.php?app=manage&wwi=ajax_nospeak&value=1&c_id='+c_id+'&m_id='+m_id;
		ajaxget(_uri);
	});
	// 管理删除主题
	$('a[nctype="themeDelManage"]').click(function(){
		showDialog('确定要删除主题吗？', 'confirm', '', function(){
			var _uri = TYQ_SITE_URL+"/index.php?app=manage&wwi=del_theme&c_id="+c_id+"&t_id="+t_id;
			ajaxget(_uri);
		});
	});
	// 管理删除回复
	$('a[nctype="replyDelManage"]').click(function(){
		...
		showDialog('确定要删除回复吗？', 'confirm', '', function(){
			...
			var _uri = TYQ_SITE_URL+'/index.php?app=manage&wwi=del_reply&c_id='+c_id+'&t_id='+t_id+'&r_id='+r_id;
			ajaxget(_uri);
		});
	});
	...
	$('a[nctype="manageApply"]').click(function(){
		var _uri = TYQ_SITE_URL+'/index.php?app=group&wwi=manage_apply&c_id='+c_id;
...
function digestYes(o){
	$.getJSON(TYQ_SITE_URL+'/index.php?app=manage&wwi=ajax&column=digest&value=1&c_id='+c_id+'&id='+t_id, function(data){
		if(data){
			o.html('取消精华');
...
// 取消加精
function digestNo(o){
	$.getJSON(TYQ_SITE_URL+'/index.php?app=manage&wwi=ajax&column=digest&value=0&c_id='+c_id+'&id='+t_id, function(data){
		if(data){
			o.html('精华');
...
// 置顶
function topYes(o){
	$.getJSON(TYQ_SITE_URL+'/index.php?app=manage&wwi=ajax&column=top&value=1&c_id='+c_id+'&id='+t_id, function(data){
		if(data){
			o.html('取消置顶');
...
// 取消置顶
function topNo(o){
	$.getJSON(TYQ_SITE_URL+'/index.php?app=manage&wwi=ajax&column=top&value=0&c_id='+c_id+'&id='+t_id, function(data){
		if(data){
			o.html('置顶');
			$('.theme-title > i').removeClass().addClass('normal');
			o.unbind().click(function(){
...
// 关闭
function shutYes(o){
	$.getJSON(TYQ_SITE_URL+'/index.php?app=manage&wwi=ajax&column=shut&value=1&c_id='+c_id+'&id='+t_id, function(data){
		if(data){
			o.html('开启');
...
// 开启
function shutNo(o){
	$.getJSON(TYQ_SITE_URL+'/index.php?app=manage&wwi=ajax&column=shut&value=0&c_id='+c_id+'&id='+t_id, function(data){
		if(data){
			o.html('关闭');
			$('.theme-title > i').removeClass().addClass('normal');
			o.unbind().click(function(){
...
function speakNo(m_id){
	_uri = TYQ_SITE_URL+'/index.php?app=manage&wwi=ajax_nospeak&value=0&c_id='+c_id+'&m_id='+m_id;
	ajaxget(_uri);
}
// 删除回复