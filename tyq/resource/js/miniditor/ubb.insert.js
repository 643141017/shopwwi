	...
	$.fn.ncUBB = function(options){
		...
		this.each(function() {
			...
			$this.find('a[nctype="chooseGoods"]').click(function(){
				...
				var _uri = TYQ_SITE_URL+'/index.php?app=theme&wwi=choose_goods&c_id='+c_id+'&count='+count;
				CUR_DIALOG = ajax_form("choosegoods", '选择商品', _uri, 510);
			});
		
			$this.find('a[nctype="uploadImage"]').click(function(){
				var _uri = TYQ_SITE_URL+'/index.php?app=theme&wwi=choose_image&c_id='+c_id;
			...
			$this.find('a[nctype="highReply"]').click(function(){
				_uri = 'index.php?app=theme&wwi=reply&c_id='+c_id+'&t_id='+t_id;
		...
		function affixDelete(o){
			...
			$.getJSON(TYQ_SITE_URL+'/index.php?app=theme&wwi=delimg&c_id='+c_id+'&id='+id, function(){
		...
		function affixDeleteManage(o){
			// 删除图片
			var id = o.attr('data-param');
			$.getJSON(TYQ_SITE_URL+'/index.php?app=manage&wwi=delimg&c_id='+c_id+'&id='+id, function(){
				o.parents('li:first').remove();
				checkInsertAffix();
			});
		}
		...
		function getUnusedAffix(){
			...
			if(len == 0){
				$.getJSON('index.php?app=theme&wwi=unused_img&c_id='+c_id, function(data){	// 获取未使用附件
					if(data != null){
						$('div[nctype="affix"]').hide();
