...
function send_sms(mobile, sec_val, sec_key) {
    $.getJSON(ApiUrl+'/index.php?app=connect&wwi=get_sms_captcha', {type:1,phone:mobile,sec_val:sec_val,sec_key:sec_key}, function(result){
...
function check_sms_captcha(mobile, captcha) {
    $.getJSON(ApiUrl + '/index.php?app=connect&wwi=check_sms_captcha', {type:1,phone:mobile,captcha:captcha }, function(result){
        if (!result.datas.error) {
