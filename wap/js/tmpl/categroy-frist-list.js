$(function() {
	...
	$('#categroy-cnt').on('click','.category', function(){
	    ...
	    $.getJSON(ApiUrl + '/index.php?app=goods_class&wwi=get_child_all', {gc_id:gc_id}, function(result){
	        var data = result.datas;
            data.WapSiteUrl = WapSiteUrl;
...
function get_brand_recommend() {
    ...
    $.getJSON(ApiUrl + '/index.php?app=brand&wwi=recommend_list', function(result){
        var data = result.datas;
        data.WapSiteUrl = WapSiteUrl;
