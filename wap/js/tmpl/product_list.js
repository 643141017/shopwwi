...
$(function(){
    ...
    $('#product_list').on('click', '.goods-store a',function(){
        ...
        $.getJSON(ApiUrl + '/index.php?app=store&wwi=store_credit', {store_id:store_id}, function(result){
            var html = '<dl>'
...
function get_list() {
    ...
    $.getJSON(ApiUrl + '/index.php?app=goods&wwi=goods_list' + window.location.search.replace('?','&'), param, function(result){
    	if(!result) {
    		result = [];
    		result.datas = [];
