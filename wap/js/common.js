...
function getSearchName() {
	...
	if (keyword == '') {
	    if(getCookie('deft_key_value') == null) {
	        $.getJSON(ApiUrl + '/index.php?app=index&wwi=search_hot_info', function(result) {
...
function getFreeVoucher(tid) {
    ...
    $.ajax({
        type:'post',
        url:ApiUrl+"/index.php?app=member_voucher&wwi=voucher_freeex",
...
function updateCookieCart(key) {
    ...
    if (cartlist) {
        $.ajax({
            type:'post',
            url:ApiUrl+'/index.php?app=member_cart&wwi=cart_batchadd',
...
function getCartCount(key, expireHours) {
    ...
    if (getCookie('key') !== null && getCookie('cart_count') === null) {
        ...
        $.ajax({
            type:'post',
            url:ApiUrl+'/index.php?app=member_cart&wwi=cart_count',
            data:{key:key},
            dataType:'json',
            async:false,
            success:function (result) {
...
function getChatCount() {
    if ($('#header').find('.message').length > 0) {
        var key = getCookie('key');
        if (key !== null) {
            $.getJSON(ApiUrl+'/index.php?app=member_chat&wwi=get_msg_count', {key:key}, function(result){
                if (result.datas > 0) {
     ...
     */
        areaSelected: function(options) {
            ...
            function _getAreaList() {
                $.ajax({//获取区域列表
                    type:'get',
                    url:ApiUrl+'/index.php?app=area&wwi=area_list',
                    data:{area_id:ASID},
                    dataType:'json',
                    async:false,
                    success:function(result){
...
function loadSeccode(){
    ...
    $.ajax({
        type:'get',
        url:ApiUrl+"/index.php?app=seccode&wwi=makecodekey",
        async : false,
        dataType: 'json',
        success:function(result){
    ...
    $("#codeimage").attr('src',ApiUrl+'/index.php?app=seccode&wwi=makecode&k='+$("#codekey").val()+'&t=' + Math.random());
}
/**
 * 收藏店铺
 */
function favoriteStore(store_id){
    ...
    $.ajax({
        type: 'post',
        url: ApiUrl + '/index.php?app=member_favorites_store&wwi=favorites_add',
...
function dropFavoriteStore(store_id){
    ...
    $.ajax({
        type: 'post',
        url: ApiUrl + '/index.php?app=member_favorites_store&wwi=favorites_del',
        data: {key: key, store_id: store_id},
        dataType: 'json',
        async: false,
        success: function(result) {
...
function favoriteGoods(goods_id){
    ...
    $.ajax({
        type: 'post',
        url: ApiUrl + '/index.php?app=member_favorites&wwi=favorites_add',
...
function dropFavoriteGoods(goods_id){
    ...
    $.ajax({
        type: 'post',
        url: ApiUrl + '/index.php?app=member_favorites&wwi=favorites_del',
