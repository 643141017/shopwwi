...
class member_paymentControl extends mobileMemberControl {
    ...
    public function __construct() {
        ...
        if($_GET['wwi'] != 'payment_list' && !$_POST['payment_code']) {
            $payment_code = 'alipay';

            if(in_array($_GET['wwi'], array('wx_app_pay', 'wx_app_pay3', 'wx_app_vr_pay', 'wx_app_vr_pay3'), true)) {
                $payment_code = 'wxpay';
            }
            else if (in_array($_GET['wwi'],array('alipay_native_pay','alipay_native_vr_pay'),true)) {
     ...
     * 可用支付参数列表
     */
    public function payment_listWwi() {
        $model_mb_payment = Model('mb_payment');
     ...
     * 微信APP虚拟订单支付
     */
    public function wx_app_vr_payWwi() {
        $pay_sn = $_POST['pay_sn'];
     ...
     * 微信APP订单支付
     */
    public function wx_app_pay3Wwi() {
        $pay_sn = $_POST['pay_sn'];
     ...
     * 微信APP虚拟订单支付
     */
    public function wx_app_vr_pay3Wwi() {
        $pay_sn = $_POST['pay_sn'];
     ...
     * 取得支付宝移动支付 订单信息 实物订单
     */
    public function alipay_native_payWwi() {
        $pay_sn = $_POST['pay_sn'];
     ...
     * 取得支付宝移动支付 订单信息 虚拟订单
     */
    public function alipay_native_vr_payWwi() {
        $pay_sn = $_POST['pay_sn'];
        if (!preg_match('/^\d+$/',$pay_sn)){
            output_error('支付单号错误');
        }
