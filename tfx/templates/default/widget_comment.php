...
$(document).ready(function(){
     ...
     * 评论
     **/
    var url_comment_list = "index.php?app=comment&wwi=comment_list&type=<?php echo $output['comment_type'];?>&comment_id=<?php echo $output['comment_id'];?>";
    var url_comment_save = "index.php?app=comment&wwi=comment_save&type=<?php echo $output['comment_type'];?>";

    $("#comment_message").tfx_publish({
    ...
    $("[nc_type=comment_drop]").live('click',function(){
        if(confirm('<?php echo $lang['nc_ensure_del'];?>')) {
            ...
            $.getJSON("index.php?app=comment&wwi=comment_drop&type=<?php echo $output['comment_type'];?>", { comment_id: $(this).attr("comment_id")}, function(json){
                if(json.result == "true") {
                    $("#comment_count").tfx_count({type:'-'});
                    item.remove();
                } else {
...
<div id="widgetcommenttitle" class="widget-comment-title">
  <h3><?php echo $lang['tfx_comment_title'];?></h3>
...
<form id="add_form">
  ...
  <div><a id="btn_show_smilies" href="javascript:void(0)" class="smile"><?php echo $lang['tfx_comment_smiles'];?></a> 
    <!-- 站外分享 -->
    <?php require('widget_share.php');?>
  <input id="btn_publish_comment" class="input-button" type="button" value="<?php echo $lang['tfx_text_publish'];?>" />
  </div>
</form>
</dd>
<div class="clear">&nbsp;</div>
