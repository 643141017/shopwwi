...
$(document).ready(function(){
    ...
    $("img.lazy").tfx_lazyload();

    //喜欢
    $("[nc_type=tfx_like]").tfx_like({type:'personal'});
});
</script>
...
<div class="commend-goods-list">
  ...
  <ul id="pinterest">
    <li id="btn_personal_publish" class="item" style="display:none;">
      <div class="pinterest-button"><a id="btn_show_publish_div" href="javascript:void(0)" class="pngFix"><?php echo $lang['tfx_personal_publish'];?></a></div>
    </li>
    ...
    <li class="item">
      ...
      <?php } ?>
      <?php } ?>
      <div class="picture"> <a href="<?php echo TFX_SITE_URL;?>/index.php?app=personal&wwi=detail&personal_id=<?php echo $value['personal_id'];?>" target="_blank">
        <?php $personal_image_array = getTfxPersonalImageUrl($value,'list');?>
        <?php $size = getTfxImageSize($personal_image_array[0], 238);?>
        <img class="lazy" height="<?php echo $size['height'];?>" width="<?php echo $size['width'];?>" src="<?php echo TFX_TEMPLATES_URL;?>/images/loading.gif" data-src="<?php echo $personal_image_array[0];?>" /> </a> </div>
      <div class="handle"> <span class="like-btn"><a nc_type="tfx_like" like_id="<?php echo $value['personal_id'];?>" href="javascript:void(0)"><i class="pngFix"></i><span><?php echo $lang['tfx_text_like'];?></span><em><?php echo $value['like_count']<=999?$value['like_count']:'999+';?></em></a></span> <span class="comment"><a href="<?php echo TFX_SITE_URL;?>/index.php?app=personal&wwi=detail&personal_id=<?php echo $value['personal_id'];?>" target="_blank"><i title="<?php echo $lang['tfx_text_comment'];?>" class="pngFix">&nbsp;</i><em><?php echo $value['comment_count']<=999?$value['comment_count']:'999+';?></em></a></span> </div>
      <dl class="commentary">
        <dt><span class="thumb size30"><i></i><a href="<?php echo TFX_SITE_URL;?>/index.php?app=home&member_id=<?php echo $value['commend_member_id'];?>" target="_blank"> <img src="<?php echo getMemberAvatar($value['member_avatar']);?>" alt="<?php echo $value['member_name'];?>" onload="javascript:DrawImage(this,30,30);" /> </a></span></dt>
        <dd>
          <h4><a href="<?php echo TFX_SITE_URL;?>/index.php?app=home&member_id=<?php echo $value['commend_member_id'];?>" target="_blank"> <?php echo $value['member_name'];?> </a> </h4>
          <p> <a href="<?php echo TFX_SITE_URL;?>/index.php?app=personal&wwi=detail&personal_id=<?php echo $value['personal_id'];?>" target="_blank"> <?php echo $value['commend_message'];?> </a> </p>
        </dd>
      </dl>
    </li>
    <?php } ?>
  ...
  <div class="no-content"><i class="personal pngFix">&nbsp;</i>
    ...
    <p><?php echo $lang['tfx_personal_list_none'];?></p>
    <?php } else { ?>
    <?php if($_GET['wwi'] == 'like_list') { ?>
    <?php if($output['owner_flag'] === TRUE){ ?>
    <p><?php echo $lang['tfx_personal_like_list_none'];?></p>
    <?php } else { ?>
    <p><?php echo $lang['nc_quote1'];?><?php echo $output['member_info']['member_name'];?><?php echo $lang['nc_quote2'];?><?php echo $lang['tfx_personal_like_list_none_owner'];?></p>
    <?php } ?>
    <?php } else { ?>
    <?php if($output['owner_flag'] === TRUE){ ?>
    <p><?php echo $lang['tfx_personal_list_none_publish'];?></p>
    <a id="link_personal_publish" href="javascript:void(0)"><?php echo $lang['tfx_personal_publish'];?></a></div>
  <?php } else { ?>
  <p><?php echo $lang['nc_quote1'];?><?php echo $output['member_info']['member_name'];?><?php echo $lang['nc_quote2'];?><?php echo $lang['tfx_personal_list_none_publish_owner'];?></p>
  <?php } ?>
  <?php } ?>
  <?php } ?>
</div>
...
<script type="text/javascript" src="<?php echo TFX_RESOURCE_SITE_URL;?>/js/fileupload/jquery.iframe-transport.js" charset="utf-8"></script> 
<script type="text/javascript" src="<?php echo TFX_RESOURCE_SITE_URL;?>/js/fileupload/jquery.ui.widget.js" charset="utf-8"></script> 
<script type="text/javascript" src="<?php echo TFX_RESOURCE_SITE_URL;?>/js/fileupload/jquery.fileupload.js" charset="utf-8"></script> 
<!-- 发布个人秀 --> 
<script type="text/javascript">
$(document).ready(function(){

    $("#publish_div").tfx_form({title:'<?php echo $lang['tfx_personal_publish'];?>'});
    ...
    function show_publish_form() {
        $.getJSON("index.php?app=publish&wwi=personal_limit", {}, function(json){
            if(json.result == "true") {
                $("#publish_div").tfx_form_show({width:480});
            } else {
    ...
    $("#commend_message").tfx_publish({
        ...
        if($("#personal_image").val() != "") {
            ...
            show_personal_message("<?php echo $lang['tfx_text_upload_image'];?>");
        }
    });
    ...
    $("#fileupload").fileupload({
        dataType: 'json',
            url: "<?php echo TFX_SITE_URL.DS.'index.php?app=publish&wwi=personal_image_upload';?>",
            add: function(e,data) {
                ...
                if(result.status == "success") {
                    $("#_personal_image_list img").attr("src","<?php echo UPLOAD_SITE_URL.DS.ATTACH_TFX.DS.$_SESSION['member_id'].DS;?>"+result.file);
    ...
    $("[nc_type='btn_personal_image_delete']").live('click',function(){
        ...
        $.getJSON("<?php echo TFX_SITE_URL.DS.'index.php?app=publish&wwi=personal_image_delete';?>", { image_name: image_name }, function(json){
        });
    ...
    $("#btn_personal_link_add").click(function() {
        ...
        if(link != '') {
            $.getJSON("<?php echo TFX_SITE_URL.DS.'index.php?app=publish&wwi=personal_link_add';?>", { link: link}, function(data){
                if(data.result == "true") {
...
<div id="publish_div" style="display: none;">
  <form action="<?php echo TFX_SITE_URL;?>/index.php?app=publish&wwi=personal_save" method="post" id="add_form" class="publish-div">
    ...
    <div class="side-bar">
      ...
      <div id="personal_image_upload" class="personal-image-upload">
        <div class="upload-btn"><a href="javascript:void(0);"> <span style="width: 80px; height: 30px; position: absolute; left: 0; top: 0; z-index: 999; cursor:pointer; ">
          ...
          <input id="submit_button" style="display:none" type="button" value="<?php echo $lang['tfx_text_upload_image'];?>" onClick="submit_form($(this))" />
          </a></div>
        <div id="personal_image_info" class="personal_image_info" style="display:none;" title="<?php echo $lang['tfx_text_uploading'];?>">&nbsp;</div>
      </div>
    </div>
    <div class="express">
      <dl class="type">
        <dt><?php echo $lang['tfx_text_class'];?></dt>
        <dd>
      ...
      <dl class="intro">
        <dt><?php echo $lang['tfx_text_explain'];?></dt>
        <dd>
          <textarea cols="55" rows="3" name="commend_message" id="commend_message"></textarea>
        </dd>
      </dl>
      <dl class="url">
        <dt> <?php echo $lang['tfx_text_buy_link'];?>
          <?php if(C('taobao_api_isuse')) { ?>
          <span><?php echo $lang['tfx_personal_publish_explain_taobao'];?></span>
          <?php } else { ?>
          <span><?php echo $lang['tfx_personal_publish_explain_store'];?></span>
          <?php } ?>
        </dt>
        <dd>
          ...
          <div id="personal_image_link"><a id="btn_personal_link" href="javascript:void(0);" class="add-btn"><?php echo $lang['tfx_personal_add_link'];?></a> </div>
        </dd>
      </dl>
    </div>
    <div class="handle">
      <div id="personal_image_notice" class="personal_image_notice" style="display:none;"></div>
      <input id="btn_publish" type="button" value="<?php echo $lang['tfx_text_commend'];?>" />
      <!-- 站外分享 -->
