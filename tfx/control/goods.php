...
class goodsControl extends TfxControl{
    ...
    public function listWwi() {
        ...
        $order = 'tfx_sort asc,commend_time desc';
        if($_GET['order'] == 'hot') {
            $order = 'tfx_sort asc,click_count desc';
        }
        self::get_goods_list($condition,$order);

        Tpl::output('html_title',Language::get('nc_tfx_goods').'-'.Language::get('nc_tfx').'-'.C('site_name'));
    ...
    public function detailWwi() {
        ...
        $detail = $model_tfx_goods->getOneWithUserInfo($condition);
        if(empty($detail)) {
            header('location: '.TFX_SITE_URL);die;
        }
        ...
        $model_tfx_goods->modify($update,$condition);
        ...
        Tpl::output('html_title',$detail['commend_goods_name'].'-'.Language::get('nc_tfx_goods').'-'.Language::get('nc_tfx').'-'.C('site_name'));
