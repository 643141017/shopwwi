...
class publishControl extends TfxControl{
    ...
    public function goods_favoritesWwi() {
    ...
    private function get_commend_goods_list() {
        ...
        $commend_goods_list = $model_tfx_goods->getList(array('commend_member_id'=>$_SESSION['member_id']));
    ...
    public function goods_saveWwi() {
        ...
        if(empty($_POST['commend_message'])) {
            $commend_goods_info['commend_message'] = Language::get('tfx_goods_default_commend_message');
        } else {
            $commend_goods_info['commend_message'] = trim($_POST['commend_message']);
        }
        $commend_goods_info['commend_time'] = time();
        $commend_goods_info['tfx_commend'] = 0;
        $commend_goods_info['tfx_sort'] = 255;
        ...
        if(empty($goods_relation)) {
            ...
            if(!empty($default_class)) {
                ...
                if(empty($goods_class)) {
                    showDialog(Language::get('tfx_goods_class_none'),'reload','error','');
                } else {
        ...
        $result = $model_tfx_goods->save($commend_goods_info);
        $message = Language::get('nc_common_save_fail');
            ...
            if($result) {
                ...
                if(isset($_POST['share_app_items'])) {
                    ...
                    $commend_goods_info['url'] = TFX_SITE_URL.DS."index.php?app=goods&wwi=detail&goods_id=".$result;
     ...
     * 个人秀图片上传
     **/
    public function personal_image_uploadWwi() {
        $data = array();
        $data['status'] = 'success';
        if(isset($_SESSION['member_id'])) {
            if(!empty($_FILES['personal_image_ajax']['name'])) {
                $upload = new UploadFile();
                $upload->set('default_dir',ATTACH_TFX.DS.$_SESSION['member_id']);
     ...
     * 个人秀图片删除
     **/
    public function personal_image_deleteWwi() {
        $data = array();
        $data['status'] = 'success';
     ...
     * 个人秀数量限制检查
     **/
    public function personal_limitWwi() {
    ...
    private function check_personal_limit() {
        $personal_limit = C('tfx_personal_limit');
     ...
     * 个人秀购买链接添加
     */
    public function personal_link_addWwi() {
        ...
        if($result) {
            ...
            self::return_json(Language::get('tfx_wrong_url'),'false');
        }
    }
     ...
     * 个人秀保存
     **/
    public function personal_saveWwi() {
        ...
        if(empty($_POST['commend_message'])) {
            $personal_info['commend_message'] = Language::get('tfx_personal_default_commend_message');
        } else {
        ...
        $personal_info['tfx_commend'] = 0;
        $personal_info['tfx_sort'] = 255;
        ...
        if($result) {
            ...
            if(isset($_POST['share_app_items'])) {
                ...
                $personal_info['url'] = TFX_SITE_URL.DS."index.php?app=personal&wwi=detail&personal_id=".$result;
                self::share_app_publish('publish',$personal_info);
            }
        }
        showDialog($message,TFX_SITE_URL.DS.'index.php?app=home&wwi=personal',$result? 'succ' : 'error','');
    }



    public function albumWwi() {
