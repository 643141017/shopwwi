...
class TfxControl{
    ...
    public function __construct(){
         ...
         */
        $search_type = array();
        $search_type['goods'] = Language::get('nc_tfx_goods');
        $search_type['personal'] = Language::get('nc_tfx_personal');
         ...
         * seo
         */
        Tpl::output('html_title',Language::get('nc_tfx').'-'.C('site_name').' - Powered by ShopWWI');
        Tpl::output('seo_keywords',C('tfx_seo_keywords'));
        Tpl::output('seo_description',C('tfx_seo_description'));
    ...
    protected function get_goods_list($condition,$order='commend_time desc') {
        $model_tfx_goods = Model('micro_goods');
        $page_number = 35;
        $field = 'micro_goods.*,member.member_name,member.member_avatar';
        $list = $model_tfx_goods->getListWithUserInfo($condition,$page_number,$order,$field);
        Tpl::output('show_page',$model_tfx_goods->showpage(2));
    ...
    protected function share_app_publish($type,$publish_info=array()) {
        ...
        switch ($type) {
        case 'comment':
            $param['comment'] = "'".$_SESSION['member_name']."'".Language::get('tfx_text_zai').Language::get('nc_tfx').Language::get('tfx_text_comment').Language::get('tfx_text_le').Language::get("nc_tfx_{$publish_info['type']}_content");
            $param['title'] = "'".$_SESSION['member_name']."'".Language::get('tfx_text_zai').Language::get('nc_tfx').Language::get('tfx_text_comment').Language::get('tfx_text_le').Language::get("nc_tfx_{$publish_info['type']}_content");
            break;
        case 'publish':
            $param['comment'] = "'".$_SESSION['member_name']."'".Language::get('tfx_text_zai').Language::get('nc_tfx').Language::get('tfx_text_commend').Language::get('tfx_text_le').Language::get("nc_tfx_{$publish_info['type']}_content");
            $param['title'] = "'".$_SESSION['member_name']."'".Language::get('tfx_text_zai').Language::get('nc_tfx').Language::get('tfx_text_commend').Language::get('tfx_text_le').Language::get("nc_tfx_{$publish_info['type']}_content");
            break;
        case 'share':
            $param['comment'] = "'".$_SESSION['member_name']."'".Language::get('tfx_text_zai').Language::get('nc_tfx').Language::get('tfx_text_share').Language::get('tfx_text_le').Language::get("nc_tfx_{$publish_info['type']}_content");
            $param['title'] = "'".$_SESSION['member_name']."'".Language::get('tfx_text_zai').Language::get('nc_tfx').Language::get('tfx_text_share').Language::get('tfx_text_le').Language::get("nc_tfx_{$publish_info['type']}_content");
            break;
        }
    ...
    protected function get_share_app_personal_content($personal_info,$param) {
        $personal_image_array = getTfxPersonalImageUrl($personal_info,'list');
        $personal_image_array_tiny = getTfxPersonalImageUrl($personal_info,'tiny');
        $content_str = "
            <div class='fd-media'>
    ...
    protected function get_sidebar_list($member_id) {
        //随心看
        $model_tfx_goods = Model('micro_goods');
        $sidebar_goods_list = $model_tfx_goods->getList(array('commend_member_id'=>$member_id),null,'commend_time desc','*',9);
        Tpl::output('sidebar_goods_list',$sidebar_goods_list);
        //个人秀
        $model_tfx_personal = Model('micro_personal');
        $sidebar_personal_list = $model_tfx_personal->getList(array('commend_member_id'=>$member_id),null,'commend_time desc','*',9);
    ...
    protected function drop_personal_image($commend_image) {
        ...
        foreach ($image_array as $image_name) {
            ...
            foreach ($image as $value) {
                $image_name = BasePath.DS.ATTACH_TFX.DS.$_SESSION['member_id'].DS.$value;
     ...
     * 微商城广告
     */
    protected function get_tfx_adv($type='index') {
